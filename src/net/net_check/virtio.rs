    ...
    fn probe(&mut self, pci_device: &PciDevice) -> Result<(), &'static str> {
        ...
        if irq > 0 && irq < 255 {
            ...
            crate::serial_println!(
                "[virtio-net-drv] IRQ {} routed to vector {}",
                irq,
                crate::apic::VIRTIO_VECTOR
            );
        }
...
impl NetworkDriver for VirtioNetDriver {
    fn mac_address(&self) -> [u8; 6] {
        self.inner.as_ref().map(|d| d.mac()).unwrap_or([0; 6])
    }

    fn link_up(&self) -> bool {
        self.inner.as_ref().map(|d| d.is_link_up()).unwrap_or(false)
    }
...
pub fn register() {
    crate::drivers::register(DRIVER_INFO, || Box::new(VirtioNetDriver::new()));
    crate::drivers::net::register_net_driver(DRIVER_INFO, || Box::new(VirtioNetDriver::new()));
}
