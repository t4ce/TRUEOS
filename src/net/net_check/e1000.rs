...
use super::{NetStats, NetworkDriver};
use crate::drivers::{Driver, DriverCategory, DriverInfo, DriverStatus};
...
impl E1000Driver {
    ...
    fn reset(&mut self) {
        crate::serial_println!(
            "[E1000] Resetting device (is_ich={}, is_spt={})...",
            self.is_ich,
            self.is_spt
        );
    ...
    fn read_mac(&mut self) {
        ...
        let (done_bit, addr_shift) = if self.is_ich {
            (1 << 1, 2)
        } else {
            (1 << 4, 8)
        };
    ...
    fn enable_tx(&mut self) {
        ...
        let tctl = TCTL_EN | TCTL_PSP | (15 << TCTL_CT_SHIFT) | (64 << TCTL_COLD_SHIFT) | TCTL_RTLC;
...
impl Driver for E1000Driver {
    ...
    fn probe(&mut self, pci_device: &PciDevice) -> Result<(), &'static str> {
        ...
        self.mmio_base = crate::memory::map_mmio(bar0, E1000_MMIO_SIZE).map_err(|e| {
            crate::serial_println!("[E1000] map_mmio failed: {}", e);
            "Failed to map E1000 MMIO"
        })?;
        ...
        crate::serial_println!(
            "[E1000] STATUS={:#010X} CTRL={:#010X} RCTL={:#010X} TCTL={:#010X}",
            self.read_reg(REG_STATUS),
            self.read_reg(REG_CTRL),
            self.read_reg(REG_RCTL),
            self.read_reg(0x0400)
        );
        if self.is_spt {
            crate::serial_println!(
                "[E1000] FWSM={:#010X} CTRL_EXT={:#010X} EXTCNF_CTRL={:#010X}",
                self.read_reg(REG_FWSM),
                self.read_reg(REG_CTRL_EXT),
...
impl NetworkDriver for E1000Driver {
    ...
    fn poll(&mut self) {
        ...
        if icr & ICR_LSC != 0 {
            let status = self.read_reg(REG_STATUS);
            self.link_up
                .store(status & STATUS_LU != 0, Ordering::SeqCst);
        }
    }
...
pub fn register() {
    crate::drivers::register(DRIVER_INFO, || Box::new(E1000Driver::new()));
    crate::drivers::net::register_net_driver(DRIVER_INFO, || Box::new(E1000Driver::new()));
}
