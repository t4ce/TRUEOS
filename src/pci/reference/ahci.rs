...
impl PortMemory {
    pub fn new() -> Self {
        Self {
            cmd_list: Box::new(HbaCmdList {
                headers: [HbaCmdHeader::new(); 32],
            }),
...
pub fn init(bar5: u64) -> bool {
    ...
    crate::serial_println!(
        "[AHCI] Mapping MMIO at phys={:#x} size={:#x}",
        abar_phys,
        AHCI_MMIO_SIZE
    );
    ...
    crate::serial_println!(
        "[AHCI] Initializing at ABAR phys={:#x} virt={:#x}",
        abar_phys,
        abar_virt
    );
    ...
    crate::serial_println!(
        "[AHCI] {} ports implemented, {} command slots, 64-bit DMA: {}",
        pi.count_ones(),
        num_cmd_slots,
        s64a
    );
            ...
            if det == 3 && ipm == 1 {
                ...
                if device_type != AhciDeviceType::None {
                    ...
                    if !s64a && (clb_phys > 0xFFFF_FFFF || fb_phys > 0xFFFF_FFFF) {
                        crate::serial_println!(
                            "[AHCI] WARNING: Port {} DMA buffers above 4GB \
                            but controller lacks S64A! clb={:#x} fb={:#x}",
                            i,
                            clb_phys,
                            fb_phys
                        );
...
pub fn identify_device(port_num: u8) -> Result<u64, &'static str> {
    ...
    let port_memory = controller.port_memory[port_num as usize]
        .as_mut()
    ...
    let words = unsafe { core::slice::from_raw_parts(identify_buffer.as_ptr() as *const u16, 256) };
    ...
    for i in 27..47 {
        ...
        let c2 = (w & 0xFF) as u8;
        if c1 >= 0x20 && c1 < 0x7F {
            model.push(c1 as char);
        }
        if c2 >= 0x20 && c2 < 0x7F {
            model.push(c2 as char);
        }
    }
    ...
    for i in 10..20 {
        ...
        let c2 = (w & 0xFF) as u8;
        if c1 >= 0x20 && c1 < 0x7F {
            serial.push(c1 as char);
        }
        if c2 >= 0x20 && c2 < 0x7F {
            serial.push(c2 as char);
        }
    }
    let serial = String::from(serial.trim());

    crate::serial_println!(
        "[AHCI] Port {}: {} sectors ({} MB), model: {}, serial: {}",
        port_num,
        sector_count,
        sector_count / 2048,
        model,
...
pub fn read_sectors(
    ...
    // Find port info
    let port_idx = controller
        .ports
        .iter()
        .position(|p| p.port_num == port_num)
...
use crate::security::{DiskId, StorageOperation, StorageSecurityError};
...
pub fn secure_read_sectors(
    ...
    // Check security
    storage::check_operation(disk, op, task_id).map_err(StorageError::Security)?;

    // Audit the operation
    storage::audit_operation(task_id, disk, op, true);

    // Perform the read
    read_sectors(port_num, lba, count, buffer).map_err(StorageError::Io)
}
...
pub fn secure_write_sectors(
    ...
    // Perform the write
    write_sectors(port_num, lba, count, buffer).map_err(StorageError::Io)
}

/// Secure format disk - VERY dangerous, requires explicit unlock
///
/// This zeros out the entire disk. Danger level 5.
pub fn secure_format_disk(port_num: u8, task_id: u64) -> Result<(), StorageError> {
    let disk = DiskId(port_num);
    ...
    while formatted < total_sectors {
        write_sectors(port_num, formatted, 1, &zero_buffer).map_err(StorageError::Io)?;
        formatted += 1;
...
pub fn send_smart_command(port_num: u8, feature: u8, _has_data: bool) -> Result<(), &'static str> {
    ...
    loop {
        ...
        if (port.is & (1 << 30)) != 0 {
            return Err("SMART command error");
        }
        timeout += 1;
        if timeout > 10_000_000 {
            return Err("SMART command timeout");
        }
        core::hint::spin_loop();
    }
...
pub fn smart_read_data(port_num: u8, feature: u8) -> Result<[u8; 512], &'static str> {
    ...
    loop {
        ...
        if (port.is & (1 << 30)) != 0 {
            return Err("SMART read error");
        }
        timeout += 1;
        if timeout > 10_000_000 {
            return Err("SMART read timeout");
        }
        core::hint::spin_loop();
    }
