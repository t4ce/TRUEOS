...
pub fn smoke_test() {
    ...
    crate::log!("path: smoke_test end\n");
}

#[inline(never)]
fn smoke_test_basics() {
    let p = Path::new("/usr/bin/ls");
    crate::log!("path: p='{}' abs={}\n", p.as_str(), p.is_absolute());
    if let Some(parent) = p.parent() {
        crate::log!("path: parent='{}'\n", parent.as_str());
    } else {
        crate::log!("path: parent=<none>\n");
    }
    crate::log!(
        "path: file={:?} stem={:?} ext={:?}\n",
        p.file_name(),
...
fn smoke_test_prefix() {
    ...
    match sp_src.strip_prefix(Path::new("/usr")) {
        Ok(rest) => crate::log!("path: strip_prefix('/usr') => '{}'\n", rest.as_str()),
        Err(_) => crate::log!("path: strip_prefix('/usr') => ERR\n"),
    }
    match sp_src.strip_prefix(Path::new("/nope")) {
        ...
        Err(_) => crate::log!("path: strip_prefix('/nope') => ERR (expected)\n"),
    }
}
...
fn smoke_test_components() {
    ...
    crate::log!("path: components of '{}' =>\n", weird.as_str());
    for c in weird.components() {
        match c {
            Component::RootDir => crate::log!("  - RootDir\n"),
            Component::CurDir => crate::log!("  - CurDir\n"),
            Component::ParentDir => crate::log!("  - ParentDir\n"),
            Component::Normal(s) => crate::log!("  - Normal('{}')\n", s),
        }
    }
}
...
fn smoke_test_bytes() {
    match PathBuf::from_bytes(b"/hello/world.txt") {
        Ok(pb) => crate::log!("path: from_bytes => '{}'\n", pb.as_str()),
        Err(e) => crate::log!("path: from_bytes => ERR {:?}\n", e),
    }
    let lossy = PathBuf::from_bytes_lossy(b"/bad\xFFpath");
    crate::log!("path: from_bytes_lossy => '{}'\n", lossy.as_str());
}
