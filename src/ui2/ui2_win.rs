...
impl Ui2SurfaceWindow {
    ...
    pub fn render_julia(&self, ticks: u64, tick_hz: u64, repaint_reason: &'static str) -> bool {
        let repaint_window_id = if is_window_minimized(self.window_id) {
            0
        } else {
            self.window_id
        };
        if !crate::r::io::cabi::queue_render_julia_to_texture(
            self.tex_id,
            ticks,
            tick_hz,
            repaint_window_id,
            ...
            crate::log!(
                "ui2-surface-window: julia render queue failed window={} tex={}\n",
                self.window_id,
                self.tex_id
            );
            return false;
    ...
    pub fn render_burning_ship(
        ...
        if !crate::r::io::cabi::queue_render_burning_ship_to_texture(
            self.tex_id,
            ticks,
            tick_hz,
            repaint_window_id,
            ...
            crate::log!(
                "ui2-surface-window: burning ship render queue failed window={} tex={}\n",
                self.window_id,
                self.tex_id
            );
            return false;
