...
/// Inject relative motion while retaining the UDP tablet source identity.
///
/// UI4 selects cursor sources by `(controller, slot, endpoint, kind)`.  RDP
/// therefore cannot switch to `HID_KIND_MOUSE` for captured motion without
/// becoming a different cursor.  The reserved cursor-event fields carry the
/// original relative delta for UI4's center-snap path while the accumulated
/// coordinates keep the ordinary tablet/HUT views coherent.
pub(crate) fn inject_udp_tablet_relative_event(
    udp_device_id: u16,
    dx: i32,
    dy: i32,
    buttons_down: u32,
    ...
    let dy_i16 = dy.clamp(i16::MIN as i32, i16::MAX as i32) as i16;
    let now_ms = now_ms_u32();

    let (seq, x, y, x_raw, y_raw, q15_x, q15_y) = {
        let mut runtimes = HID_RUNTIMES.lock();
        let runtime = runtime_mut_or_insert(
            &mut runtimes,
            HID_UDP_CONTROLLER_ID,
        ...
        runtime.seq = runtime.seq.wrapping_add(1);
        runtime.mouse_x = clamp01(runtime.mouse_x + f64::from(dx) * HID_MOUSE_NORM_PER_DELTA);
        runtime.mouse_y = clamp01(runtime.mouse_y + f64::from(dy) * HID_MOUSE_NORM_PER_DELTA);
        runtime.mouse_buttons_down = buttons_down;

        let x = runtime.mouse_x;
        let y = runtime.mouse_y;
        let x_raw = ((x * 65535.0) + 0.5).clamp(0.0, 65535.0) as u16;
        let y_raw = ((y * 65535.0) + 0.5).clamp(0.0, 65535.0) as u16;
        let q15_x = ((x * 32767.0) + 0.5).clamp(0.0, 32767.0) as u16;
        let q15_y = ((y * 32767.0) + 0.5).clamp(0.0, 32767.0) as u16;
        ...
        (runtime.seq as u32, x, y, x_raw, y_raw, q15_x, q15_y)
    };
    ...
    push_cursor_event(TrueosHidCursorEvent {
        ...
        hid_kind: HID_KIND_TABLET,
        reserved0: 1,
        reserved1: dx_i16 as u16,
        buttons_down,
        wheel,
        reserved2: dy_i16 as u16,
        x,
        y,
        flags,
    });
