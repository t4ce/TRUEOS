...
fn transport_speed_from_bulk_mps(target: &MassTarget) -> usb_if::Speed {
    let max_packet_size = target
        .bulk_in_max_packet_size
        .min(target.bulk_out_max_packet_size);
    if max_packet_size >= 1024 {
        usb_if::Speed::SuperSpeed
...
fn trace_mass_io(command_tag: u32) -> bool {
    (command_tag & 0xFF) < 0x10 || (command_tag & 0x3F) == 0
}
...
fn register_block_device(
    ...
    let handle = block::register_device_with_worker(
        desc,
        UsbMassBlockDevice {
...
pub async fn mass_storage_task(
    ...
    if known_skhynix_green && transport_kind == mass::MassTransportKind::Bot {
        ...
        Timer::after(EmbassyDuration::from_millis(KNOWN_SKHYNIX_BOT_STARTUP_SETTLE_MS)).await;
        crate::log!(
    ...
    // BOT health checks must share runtime ownership with block I/O. With the
    // conservative keepalive disabled, retain the live transport without
    // periodically taking it away from queued filesystem consumers.
...
pub(crate) async fn maybe_start_mass_storage(
    ...
    let port_speed = transport_speed_from_bulk_mps(&target);
