...
fn trace_read10_phase(tag: u32, cdb: &[u8], _len: usize) -> bool {
    let Some((_lba, _blocks)) = read10_cdb_lba_blocks(cdb) else {
        return false;
    };
    let tag_low = tag & 0xFF;
    tag_low < 0x10 || (tag & 0x3F) == 0
}
...
async fn bot_command_in(
    ...
    for _ in 0..BOT_IO_RETRIES {
        ...
        let Some(result) = with_read10_phase_trace_or_none(
            bulk_out.submit_out_and_wait(&cbw),
            cmd,
            "cbw-wait",
            tag,
            bulk_out_ep,
...
async fn bot_command_no_data(
    ...
    for _ in 0..BOT_IO_RETRIES {
        let Some(result) =
            with_timeout_or_none(bulk_out.submit_out_and_wait(&cbw), BOT_IO_TIMEOUT_MS).await
        else {
            log_bot_transport_debug(
                "cbw-timeout",
                cmd,
...
async fn bot_command_out(
    ...
    for _ in 0..BOT_IO_RETRIES {
        let Some(result) =
            with_timeout_or_none(bulk_out.submit_out_and_wait(data), BOT_IO_TIMEOUT_MS).await
        else {
            log_bot_transport_debug(
                "data-timeout",
                cmd,
