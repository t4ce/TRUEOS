...
const TRUEOS_EINPROGRESS: c_int = 115;
...
struct TrueosAddrInfo {
    ...
    ai_protocol: c_int,
    ai_addrlen: u32,
    // TRUEOS targets the x86_64 Linux-like/musl libc ABI.  In that ABI
    // `ai_addr` precedes `ai_canonname`; reversing them makes Rust's
    // `LookupHost` read a null canonname as the socket-address pointer.
    ai_addr: *mut c_void,
...
const _: () = {
    assert!(core::mem::size_of::<TrueosAddrInfo>() == 48);
    assert!(core::mem::align_of::<TrueosAddrInfo>() == 8);
    assert!(core::mem::offset_of!(TrueosAddrInfo, ai_addr) == 24);
    assert!(core::mem::offset_of!(TrueosAddrInfo, ai_canonname) == 32);
    assert!(core::mem::offset_of!(TrueosAddrInfo, ai_next) == 40);
};
...
pub(crate) fn socket_poll_events(fd: c_int, events: i16) -> Option<i16> {
    const POLLIN: i16 = 0x0001;
    const POLLOUT: i16 = 0x0004;
    const POLLERR: i16 = 0x0008;
    const POLLHUP: i16 = 0x0010;
    ...
    let (backend, cabi) = {
        ...
        match sockets.get(fd)? {
            ...
            | SocketFd::MioUdp { backend } => (*backend, false),
            SocketFd::Cabi { backend, .. } => (*backend, true),
            SocketFd::PendingListener { .. } | SocketFd::PendingUdp => {
                return Some(0);
            }
        }
    };
    if cabi {
        ...
        let rc = crate::r::net::socket_cabi::trueos_cabi_socket_tcp_poll_connect(backend, 0);
        return Some(if rc == 0 {
            POLLOUT
        } else if rc == -TRUEOS_ETIMEDOUT || rc == -TRUEOS_EINPROGRESS {
            0
        } else {
            POLLERR
        });
    ...
    if events & POLLIN != 0 {
        interests |= crate::mio_compat::READY_READABLE;
    }
    if events & POLLOUT != 0 {
    ...
    if ready & crate::mio_compat::READY_WRITABLE != 0 {
        revents |= POLLOUT;
    }
    if ready & crate::mio_compat::READY_ERROR != 0 {
        revents |= POLLERR;
    }
...
pub unsafe extern "C" fn socket(domain: c_int, socket_type: c_int, protocol: c_int) -> c_int {
    let base_socket_type = socket_type & TRUEOS_SOCK_TYPE_MASK;
    let nonblocking = socket_type & TRUEOS_SOCK_NONBLOCK != 0;
    if domain == TRUEOS_AF_INET
        && base_socket_type == TRUEOS_SOCK_DGRAM
    ...
    if SOCKET_FDS
        ...
        .is_err()
    {
        let _ = crate::r::net::socket_cabi::trueos_cabi_socket_tcp_close(backend as u32);
        TRUEOS_ERRNO.store(TRUEOS_EAGAIN, Ordering::Relaxed);
        return -1;
    }
    if socket_type & TRUEOS_SOCK_CLOEXEC != 0 {
        let _ = FD_FLAGS.lock().insert(fd, TRUEOS_FD_CLOEXEC);
    }
    crate::hv::hvlogf(format_args!(
        "std-abi socket-open fd={} backend={} nonblocking={}",
        fd, backend, nonblocking
    ));
    TRUEOS_ERRNO.store(0, Ordering::Relaxed);
    fd
}
...
pub unsafe extern "C" fn setsockopt(
    ...
    let rc = crate::r::net::socket_cabi::trueos_cabi_socket_tcp_set_nonblocking(backend, 0);
...
pub unsafe extern "C" fn accept4(
    ...
    let child_fd = next_file_fd();
    if SOCKET_FDS
        .lock()
        .insert(child_fd, SocketFd::MioStream { backend: child })
        .is_err()
    {
        let _ = unsafe { crate::mio_compat::trueos_mio_socket_close(child) };
        TRUEOS_ERRNO.store(TRUEOS_EAGAIN, Ordering::Relaxed);
    ...
    crate::hv::hvlogf(format_args!(
        "std-abi socket-accept listener_fd={} listener_backend={} stream_fd={} stream_backend={}",
        socket_id, backend, child_fd, child
    ));
    TRUEOS_ERRNO.store(0, Ordering::Relaxed);
    child_fd
}
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn shutdown(socket_id: c_int, how: c_int) -> c_int {
    if !(0..=2).contains(&how) {
        TRUEOS_ERRNO.store(TRUEOS_EINVAL, Ordering::Relaxed);
        return -1;
    }
    let cabi_backend = {
        let sockets = SOCKET_FDS.lock();
        match sockets.get(socket_id) {
            Some(SocketFd::Cabi { backend, .. } | SocketFd::PendingListener { backend, .. }) => {
                Some(*backend)
            }
            Some(_) => None,
            None => {
    ...
    if let Some(backend) = cabi_backend {
        return posix_rc_i32(crate::r::net::socket_cabi::trueos_cabi_socket_tcp_shutdown(
            backend, how as u32,
        ));
    }
    // Mio closes the backend when the owning TcpStream is dropped. Treat the
    // half-close as successful until the virtual TCP API grows half-close support.
    TRUEOS_ERRNO.store(0, Ordering::Relaxed);
    0
}
...
pub unsafe extern "C" fn connect(socket_id: c_int, addr: *const c_void, addr_len: u32) -> c_int {
    ...
    let socket = {
        let sockets = SOCKET_FDS.lock();
        match sockets.get(socket_id) {
            Some(SocketFd::MioUdp { backend }) => (Some(*backend), None),
            ...
            Some(SocketFd::Cabi {
                backend,
                nonblocking,
            }) => (None, Some((*backend, *nonblocking, false))),
    ...
    let Some((backend, nonblocking, pending)) = socket.1 else {
        TRUEOS_ERRNO.store(TRUEOS_EBADF, Ordering::Relaxed);
        return -1;
    };
    if pending && nonblocking {
        ...
        if !replaced {
            let _ = unsafe { crate::mio_compat::trueos_mio_socket_close(native_backend) };
            TRUEOS_ERRNO.store(TRUEOS_EBADF, Ordering::Relaxed);
            return -1;
        }
...
pub(crate) fn socket_set_nonblocking(fd: c_int, nonblocking: bool) -> Result<(), c_int> {
    let backend = {
        let sockets = SOCKET_FDS.lock();
        match sockets.get(fd) {
            Some(SocketFd::PendingListener { backend, .. } | SocketFd::Cabi { backend, .. }) => {
                Some(*backend)
            }
            Some(
                SocketFd::MioListener { .. }
...
pub unsafe extern "C" fn getsockopt(
    ...
    let value: c_int = if level == SOL_SOCKET && optname == SO_ERROR {
        enum ErrorBackend {
            Cabi(u32),
            Mio(u32),
            None,
        ...
        let backend = {
            let sockets = SOCKET_FDS.lock();
            match sockets.get(socket_id) {
                Some(SocketFd::Cabi { backend, .. }) => ErrorBackend::Cabi(*backend),
                Some(SocketFd::MioStream { backend }) => ErrorBackend::Mio(*backend),
                _ => ErrorBackend::None,
            }
        };
        match backend {
            ErrorBackend::Cabi(backend) => {
                crate::r::net::socket_cabi::trueos_cabi_socket_tcp_take_error(backend).max(0)
            }
            ErrorBackend::Mio(backend) => {
                let status = unsafe { crate::mio_compat::trueos_mio_socket_take_error(backend) };
                if status == 0 {
                    0
                } else {
                    mio_status_to_errno(status)
                }
            }
            ErrorBackend::None => 0,
        }
    ...
    if len < core::mem::size_of::<c_int>() as u32
        || !copy_to_abi_out(optval.cast::<u8>(), &value.to_ne_bytes())
        || !copy_to_abi_out(
            optlen.cast::<u8>(),
...
pub unsafe extern "C" fn send(
    ...
    if let Some(backend) = udp_backend {
        ...
        let mut peer = crate::mio_compat::TrueosMioSocketAddr::default();
        let rc = unsafe { crate::mio_compat::trueos_mio_socket_peer_addr(backend, &mut peer) };
        if rc != 0 {
    ...
    let (backend, is_mio) = {
        let sockets = SOCKET_FDS.lock();
        match sockets.get(socket_id) {
            Some(SocketFd::MioStream { backend }) => (*backend, true),
            Some(socket) => (socket.backend(), false),
            None => {
                TRUEOS_ERRNO.store(TRUEOS_EBADF, Ordering::Relaxed);
                return -1;
            }
    ...
    let raw = if is_mio {
        ...
        unsafe {
            crate::mio_compat::trueos_mio_tcp_stream_write(backend, input.as_ptr(), input.len())
        }
    } else {
...
pub unsafe extern "C" fn recv(
    ...
    if let Some(backend) = udp_backend {
        ...
        return posix_mio_isize(unsafe {
            crate::mio_compat::trueos_mio_udp_socket_recv_from(
                backend,
                &mut from,
                output.as_mut_ptr(),
                output.len(),
    ...
    // native TRUEOS selector reports it readable.
    let unix_rx = {
        let files = OPEN_FILES.lock();
        match files.get(socket_id) {
    ...
    let raw = if is_mio {
        ...
        unsafe {
            crate::mio_compat::trueos_mio_tcp_stream_read(
                backend,
                output.as_mut_ptr(),
                output.len(),
            )
        ...
        crate::r::net::socket_cabi::trueos_cabi_socket_tcp_recv(
            backend,
            buf.cast::<u8>(),
            len,
            flags,
...
pub unsafe extern "C" fn getpeername(
    ...
    let peer = {
        ...
        match socket {
            SocketFd::Cabi { backend, .. } => {
                let mut addr_be = 0u32;
                let mut port_be = 0u16;
                let rc = crate::r::net::socket_cabi::trueos_cabi_socket_tcp_peer_v4(
                    *backend,
                    &mut addr_be,
                    &mut port_be,
                );
