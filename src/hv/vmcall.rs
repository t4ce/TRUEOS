...
pub const OP_BP_UI4_SCENE_WRITE_OPAQUE_RGBA8: u32 = 0xC8; // arg0 window,arg1 byte offset,payload RGBA8 -> rc
    ...
    match op {
            ...
            let batch = request_payload(vm_id, req_len)
                ...
                .map(|payload| unsafe {
                    core::ptr::read_unaligned(
                        payload.as_ptr().cast::<v::vgpu::IndexedDrawBatchV2>(),
                    )
                });
            let result = batch.ok_or(-22).and_then(|batch| {
        ...
        OP_BP_VGPU_RETAINED_MESH_CREATE => {
            ...
            let result = descriptor.ok_or(-22).and_then(|descriptor| {
                crate::r::io::vgpu_cabi::broker_retained_mesh_create(principal, arg0, descriptor)
            });
            match result {
        ...
        OP_BP_VGPU_RETAINED_FRAME_SUBMIT => {
            ...
            let submit = request_payload(vm_id, req_len)
                ...
                .map(|payload| unsafe {
                    core::ptr::read_unaligned(
                        payload.as_ptr().cast::<v::vgpu::RetainedFrameSubmit>(),
                    )
                });
            let result = submit.ok_or(-22).and_then(|submit| {
                crate::r::io::vgpu_cabi::broker_retained_frame_submit(principal, arg0, arg1, submit)
            });
            match result {
                Ok(point) => write_record_response(vm_id, seq, 0, &point),
                Err(rc) => write_response(vm_id, seq, STATUS_OK, (rc as i64) as u64, 0),
        ...
        OP_BP_VGPU_RETAINED_FRAME_SUBMIT_V2 => {
            let principal = crate::gpu::vgpu::Principal::HullGuest(vm_id as u16);
            let submit = request_payload(vm_id, req_len)
                .filter(|payload| {
                    payload.len() == core::mem::size_of::<v::vgpu::RetainedFrameSubmitV2>()
                })
                .map(|payload| unsafe {
                    core::ptr::read_unaligned(
                        payload.as_ptr().cast::<v::vgpu::RetainedFrameSubmitV2>(),
                    )
                });
            let result = submit.ok_or(-22).and_then(|submit| {
        ...
        OP_BP_VGPU_RETAINED_FRAME_SUBMIT_V3 => {
            let principal = crate::gpu::vgpu::Principal::HullGuest(vm_id as u16);
            let submit = request_payload(vm_id, req_len)
                .filter(|payload| {
                    payload.len() == core::mem::size_of::<v::vgpu::RetainedFrameSubmitV3>()
                })
                .map(|payload| unsafe {
                    core::ptr::read_unaligned(
                        payload.as_ptr().cast::<v::vgpu::RetainedFrameSubmitV3>(),
                    )
                });
            let result = submit.ok_or(-22).and_then(|submit| {
                crate::r::io::vgpu_cabi::broker_retained_frame_submit_v3(
                    principal, arg0, arg1, submit,
                )
            });
            match result {
        ...
        OP_BP_VGPU_RETAINED_FRAME_SUBMIT_V4 => {
            let principal = crate::gpu::vgpu::Principal::HullGuest(vm_id as u16);
            let submit = request_payload(vm_id, req_len)
                .filter(|payload| {
                    payload.len() == core::mem::size_of::<v::vgpu::RetainedFrameSubmitV4>()
                })
                .map(|payload| unsafe {
                    core::ptr::read_unaligned(
                        payload.as_ptr().cast::<v::vgpu::RetainedFrameSubmitV4>(),
                    )
                });
            let result = submit.ok_or(-22).and_then(|submit| {
                crate::r::io::vgpu_cabi::broker_retained_frame_submit_v4(
                    principal, arg0, arg1, submit,
                )
            });
            match result {
        ...
        OP_BP_UI4_SCENE_SET_CURSOR_STEP => {
            let step = match req_len as usize {
                0 => None,
                len if len
                    == core::mem::size_of::<crate::ui4::blueprint_text::TrueosUi4CursorStep>() =>
                {
                    let Some(payload) = request_payload(vm_id, req_len) else {
                        write_response(vm_id, seq, STATUS_BAD_ARG, 0, 0);
                        return DispatchOutcome::Resume;
                    ...
                    Some(unsafe {
                        core::ptr::read_unaligned(
                            payload
                                .as_ptr()
                                .cast::<crate::ui4::blueprint_text::TrueosUi4CursorStep>(),
                        )
                    })
                }
                _ => {
            ...
            let rc = unsafe {
                crate::ui4::blueprint_text::trueos_cabi_ui4_scene_set_cursor_step(
                    arg0 as u32,
                    step.as_ref()
        ...
        OP_BP_AUDIO_NATIVE_RENDER_V2 => {
            use crate::aud::native_engine::{
                MAX_COMMANDS, NativeBlockHeaderV2, NativeRenderCommandV2,
            };

            let count = arg0 as usize;
            let header_size = core::mem::size_of::<NativeBlockHeaderV2>();
            let command_size = core::mem::size_of::<NativeRenderCommandV2>();
            let expected = count
                .checked_mul(command_size)
                .and_then(|bytes| header_size.checked_add(bytes));
            let n = core::cmp::min(req_len as usize, PAYLOAD_CAP);
            ...
            for index in 0..count {
                ...
                commands.push(unsafe {
                    core::ptr::read_unaligned(
                        data.as_ptr().add(offset).cast::<NativeRenderCommandV2>(),
                    )
                });
            }
            let log_sample = sampled_log(&BLUEPRINT_AUDIO_WRITE_LOG_SEQ);
            if log_sample {
                crate::log_info!(
                    target: "audio";
                    "blueprint-audio-native-vmcall-v2: vm={} frames={} commands={} revision={} absolute_frame={}\n",
                    vm_id, header.block_frames, count, header.revision, header.absolute_frame
                );
            }
            let rc = match crate::aud::native_engine::render_block_v2(&header, &commands) {
                Ok(pcm) => match crate::aud::pcm_lane::submit_i16_stereo_48k(
                    "blueprint-audio-native-vmcall-v2",
                    pcm,
                ) {
                    Ok(frames) => frames as i64,
                    Err(crate::aud::pcm_lane::PcmLaneError::QueueFull) => -16,
            ...
            if log_sample {
                crate::log_info!(
                    target: "audio";
                    "blueprint-audio-native-vmcall-v2: vm={} rc={} pending_frames={}\n",
