...
use crate::{
    Error, Q8_BLOCK_BYTES, Q8_BLOCK_VALUES, q8_row_bytes, quantize_q8,
};
...
impl Q8VnniProjector {
    ...
    pub fn project_rows(
        ...
        if matrix.len() != matrix_bytes
            || last_row > rows
            || activation.columns() != columns
        {
            return Err(Error::Shape);
        }
...
pub fn validate_q8_vnni_matrix(
    matrix: &[u8],
    rows: usize,
    columns: usize,
) -> Result<(), Error> {
...
unsafe fn project_rows_avx_vnni(
    ...
    use core::arch::x86_64::{
        __m256i, _mm256_cvtepi32_ps, _mm256_dpbusd_avx_epi32,
        _mm256_fmadd_ps, _mm256_loadu_si256, _mm256_set1_ps,
        _mm256_setzero_ps, _mm256_setzero_si256, _mm256_sign_epi8,
        _mm256_storeu_ps,
    };
    ...
    macro_rules! update_row {
        ($accumulator:expr, $magnitude:expr, $activation_signs:expr, $weight_block:expr, $scale:expr) => {{
            let weight_block = $weight_block;
            let weights = unsafe {
                _mm256_loadu_si256(weight_block.add(2).cast::<__m256i>())
            };
            let signed_weights = _mm256_sign_epi8(weights, $activation_signs);
            let dots = _mm256_dpbusd_avx_epi32(
    ...
    while relative_row + Q8_VNNI_ROWS_PER_TILE <= output.len() {
        let row0 = unsafe {
            matrix
                .as_ptr()
                .add((first_row + relative_row) * row_bytes)
        };
        ...
        for block_index in 0..activation.block_count() {
            ...
            let activation_signs = unsafe {
                _mm256_loadu_si256(activation_block.as_ptr().add(2).cast::<__m256i>())
            };
...
fn detect_q8_vnni_capabilities() -> Q8VnniCapabilities {
    ...
    let avx_vnni =
        avx2 && leaf7.eax >= 1 && __cpuid_count(7, 1).eax & CPUID_7_1_EAX_AVX_VNNI != 0;
    Q8VnniCapabilities {
        ymm_state: true,
        avx2,
        avx_vnni,
...
mod tests {
    ...
    fn one_time_matrix_admission_rejects_signed_byte_minimum() {
        ...
        matrix[2] = 0x80;
        assert_eq!(
            validate_q8_vnni_matrix(&matrix, 1, 32),
            Err(Error::Encoding)
        );
    }
    ...
    fn every_available_vnni_tile_matches_the_pinned_scalar_oracle_bitwise() {
        ...
        for (rows, columns) in [(1, 1024), (4, 1024), (7, 1024), (5, 4608)] {
            ...
            let expected = crate::q8_matrix_vector_quantized(&matrix, rows, columns, &input)
                .unwrap();
    ...
    fn row_range_surface_preserves_global_row_order() {
        ...
        projector
            ...
            .unwrap();
        assert_eq!(
            middle.iter().map(|value| value.to_bits()).collect::<Vec<_>>(),
            all[2..7]
                .iter()
