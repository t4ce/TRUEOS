...
use crate::intel::gpgpu::{
    ...
    ShaderToyRuntimeState, SkyboxSampleRgb565Params, Ui4CompositorCompletion,
...
struct BlueprintSceneSurface {
    // Surface identity is distinct from the one broker/input window. A
    // background shares owner/session/window but owns all producer state.
    render_target: u32,
    owner: WindowOwner,
    ...
    pending_physical_keyboard: VecDeque<crate::r::keyboard::TrueosKeyboardOutputEvent>,
    pending_physical_burst: VecDeque<crate::r::keyboard::TrueosKeyboardOutputEvent>,
...
fn open_blueprint_frame(
    ...
    open_blueprint_surface(x, y, width, height, cadence, visual_target_hz, None)
}
...
fn open_blueprint_surface(
    ...
    let render_target = if parent.is_some() {
        if super::window_broker::attach_window_background(owner, window, frame).is_err() {
            let _ = destroy_frame(frame);
            return 0;
        }
...
/// Version 1 physical keyboard stream. The record layout is unchanged;
/// kind 4 uses USB HID keyboard-page usages in key_code, with PRESS clear on
/// release. Synthetic committed text retains kind 1. Kind 3 resets held state.
/// The legacy poll remains independent and receives no physical duplicates.
pub unsafe extern "C" fn trueos_cabi_ui4_scene_keyboard_event_take_v1(
    window_id: u32,
    out: *mut crate::r::keyboard::TrueosKeyboardOutputEvent,
) -> i32 {
    if out.is_null() {
            ...
            Ui4InputEvent::Keyboard(event) => {
                ...
                if event.event.flags & crate::r::keyboard::KEYBOARD_OUTPUT_FLAG_SYNTHETIC != 0 {
                    enqueue_window_keyboard_event(
                        &mut surface.pending_physical_keyboard,
                        &mut surface.pending_physical_burst,
                        event.event,
                    );
                }
                enqueue_window_keyboard_event(
...
fn enqueue_physical_keyboard_event(
    ...
    if pending.len() >= MAX_PENDING_KEYBOARD_EVENTS {
        // A dropped release must never leave an application's key held forever.
        pending.clear();
        pending.push_back(crate::r::keyboard::TrueosKeyboardOutputEvent {
            kind: crate::r::keyboard::KEYBOARD_OUTPUT_KIND_DEVICE_LOST,
            flags: crate::r::keyboard::KEYBOARD_OUTPUT_FLAG_DEVICE_LOST,
            ..Default::default()
        });
...
pub unsafe extern "C" fn trueos_cabi_ui4_scene_frame_get_position(
    window_id: u32,
    out_xy: *mut i32,
) -> i32 {
    if out_xy.is_null() {
    ...
    if crate::hv::current_hull_guest_context_vm_id().is_some() {
        ...
        unsafe {
            core::ptr::copy_nonoverlapping(bytes.as_ptr(), out_xy.cast::<u8>(), 8);
        }
        return 0;
    }
    let Some(owner) = blueprint_owner() else {
    ...
    let Some(surface) = surfaces
        .iter()
        .find(|s| s.owner == owner && s.window.raw() == window_id)
    else {
        return ERROR_NOT_FOUND;
    };
    let Ok((placement, _)) = window_resize_state(owner, surface.window) else {
        return ERROR_UI4;
    };
    unsafe {
        out_xy.write(placement.x);
        out_xy.add(1).write(placement.y);
    }
    0
}
...
pub extern "C" fn trueos_cabi_ui4_scene_frame_set_position(window_id: u32, x: i32, y: i32) -> i32 {
    ...
    if let Some(pending) = &mut surface.pending_resize {
        // A resize commits its placement with its first published frame. Keep
        // moves made before that publish instead of snapping to the old origin.
        pending.placement.x = placement.x;
        pending.placement.y = placement.y;
        pending.previous_placement.x = placement.x;
        pending.previous_placement.y = placement.y;
    }
    0
}
...
/// Allow primary selection clicks through to this frame while it is a compact control.
pub extern "C" fn trueos_cabi_ui4_scene_frame_primary_activation(
    window_id: u32,
    enabled: u32,
) -> i32 {
    if enabled > 1 {
    ...
    if crate::hv::current_hull_guest_context_vm_id().is_some() {
        return guest_status(
            trueos_vm::vmcall::OP_BP_UI4_SCENE_FRAME_PRIMARY_ACTIVATION,
            window_id as u64,
            enabled as u64,
            &[],
        );
    ...
    match super::window_broker::set_window_primary_activation(owner, window, enabled != 0) {
        Ok(()) => 0,
        Err(_) => ERROR_UI4,
    }
}
...
pub unsafe extern "C" fn trueos_cabi_ui4_scene_shadertoy_render(
    ...
    let (lease, destination, mut state) = {
        ...
        if surface.shadertoy_registered & (1u32 << shadertoy_program_id(params.shader_id).unwrap())
            == 0
        {
            return ERROR_STATE;
        }
...
pub extern "C" fn trueos_cabi_ui4_solara_frame_publish(
    ...
    if let Some(pending) = surface.pending_resize {
        ...
        } else if surface.render_target != surface.window.raw() {
            super::window_broker::publish_window_background(owner, surface.window, damage)
        } else {
...
fn commit_layered_resize_if_ready(
    ...
    for index in members {
        ...
        surface.pending_resize_ready = false;
        super::retire_frame_when_released(old);
    }
    0
}
...
fn release_surface(mut surface: BlueprintSceneSurface, release: BlueprintSurfaceRelease) {
    ...
    let close = if surface.render_target == surface.window.raw() {
        finish_window_session_with_request(surface.owner, surface.session, request)
    } else {
        // The sole window owner performs the shared close. Surface storage
        // remains protected by the normal compositor/display reader leases.
        super::retire_frame_when_released(surface.frame);
        return;
    };
