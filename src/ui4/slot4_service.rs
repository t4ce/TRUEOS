...
fn software_cursor_rects() -> Slot4Rects {
    ...
    for visual in &visuals {
        if visual
            .cell_outline
            .is_some_and(|outline| push_cursor_cell_outline(
                &mut rects,
                visual.x,
                visual.y,
                screen_w,
...
/// Let a selected Blueprint supply static grid geometry once, while slot 4
/// keeps the outline on the cursor plane at display cadence. Returning false
/// preserves the normal crosshair in window chrome and outside the grid.
fn push_cursor_cell_outline(
    ...
    visual: super::Ui4CursorCellOutlineVisual,
) -> bool {
    let Some(window) = super::window_broker::window_snapshot(visual.key.owner, visual.key.window)
    else {
        return false;
    };
    ...
    let local_x = i64::from(cursor_x).saturating_sub(i64::from(placement.x));
    let local_y = i64::from(cursor_y).saturating_sub(i64::from(placement.y));
    let (Ok(local_x), Ok(local_y)) = (u32::try_from(local_x), u32::try_from(local_y)) else {
        return false;
    };
    let Some((x, y, width, height)) = cell_outline_bounds(local_x, local_y, visual.outline)
    else {
        return false;
    };
    let x = i64::from(placement.x).saturating_add(i64::from(x));
    let y = i64::from(placement.y).saturating_add(i64::from(y));
    push_clipped_rect_border(rects, x, y, width, height, visual.outline.stroke, screen_w, screen_h, color);
    true
}

fn cell_outline_bounds(
    local_x: u32,
    local_y: u32,
    outline: super::Ui4CursorCellOutline,
) -> Option<(u32, u32, u32, u32)> {
    if !outline.is_valid() || local_x < outline.origin_x || local_y < outline.origin_y {
        return None;
    }
    let horizontal = u64::from(local_x.saturating_sub(outline.origin_x));
    let column = horizontal
        .saturating_mul(super::Ui4CursorCellOutline::SUBPIXELS_PER_PIXEL)
        .checked_div(u64::from(outline.cell_width_subpx))?;
    let row = u32::from(local_y.saturating_sub(outline.origin_y))
        .checked_div(outline.cell_height)?;
    if column >= u64::from(outline.columns) || row >= outline.rows {
        return None;
    }
    let left_subpx = column.saturating_mul(u64::from(outline.cell_width_subpx));
    let right_subpx = left_subpx.saturating_add(u64::from(outline.cell_width_subpx));
    let round_subpx = |value: u64| {
        value
            .saturating_add(super::Ui4CursorCellOutline::SUBPIXELS_PER_PIXEL / 2)
            .checked_div(super::Ui4CursorCellOutline::SUBPIXELS_PER_PIXEL)
            .unwrap_or(0)
    };
    let left = u64::from(outline.origin_x).saturating_add(round_subpx(left_subpx));
    let right = u64::from(outline.origin_x).saturating_add(round_subpx(right_subpx));
    let top = u64::from(outline.origin_y)
        .saturating_add(u64::from(row).saturating_mul(u64::from(outline.cell_height)));
    let bottom = top.saturating_add(u64::from(outline.cell_height));
...
fn push_clipped_rect_border(
    ...
    let stroke = stroke.min(width / 2).min(height / 2).max(1);
    let inner_height = height.saturating_sub(stroke.saturating_mul(2));
    push_clipped_overlay_rect(rects, x, y, width, stroke, screen_w, screen_h, color);
    push_clipped_overlay_rect(
        rects,
        x,
        y.saturating_add(i64::from(height.saturating_sub(stroke))),
        width,
        stroke,
        screen_w,
        screen_h,
    ...
    if inner_height != 0 {
        push_clipped_overlay_rect(
            rects,
            x,
            y.saturating_add(i64::from(stroke)),
            stroke,
            inner_height,
            screen_w,
            screen_h,
        ...
        push_clipped_overlay_rect(
            rects,
            x.saturating_add(i64::from(width.saturating_sub(stroke))),
            y.saturating_add(i64::from(stroke)),
            stroke,
            inner_height,
            screen_w,
...
fn push_clipped_overlay_rect(
    ...
    let right = x.saturating_add(i64::from(width));
    let bottom = y.saturating_add(i64::from(height));
    let left = x.clamp(0, i64::from(screen_w));
    let top = y.clamp(0, i64::from(screen_h));
    let right = right.clamp(0, i64::from(screen_w));
    let bottom = bottom.clamp(0, i64::from(screen_h));
    if right > left && bottom > top {
        push_overlay_rect(
            rects,
            left as u32,
