...
use super::{blueprint_owner, display_registry::DisplayRegistry, surface_mut, SURFACES,
    ERROR_CONTEXT, ERROR_NOT_FOUND, ERROR_UI4};
...
pub extern "C" fn trueos_cabi_ui4_display_open_v1() -> u64 {
    if crate::hv::current_hull_guest_context_vm_id().is_some() {
        ...
        return if status == trueos_vm::vmcall::STATUS_OK { token } else { 0 };
    }
...
pub extern "C" fn trueos_cabi_ui4_display_retain_v1(connection: u64) -> i32 {
    if crate::hv::current_hull_guest_context_vm_id().is_some() {
        let (status, result) = guest_call(trueos_vm::vmcall::OP_BP_UI4_DISPLAY_RETAIN_V1, connection, 0);
        return if status == trueos_vm::vmcall::STATUS_OK { result as i64 as i32 } else { ERROR_UI4 };
    }
    let Some(owner) = blueprint_owner() else { return ERROR_CONTEXT; };
    if DISPLAYS.lock().retain(owner, connection) { 0 } else { ERROR_NOT_FOUND }
}

pub extern "C" fn trueos_cabi_ui4_display_close_v1(connection: u64) -> i32 {
    if crate::hv::current_hull_guest_context_vm_id().is_some() {
        let (status, result) = guest_call(trueos_vm::vmcall::OP_BP_UI4_DISPLAY_CLOSE_V1, connection, 0);
        return if status == trueos_vm::vmcall::STATUS_OK { result as i64 as i32 } else { ERROR_UI4 };
    }
    let Some(owner) = blueprint_owner() else { return ERROR_CONTEXT; };
    if DISPLAYS.lock().close(owner, connection) { 0 } else { ERROR_NOT_FOUND }
}

pub extern "C" fn trueos_cabi_ui4_display_validate_window_v1(connection: u64, window: u32) -> i32 {
    if crate::hv::current_hull_guest_context_vm_id().is_some() {
        let (status, result) = guest_call(trueos_vm::vmcall::OP_BP_UI4_DISPLAY_VALIDATE_WINDOW_V1, connection, window);
        return if status == trueos_vm::vmcall::STATUS_OK { result as i64 as i32 } else { ERROR_UI4 };
    }
    let Some(owner) = blueprint_owner() else { return ERROR_CONTEXT; };
    if !DISPLAYS.lock().valid(owner, connection) { return ERROR_NOT_FOUND; }
    let surfaces = &mut *SURFACES.lock();
    let Some(surface) = surface_mut(surfaces, owner, window) else { return ERROR_NOT_FOUND; };
    if crate::ui4::window_state(owner, surface.window).is_ok() { 0 } else { ERROR_NOT_FOUND }
}
