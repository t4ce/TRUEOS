...
pub(super) fn claim_group(
    current: GroupPlan,
    requested: LeaseGroup,
    now_ms: u64,
    grace_ms: u64,
    raise: bool,
) -> Option<GroupPlan> {
    if !(1..=2).contains(&requested.slots) {
        return None;
    }
    let mut plan = current;
    if let Some(index) = plan
        .iter()
        .position(|group| group.is_some_and(|g| g.window == requested.window))
    {
        if plan[index]?.slots != requested.slots {
            return None;
        }
...
mod tests {
    ...
    fn second_dual_waits_for_first_dual_to_be_revocable() {
        ...
        assert_eq!(
            claim_group(initial, g(2, 2, 1400), 1400, 500, true),
            Some([Some(g(2, 2, 1400)), None, None])
        );
    }
    #[test]
    fn focus_raises_both_members_over_the_other_dragged_window() {
        let initial = [Some(g(1, 2, 900)), Some(g(2, 1, 900)), None];
        assert_eq!(
            claim_group(initial, g(1, 2, 1000), 1000, 500, true),
            Some([Some(g(2, 1, 900)), Some(g(1, 2, 1000)), None])
        );
    }
    #[test]
