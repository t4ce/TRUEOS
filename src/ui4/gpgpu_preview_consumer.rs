...
enum CppFontRushLayerStage {
    ...
    TitleHold,
    /// Publish one transparent frame before any cache setup starts.
    BlankPrime,
    /// Keep the proven blank scanout live until cache charge and the minimum
    /// two-second visual pause have both completed.
    BlankHold,
...
fn cpp_font_rush_showcase_next_stage(
    ...
    Ok(match stage {
        ...
        CppFontRushLayerStage::CacheCharge { .. } => CppFontRushLayerStage::BlankHold,
...
fn advance_cpp_font_rush_showcase(
    ...
    if stage == CppFontRushLayerStage::BlankPrime {
        ...
        crate::log_info!(
            target: "ui4";
            "ui4 cpp-font-rush rgba8 cache allocated request={} cache={} classes={} entries_per_class={} atlas={}x{} tile={}x{} bytes={} lifecycle=run-owned blank_live_ms={} charge=behind-blank-before-section-pulse\n",
            request_serial,
            cache.id(),
            crate::r::font_kernel_service::FONT_RUSH_RGBA8_CACHE_CLASSES,
            ...
            stage_started.saturating_duration_since(run_started).as_millis(),
        );
        previews[0]
            .font_rush
            .as_mut()
    ...
    if matches!(stage, CppFontRushLayerStage::CacheCharge { .. })
        && next == CppFontRushLayerStage::BlankHold
    {
        previews[0]
            .font_rush
            .as_ref()
...
async fn drain_cpp_font_rush_pending(previews: &mut [ActivePreview]) {
    ...
    for preview in previews {
        let (
            ...
            cache_charge,
            pending,
            title_source_pending,
            section_source_pending,
        ) = preview
            .font_rush
            .as_mut()
            .map(|state| {
...
mod tests {
    use super::{
        CPP_FONT_RUSH_BLANK_MIN_MS, CPP_FONT_RUSH_CACHE_FONT_PIXELS, CPP_FONT_RUSH_CADENCE_MS,
        CPP_FONT_RUSH_SECTION_CADENCE_MS, CPP_FONT_RUSH_SECTION_PRESENTATION_SCALE,
    ...
    #[test]
    fn cpp_font_rush_blanks_and_charges_before_section_pulse() {
        ...
        let mut stage =
            cpp_font_rush_showcase_next_stage(CppFontRushLayerStage::BlankPrime).unwrap();
        let mut charges = 0usize;
        while matches!(stage, CppFontRushLayerStage::CacheCharge { .. }) {
            charges += 1;
            stage = cpp_font_rush_showcase_next_stage(stage).unwrap();
        }
        assert_eq!(
            charges,
            crate::r::font_kernel_service::FONT_RUSH_RGBA8_CACHE_CLASSES
    ...
    #[test]
    fn cpp_font_rush_showcase_scales_cached_pixels_without_clipping() {
