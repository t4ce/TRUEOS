...
/// Own one complete subscriber session on the calling LastAP task. Frame
/// generation, Annex-B fragmentation, checked network receipts, and cadence
/// all remain in this one async state machine; waits yield cooperatively.
    ...
    F: AsyncFnMut(u32) -> Option<Vec<u8>>,
{
    ...
    crate::log_info!(target: "intel/media-encode";
        "intel/media-encode: udp-live staged=1 protocol=tme1 version={} session={} payload=annexb-access-units target_fps={} target_units={} live_high_water_cap=1 pipeline=single-lastap-task transport=udp-subscribe-unicast listen_port={} subscriber_token=TME1GET1 network=waiting\n",
        VERSION,
        request.session_id,
        request.target_hz,
    ...
    crate::log_info!(target: "intel/media-encode";
        "intel/media-encode: udp-live subscriber=accepted session={} peer={}.{}.{}.{}:{} wait_polls={} action=start-single-task-capture-encode-send\n",
        request.session_id,
        remote.addr[0],
        remote.addr[1],
        remote.addr[2],
    ...
    let period_ticks = (embassy_time::TICK_HZ / request.target_hz as u64).max(1);
    let started_ns = crate::chronos::monotonic_nanos();
    let mut next_deadline = Instant::now();
    let mut datagram_sequence = 0u32;
    for index in 0..request.access_unit_count {
        if index != 0 {
            ...
            if now > next_deadline {
                ...
                report.max_late_us = report
                    .max_late_us
                    .max(now.saturating_duration_since(next_deadline).as_micros());
                // Do not emit catch-up bursts after hardware or NIC pressure.
                next_deadline = now;
            }
        }
        ...
        report.high_water_access_units = 1;
