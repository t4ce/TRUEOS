...
fn text_bounds(msg: &[u8], fb_w: f32, fb_h: f32) -> (f32, f32, f32, f32) {
    let atlas = crate::gfx::text::font_atlas_large_view();
    let (x, y) = centered_text_origin(msg, fb_w, fb_h);
...
pub async fn gfx_loadscreen_task() {
    ...
    let (bounds_x, bounds_y, bounds_w, bounds_h) = text_bounds(MSG, fb_w, fb_h);
    let clear_x = (bounds_x - TEXT_PAD_X).max(0.0);
    let clear_y = (bounds_y - TEXT_PAD_Y).max(0.0);
    let clear_w = (bounds_w + TEXT_PAD_X * 2.0).min(fb_w - clear_x);
