...
fn default_metric_from_mesh(mesh: &ImbaFontIconMesh) -> SvgGlyphMetric {
    let height = mesh.height.max(1e-6);
    let width = (mesh.width / height).max(0.0);
    let advance_w = if width > 0.0 { width + 0.08 } else { 0.5 };
    SvgGlyphMetric {
        baseline_y: 1.0,
        ink_top: 0.0,
        ink_bottom: 1.0,
