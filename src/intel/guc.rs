// GuC firmware bootstrap and host transport.
//
// Firmware READY and an enabled CTB are prerequisites for engine admission.
...
const GUC_ENGINE_REGSET_COUNT: usize =
    GUC_RCS0_REGSET_COUNT + GUC_VCS0_REGSET_COUNT + GUC_VCS2_REGSET_COUNT + GUC_BCS0_REGSET_COUNT;
...
const GUC_VCS_GOLDEN_CONTEXT_BYTES: usize = 2 * 4096;
const GUC_VCS_LRC_SKIP_BYTES: usize = 4096 + 80 * core::mem::size_of::<u32>();
...
struct GucAds {
    ...
    private_data: u32,
    _um: u32,
    // capture_instance[2][16], capture_class[2][16], capture_global[2],
    // workaround KLV address/size, then the ABI's reserved tail.
    _rest: [u32; 144],
}
...
struct GucEngineUsage {
    _engines: [[GucEngineUsageRecord; GUC_MAX_INSTANCES_PER_CLASS]; GUC_MAX_ENGINE_CLASSES],
}
...
const _: () = assert!(core::mem::size_of::<GucPolicies>() == 96);
const _: () = assert!(core::mem::size_of::<GucGtSystemInfo>() == 640);
...
pub(crate) fn load_fw() -> crate::intel::Buf {
    ...
    let Some(css) = crate::intel::uc_fw::parse_css(blob) else {
        crate::log!("intel/guc: firmware rejected reason=invalid-css\n");
        return crate::intel::empty();
    };
    let (major, minor, patch) = crate::intel::uc_fw::version_triplet(css.sw_version);
    if major != 70 {
        crate::log!(
            "intel/guc: firmware rejected reason=unsupported-major version={}.{}.{} required_major=70\n",
            major,
            minor,
            patch
        );
    ...
    crate::log!(
        "intel/guc: firmware css module_type={} header_version={} vendor=0x{:X} version={}.{}.{} private_data=0x{:X}\n",
        css.module_type,
        css.header_version,
        css.vendor,
        major,
...
fn build_ads(dev: crate::intel::Dev, ads: crate::intel::Buf) {
    ...
    // TRUEOS does not install golden reset contexts yet. Keep GuC as the
    // scheduler but disable autonomous engine reset until device-loss/reset
    // propagation is implemented at the physical-device boundary.
    crate::intel::wr32(
        buf,
        p + core::mem::offset_of!(GucPolicies, _flags),
    ...
    let vcs_regset_off =
        bcs_regset_off + GUC_BCS0_REGSET_COUNT * core::mem::size_of::<GucMmioReg>();
    let vcs_reg_state = a
        + core::mem::offset_of!(GucAds, reg_state_list)
        + (GUC_VIDEO_CLASS * GUC_MAX_INSTANCES_PER_CLASS + GUC_VCS0_INSTANCE)
            * core::mem::size_of::<GucMmioRegSet>();
    ...
    crate::intel::wr32(
        ...
        (ads.gpu + golden_off as u64) as u32,
    );
    crate::intel::wr32(
        buf,
    ...
    let vcs_golden_off = guc_ads_vcs_golden_context_offset().unwrap_or(bcs_golden_off);
    crate::intel::wr32(
        buf,
        a + core::mem::offset_of!(GucAds, _golden) + GUC_VIDEO_CLASS * 4,
        (ads.gpu + vcs_golden_off as u64) as u32,
    );
    crate::intel::wr32(
        buf,
        a + core::mem::offset_of!(GucAds, _eng) + GUC_VIDEO_CLASS * 4,
        GUC_VCS_GOLDEN_CONTEXT_BYTES.saturating_sub(GUC_VCS_LRC_SKIP_BYTES) as u32,
    );
    crate::intel::wr32(
        buf,
...
fn guc_ads_golden_context_offset() -> Option<usize> {
...
fn guc_ads_vcs_golden_context_offset() -> Option<usize> {
    crate::intel::align_up(
...
fn guc_ads_private_data_offset() -> Option<usize> {
    crate::intel::align_up(
        guc_ads_vcs_golden_context_offset()?.checked_add(GUC_VCS_GOLDEN_CONTEXT_BYTES)?,
        4096,
    )
}
...
fn build_engine_regset(buf: &mut [u8], base: usize, mmio_base: u32) {
    ...
    let mut push = |offset: u32, flags: u32| {
        ...
        crate::intel::wr32(buf, entry + core::mem::offset_of!(GucMmioReg, offset), offset);
        crate::intel::wr32(buf, entry + core::mem::offset_of!(GucMmioReg, value), 0);
        crate::intel::wr32(buf, entry + core::mem::offset_of!(GucMmioReg, flags), flags);
        crate::intel::wr32(buf, entry + core::mem::offset_of!(GucMmioReg, mask), 0);
        index += 1;
    };
    ...
    for offset in [0xE458, 0xE558, 0xE658, 0xE758, 0xE45C, 0xE55C, 0xE65C] {
        push(offset, GUC_REGSET_NEEDS_STEERING);
    }
    debug_assert_eq!(index, GUC_RCS0_REGSET_COUNT);
}

fn init_params(dev: crate::intel::Dev, ads: crate::intel::Buf) {
    let mut vals = [0u32; 6];
    vals[GUC_CTL_WA] = 0;
    // Zero deliberately leaves GuC scheduling enabled. The historical HuC
    // authentication probe set DISABLE_SCHEDULER here; that is not a valid
    // configuration for a GuC-owned submission backend.
    vals[GUC_CTL_FEATURE] = 0;
    ...
    crate::log!(
        "intel/guc: init-params feature=0x{:08X} debug=0x{:08X} ads=0x{:08X} devid=0x{:08X} scheduler=enabled submission_owner=guc\n",
