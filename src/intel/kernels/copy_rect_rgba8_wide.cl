...
// - One SIMD16 work item copies up to four adjacent pixels across up to sixty-four rows.
// - No scaling, filtering, color conversion, or blending.

__attribute__((intel_reqd_sub_group_size(16)))
__kernel void copy_rect_rgba8_wide(
    ...
    uint height)
{
    uint base_x = get_global_id(0) << 2;
    uint base_y = get_global_id(1) << 6;

    if (base_x >= width || base_y >= height) {
        return;
    }
