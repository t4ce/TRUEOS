...
typedef struct FillRectWorklistDesc {
    uint dst_xy;
    uint size;
    uint color_rgba;
} FillRectWorklistDesc;
...
__kernel void fill_rect_worklist_rgba8(
    ...
    __global const FillRectWorklistDesc *descs,
    ...
    uint desc_count)
{
    ...
    for (uint local_desc_id = lane; local_desc_id < desc_count; local_desc_id += 16u) {
        FillRectWorklistDesc desc = descs[desc_base + local_desc_id];
        int dst_x = unpack_i16(desc.dst_xy);
        int dst_y = unpack_i16(desc.dst_xy >> 16);
        uint width = desc.size & 0xFFFFu;
        uint height = desc.size >> 16;
        ...
        for (uint y = 0; y < height; y++) {
            ...
            for (uint x = 0; x < width; x++) {
                ...
                dst_rgba[(uint)out_y * dst_pitch_pixels + (uint)out_x] = desc.color_rgba;
            }
        }
    }
}
