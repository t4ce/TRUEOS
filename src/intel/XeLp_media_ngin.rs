...
use super::intel_guc;
use super::intel_igpu770::{forcewake_all_acquire, mmio_read32, warm_state, Igpu770WarmState};
...
impl MediaEngineDescriptor {
	...
	const fn supports_workload(self, workload: MediaWorkloadKind) -> bool {
		match workload {
			...
			MediaWorkloadKind::SessionSnapshot | MediaWorkloadKind::EngineReset | MediaWorkloadKind::Smoke => {
				matches!(
					self.provisioning,
...
fn observability_plan(desc: MediaEngineDescriptor, resources: MediaResourcePlan) -> MediaObservabilityPlan {
...
fn build_engine_plan(slot: usize, desc: MediaEngineDescriptor, guc_ready: bool) -> MediaEnginePlan {
	...
	MediaEnginePlan {
		...
		submission: MediaSubmissionPlan {
			...
			queue_depth: if desc.id.class == MediaEngineClass::VideoDecode { 2 } else { 1 },
