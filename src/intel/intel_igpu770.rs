...
pub(crate) fn rcs_render_minimal_rgb_frame(
    frame: &super::xelp_render_ngin::MinimalScreenRgbFrame,
    width: usize,
    height: usize,
) -> bool {
    let Some(warm) = warm_state() else {
        crate::log!("intel/igpu770: rcs-render skipped reason=not-warmed\n");
        return false;
    };
    if !intel_guc::ready() {
        crate::log!(
            "intel/igpu770: rcs-render skipped reason=guc-not-ready guc_status=0x{:08X}\n",
            intel_guc::status(warm)
        );
        return false;
    }
    if width == 0 || height == 0 || frame.draws.is_empty() {
        crate::log!(
            "intel/igpu770: rcs-render skipped reason=invalid-frame width={} height={} clear={} draws={}\n",
            width,
            height,
            frame.clear_rgb.is_some() as u8,
            frame.draws.len()
        );
        return false;
    }
    ...
    let Some(ring) = ggtt_map_plan_system_ram(warm.ring_phys, warm.ring_len, GPU_VA_RING_BASE)
        ...
        crate::log!("intel/igpu770: rcs-render skipped reason=ring-plan\n");
        return false;
    };
    let Some(context) =
        ggtt_map_plan_system_ram(warm.context_phys, warm.context_len, GPU_VA_CONTEXT_BASE)
    else {
        crate::log!("intel/igpu770: rcs-render skipped reason=context-plan\n");
        return false;
    };
    let Some(batch) = ggtt_map_plan_system_ram(warm.batch_phys, warm.batch_len, GPU_VA_BATCH_BASE)
    else {
        crate::log!("intel/igpu770: rcs-render skipped reason=batch-plan\n");
        return false;
    };
    ...
    let Some(dst) = ggtt_map_plan_aperture_backed(
        ...
        crate::log!("intel/igpu770: rcs-render skipped reason=fb-plan\n");
        return false;
    };
    let frame_pixels = copy_w.saturating_mul(copy_h);
    if frame_pixels == 0 {
        crate::log!("intel/igpu770: rcs-render skipped reason=empty-clamped-frame\n");
        return false;
    }

    let Some(ring_ctl) = ring_ctl_value(warm.ring_len) else {
        crate::log!(
            "intel/igpu770: rcs-render skipped reason=ring-ctl ring_len=0x{:X}\n",
            warm.ring_len
        );
        return false;
    };
    let ring_start = ring.gpu_addr as u32;
    if !init_gen12_lrc_context_image(warm, ring_start, BLT_RING_TAIL_BYTES, ring_ctl) {
        crate::log!("intel/igpu770: rcs-render skipped reason=lrc-context-init\n");
        return false;
    }
    let (context_desc_lo, context_desc_hi) = build_execlist_context_descriptor(context.gpu_addr);
    let dst_gpu_addr = dst.gpu_addr;
    ...
    while completed_pixels < frame_pixels {
        ...
        let done_value = RCS_PRESENT_RESULT_BASE
            ...
            .wrapping_add(1);
        let (batch_tail_bytes, chunk_pixels) =
            match super::xelp_render_ngin::encode_minimal_rgb_frame_batch_chunk(
                batch_dwords,
                frame,
                copy_w,
                copy_h,
                ...
                Err(err) => {
                    crate::log!(
                        "intel/igpu770: rcs-render chunk={} build-failed err={} start_pixel={} total_pixels={} draws={}\n",
                        chunk_idx,
                        err,
                        completed_pixels,
                        frame_pixels,
        ...
        if !init_gen12_lrc_context_image(warm, ring_start, ring_tail_bytes as u32, ring_ctl) {
            crate::log!(
                "intel/igpu770: rcs-render chunk={} skipped reason=lrc-context-reinit\n",
                chunk_idx
            );
            chunk_fail = true;
            break;
        ...
        if !completed {
            ...
            crate::log!(
                "intel/igpu770: rcs-render chunk={} timeout iters={} result0=0x{:08X} start=0x{:08X} expect=0x{:08X} phase={} batch_tail=0x{:X} head=0x{:08X} tail=0x{:08X} acthd=0x{:08X} ipeir=0x{:08X} ipehr=0x{:08X}\n",
                chunk_idx,
                iter,
                result0,
                start_value,
        ...
        if chunk_idx < 4 || completed_pixels >= frame_pixels {
            crate::log!(
                "intel/igpu770: rcs-render chunk={} pixels={} completed_pixels={} total_pixels={} batch_tail=0x{:X}\n",
                chunk_idx,
                chunk_pixels,
                completed_pixels,
                frame_pixels,
    ...
    let success = !chunk_fail && completed_pixels >= frame_pixels;
    crate::log!(
        "intel/igpu770: rcs-render summary success={} size={}x{} chunks={} completed_pixels={} total_pixels={} draws={} dst_gpu=0x{:X} pitch=0x{:X}\n",
        success as u8,
        copy_w,
        copy_h,
        chunk_idx,
