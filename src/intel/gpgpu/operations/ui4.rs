...
pub(crate) fn queue_ui4_video_frame_nv12_tile64_to_rgba8(
    ...
    if runtime
        .pending
        .iter()
        .any(|pending| pending.kernel == "rgba8-linear-nv12-encode")
    {
        return Err(Ui4CompositorSubmitError::Busy);
    }
...
/// input consumed by VDEnc. This request temporarily owns both compositor
/// source-alias windows, so decoded-video submissions wait until its marker
/// retires instead of retargeting the destination PTEs underneath it.
pub(crate) fn queue_ui4_stream_rgba8_to_nv12_linear(
    ...
    if !source.is_valid()
        ...
        || source.gpu != UI4_COMPOSITOR_NV12_SOURCE_GPU_BASE
        ...
        || gpu_ranges_overlap(source.phys, source.bytes, destination.phys, destination.bytes)
    {
        return Err(Ui4CompositorSubmitError::InvalidWorklist);
    }
    ...
    let job_slot = 0usize;
    let params = Ui4Rgba8ToNv12LinearParams {
        src_gpu: source.gpu,
    ...
    let dev = super::claimed_device().ok_or(Ui4CompositorSubmitError::Unavailable)?;
    let upload =
        upload_ui4_rgba8_to_nv12_linear_kernel().ok_or(Ui4CompositorSubmitError::Unavailable)?;
    let shared_state =
        ui4_compositor_rcs_state_once(dev).ok_or(Ui4CompositorSubmitError::Unavailable)?;
    let state =
        direct_rcs_job_slot(shared_state, job_slot).ok_or(Ui4CompositorSubmitError::Unavailable)?;
    ...
    probe.source_map_us = direct_rcs_elapsed_us_since(phase_started_tick);
    let phase_started_tick = direct_rcs_now_tick();
    let destination_ok = source_ok
        && direct_rcs_map_ppgtt_kernel(state, destination.gpu, destination.phys, destination.bytes);
    probe.destination_map_us = direct_rcs_elapsed_us_since(phase_started_tick);
    let phase_started_tick = direct_rcs_now_tick();
    let batch_ok = destination_ok
        && direct_rcs_encode_ui4_rgba8_to_nv12_linear_batch(
            state,
            upload,
            params,
            source.bytes,
    ...
    if !batch_ok {
        crate::log_error!(target: "ui4";
            "ui4/guc-stream-convert: queue rejected forcewake={} state={} ppgtt={} kernel={} source={} destination={} batch={} source_gpu=0x{:X} destination_gpu=0x{:X}\n",
            forcewake_ok as u8,
            mapped_ok as u8,
            ppgtt_ok as u8,
            kernel_ok as u8,
    ...
    runtime.pending.push_back(Ui4CompositorPending {
        ...
        marker_slot: SPRITE_QUAD_WORKLIST_POST_MARKER_SLOT,
        marker_value: SPRITE_QUAD_WORKLIST_POST_MARKER,
        kernel: "rgba8-linear-nv12-encode",
        stats: GpgpuWorklistSubmitStats {
            descs: 1,
            walkers: 1,
            submits: 1,
    ...
    crate::log_trace!(target: "ui4";
        "ui4/guc-stream-convert: queued serial={} job_slot={} source={}x{} destination={}x{} active={}x{}@0,{} source_gpu=0x{:X} destination_gpu=0x{:X} engine=guc-rcs cpu_pixel_math=0\n",
        serial,
        job_slot,
        source.width,
        source.height,
...
pub(crate) fn poll_ui4_compositor_submission(
    ...
    if observed == pending.marker_value {
        ...
        if pending.kernel == "nv12-media-ytile-rgba8-frame"
            || pending.kernel == "rgba8-linear-nv12-encode"
        {
            let batch_enter =
