...
// Sprite Dig owns one immutable, Bakery-generated RGBA atlas in the shared
// system-service PPGTT. It is installed once and remains pinned for the
...
const _: () = assert!(HELIO_SPRITE_ATLAS_GPU_LIMIT <= DIRECT_RCS_PPGTT_LIMIT_BYTES);
...
// Like glyph-mask payloads above, the 224 useful bytes are legal but each
// GPGPU_WALKER indirect-data start is encoded at 64-byte granularity. Keep the
// useful length in the command while placing consecutive payloads in aligned
// 256-byte slots.
const SPRITE_QUAD_WORKLIST_PAYLOAD_STRIDE_BYTES: usize =
    (SPRITE_QUAD_WORKLIST_INDIRECT_BYTES + GPGPU_WALKER_INDIRECT_ALIGNMENT_BYTES - 1)
        & !(GPGPU_WALKER_INDIRECT_ALIGNMENT_BYTES - 1);
...
// PIPE_CONTROL completion writes a QWord at POST and POST + 1.  Keep the
// diagnostic pre-dispatch cookie outside that pair; slot 25 used to be
// silently cleared by the high DWORD of the completion write.
const SPRITE_QUAD_WORKLIST_PRE_MARKER_SLOT: usize = 32;
...
const _: () = assert!(SPRITE_QUAD_WORKLIST_POST_MARKER_SLOT.is_multiple_of(2));
const _: () = assert!(
    SPRITE_QUAD_WORKLIST_PRE_MARKER_SLOT != SPRITE_QUAD_WORKLIST_POST_MARKER_SLOT
        && SPRITE_QUAD_WORKLIST_PRE_MARKER_SLOT != SPRITE_QUAD_WORKLIST_POST_MARKER_SLOT + 1
);
...
const SPRITE_QUAD_WORKLIST_DESCRIPTOR_ARRAY_BYTES: usize =
    SPRITE_QUAD_WORKLIST_MAX_DESCS * core::mem::size_of::<GpgpuSpriteQuadWorklistDesc>();
const SPRITE_QUAD_TILEMAP_STATE_OFFSET_BYTES: usize = SPRITE_QUAD_WORKLIST_DESCRIPTOR_ARRAY_BYTES;
const SPRITE_QUAD_WORKLIST_DESC_BYTES: usize = SPRITE_QUAD_TILEMAP_STATE_OFFSET_BYTES
    + SPRITE_QUAD_TILEMAP_STATE_MAX_DWORDS * core::mem::size_of::<u32>();
...
const _: () = assert!(
    SPRITE_QUAD_WORKLIST_STATE_BASE_OFFSET_BYTES
        ...
        + SPRITE_QUAD_WORKLIST_MAX_DESCS * SPRITE_QUAD_WORKLIST_PAYLOAD_STRIDE_BYTES
        <= DIRECT_RCS_BATCH_BYTES
);
const _: () = assert!(
...
const _: () = assert!(
    SPRITE_QUAD_WORKLIST_PAYLOAD_STRIDE_BYTES.is_multiple_of(GPGPU_WALKER_INDIRECT_ALIGNMENT_BYTES)
);
const _: () =
    assert!(SPRITE_QUAD_WORKLIST_PAYLOAD_STRIDE_BYTES >= SPRITE_QUAD_WORKLIST_INDIRECT_BYTES);
const _: () = assert!(
...
const _: () = assert!(
    FONT_OUTLINE_COVERAGE_BATCH_STATE_BASE_OFFSET_BYTES
        + FONT_OUTLINE_COVERAGE_BATCH_MAX_RUNS * FONT_OUTLINE_COVERAGE_BATCH_STATE_BLOCK_BYTES
...
const _: () = assert!(
    FONT_OUTLINE_COVERAGE_BATCH_PAYLOAD_BASE_OFFSET_BYTES
        + FONT_OUTLINE_COVERAGE_BATCH_MAX_RUNS * FONT_OUTLINE_COVERAGE_BATCH_PAYLOAD_STRIDE_BYTES
        <= DIRECT_RCS_BATCH_BYTES
);
const _: () = assert!(
    FONT_OUTLINE_COVERAGE_BATCH_PAYLOAD_BASE_OFFSET_BYTES
...
// One in-flight HelioC frame owns this QWord until both the ordered post-sync
// write and the saved LRC head prove retirement. The high DWORD is reserved by
// PIPE_CONTROL, so no other HelioC telemetry may use slot 1.
const HELIOC_RCS_COMPLETION_SLOT: usize = 0;
const HELIOC_RCS_COMPLETION_MARKER: u32 = 0xC0DE_C002;
const HELIOC_RCS_TERMINAL_EPILOGUE_DWORDS: usize = 13;
// The captured Cloud stages must not live in the 256 KiB command batch: the
// SIMD16 compute ISA alone is 64,544 bytes and the VS/FS pair adds another
// 13,872 bytes.  Keep immutable code and each address-bearing state class in
// distinct HelioC-only PPGTT windows so package relocation never borrows an
// ANV process heap or overlaps commands which may still be executing.
const HELIOC_RCS_GPU_VA_SHADER_BASE: u64 = 0x0874_0000;
const HELIOC_RCS_SHADER_BYTES: usize = 0x0004_0000;
const HELIOC_RCS_GPU_VA_SURFACE_STATE_BASE: u64 = 0x0878_0000;
const HELIOC_RCS_SURFACE_STATE_BYTES: usize = 0x0001_0000;
const HELIOC_RCS_GPU_VA_DYNAMIC_STATE_BASE: u64 = 0x0879_0000;
const HELIOC_RCS_DYNAMIC_STATE_BYTES: usize = 0x0001_0000;
const HELIOC_RCS_GPU_VA_INDIRECT_DESCRIPTOR_BASE: u64 = 0x087A_0000;
const HELIOC_RCS_INDIRECT_DESCRIPTOR_BYTES: usize = 0x0001_0000;
const HELIOC_RCS_GPU_VA_IMMUTABLE_STATE_LIMIT: u64 = 0x087B_0000;
const HELIOC_RCS_STATE_ARENA_BYTES: usize =
    (HELIOC_RCS_GPU_VA_IMMUTABLE_STATE_LIMIT - HELIOC_RCS_GPU_VA_SHADER_BASE) as usize;
...
const _: () = assert!(HELIOC_RCS_COMPLETION_SLOT.is_multiple_of(2));
const _: () = assert!(
    (HELIOC_RCS_COMPLETION_SLOT + 2) * core::mem::size_of::<u32>() <= DIRECT_RCS_RESULT_BYTES
);
const _: () = assert!(
...
const _: () = assert!(
    HELIOC_RCS_GPU_VA_SHADER_BASE + HELIOC_RCS_SHADER_BYTES as u64
        <= HELIOC_RCS_GPU_VA_SURFACE_STATE_BASE
);
const _: () = assert!(
    HELIOC_RCS_GPU_VA_SURFACE_STATE_BASE + HELIOC_RCS_SURFACE_STATE_BYTES as u64
        <= HELIOC_RCS_GPU_VA_DYNAMIC_STATE_BASE
);
const _: () = assert!(
    HELIOC_RCS_GPU_VA_DYNAMIC_STATE_BASE + HELIOC_RCS_DYNAMIC_STATE_BYTES as u64
        <= HELIOC_RCS_GPU_VA_INDIRECT_DESCRIPTOR_BASE
);
const _: () = assert!(
    HELIOC_RCS_GPU_VA_INDIRECT_DESCRIPTOR_BASE + HELIOC_RCS_INDIRECT_DESCRIPTOR_BYTES as u64
        <= HELIOC_RCS_GPU_VA_IMMUTABLE_STATE_LIMIT
);
const _: () = assert!(
    HELIOC_RCS_STATE_ARENA_BYTES
...
const _: () = assert!(
    HELIOC_RCS_GPU_VA_IMMUTABLE_STATE_LIMIT <= HELIOC_RCS_GPU_VA_VOLUME_A_BASE
);
const _: () = assert!(
...
mod helioc_layout_tests {
    ...
    fn helioc_control_and_resource_windows_are_ordered_and_bounded() {
        let windows = [
            ...
            (HELIOC_RCS_GPU_VA_BATCH_BASE, HELIOC_RCS_GPU_VA_SHADER_BASE),
            (HELIOC_RCS_GPU_VA_SHADER_BASE, HELIOC_RCS_GPU_VA_SURFACE_STATE_BASE),
            (
                HELIOC_RCS_GPU_VA_SURFACE_STATE_BASE,
                HELIOC_RCS_GPU_VA_DYNAMIC_STATE_BASE,
            ),
