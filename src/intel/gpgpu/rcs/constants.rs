...
const _: () = {
    ...
    assert!(
        KOKORO_QGEMM_U8_I8_ADLS_GPU
            + ((KOKORO_QGEMM_U8_I8_ADLS_BIN.len() as u64 + 4095) & !4095)
            <= KOKORO_CONV1D_U8_U8_ADLS_GPU
    );
    assert!(
        KOKORO_CONV1D_U8_U8_ADLS_GPU
            + ((KOKORO_CONV1D_U8_U8_ADLS_BIN.len() as u64 + 4095) & !4095)
...
const _: () = {
    ...
    assert!(KOKORO_CONV1D_MAX_CHANNELS * KOKORO_CONV1D_MAX_CHANNELS
        * KOKORO_CONV1D_MAX_KERNEL_SIZE == KOKORO_CONV1D_PACKED_WEIGHTS_ALLOC_BYTES);
    assert!(KOKORO_CONV1D_MAX_CHANNELS * KOKORO_CONV1D_MAX_KERNEL_SIZE
        * core::mem::size_of::<u32>() <= KOKORO_CONV1D_WEIGHT_TAP_SUMS_ALLOC_BYTES);
    assert!((KOKORO_CONV1D_MAX_TILE_TIMES + 2 * KOKORO_CONV1D_MAX_RADIUS)
        * KOKORO_CONV1D_MAX_CHANNELS <= KOKORO_CONV1D_ACTIVATIONS_ALLOC_BYTES);
    assert!(KOKORO_CONV1D_MAX_TILE_TIMES * KOKORO_CONV1D_MAX_CHANNELS
        * core::mem::size_of::<i32>() == KOKORO_CONV1D_OUTPUT_ALLOC_BYTES);
    assert!(KOKORO_CONV1D_ARENA_GPU.is_multiple_of(4096));
    assert!(KOKORO_CONV1D_ARENA_BYTES.is_multiple_of(4096));
    assert!(KOKORO_CONV1D_ARENA_GPU + KOKORO_CONV1D_ARENA_BYTES as u64
        <= KOKORO_CONV1D_ARENA_GPU_LIMIT);
    assert!(KOKORO_CONV1D_ARENA_GPU_LIMIT <= DIRECT_RCS_PPGTT_LIMIT_BYTES);
    assert!(KOKORO_CONV1D_IDD_OFFSET_BYTES + KOKORO_CONV1D_IDD_BYTES
        <= KOKORO_CONV1D_BINDING_TABLE_OFFSET_BYTES);
    assert!(KOKORO_CONV1D_OUTPUT_SURFACE_OFFSET_BYTES
        + COPY_RECT_SURFACE_STATE_DWORDS * core::mem::size_of::<u32>()
        <= KOKORO_CONV1D_PAYLOAD_OFFSET_BYTES);
    assert!(KOKORO_CONV1D_PAYLOAD_OFFSET_BYTES + KOKORO_CONV1D_INDIRECT_BYTES
        <= DIRECT_RCS_BATCH_BYTES);
    assert!(KOKORO_CONV1D_PAYLOAD_OFFSET_BYTES
    ...
    assert!((KOKORO_CONV1D_PRE_MARKER_SLOT + 1) * core::mem::size_of::<u32>()
