...
-> [trueos_helio_runtime::churn::MeshDescriptor; CHURN_FORWARD_MESH_COUNT] {
    core::array::from_fn(|mesh_id| trueos_helio_runtime::churn::MeshDescriptor {
        ...
        half_extents: [1.0; 3],
        first_vertex: (mesh_id * CHURN_FORWARD_VERTICES_PER_MESH) as u32,
        vertex_count: CHURN_FORWARD_VERTICES_PER_MESH as u32,
        first_index: (mesh_id * CHURN_FORWARD_INDICES_PER_MESH) as u32,
        index_count: CHURN_FORWARD_INDICES_PER_MESH as u32,
        base_vertex: (mesh_id * CHURN_FORWARD_VERTICES_PER_MESH) as i32,
    })
}
...
/// Create the geometry half of Picasso's retained-transform path.
///
...
pub(crate) fn create_resident_picasso_retained_mesh(
    ...
    let mut index_blob = Vec::with_capacity(index_bytes);
    for index in indices {
        index_blob.extend_from_slice(&index.to_le_bytes());
    }
    if !geometry.write(0, vertices) || !geometry.write(index_offset, &index_blob) {
        let _ = release_resident_render_buffer(&geometry);
        return Err("picasso-retained-geometry-upload");
    }
    geometry.flush();
    ...
    // V1/V2 start with one full-mesh indexed-indirect draw. V3 publishes its
    // submesh groups with the seeds; upper flag bits carry group-local slots.
...
fn picasso_retained_draw_templates(
    ...
    if seeds.is_empty()
        || seeds.len() > v::vgpu::MAX_RETAINED_SCENE_INSTANCES
        || ranges.is_empty()
        || ranges.len() > v::vgpu::MAX_RETAINED_SCENE_DRAWS
    ...
    for (range, capacity) in ranges.iter().zip(capacities) {
        if capacity == 0 {
            return Err("picasso-retained-empty-group");
        }
...
pub(crate) fn update_resident_picasso_retained_transform_seeds(
    ...
    let default_draw = [v::vgpu::RetainedDrawRange {
        first_index: 0,
    ...
    if seeds.len() > resident.max_instances
        || ranges.iter().any(|range| {
            !resident
                .topology
                .accepts_index_count(range.index_count as usize)
        })
    {
    ...
    // 36-index cube in every draw group. Picasso supplies admitted submesh
    // ranges and leaves unused groups empty, so only
    ...
    resident
        .draw_group_count
        .store(ranges.len() as u32, Ordering::Release);
