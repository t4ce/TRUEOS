...
use super::display::{
    ...
    PIPE_SCALER_WIN_SIZE_FROM_CTRL, PIPECONF_A, PIPECONF_STATE, PLANE_COLOR_PLANE_GAMMA_DISABLE,
    PLANE_CTL_ARB_SLOTS_4BPP, PLANE_CTL_ENABLE, PLANE_CTL_FORMAT_XRGB_8888, TRANS_DDI_FUNC_CTL_A,
    TRANS_HTOTAL_A, TRANS_VTOTAL_A, UNI_PLANE_AUX_DIST_OFF, UNI_PLANE_AUX_OFFSET_OFF,
    UNI_PLANE_BASE, UNI_PLANE_COLOR_CTL_OFF, UNI_PLANE_CUS_CTL_OFF, UNI_PLANE_KEYMSK_OFF,
    UNI_PLANE_OFFSET_OFF, UNI_PLANE_POS_OFF, UNI_PLANE_SIZE_OFF, UNI_PLANE_STRIDE_OFF,
    UNI_PLANE_SURF_OFF, UNI_PLANE_SURFLIVE_OFF,
};
...
const PRIMARY_CTL: u32 = PLANE_CTL_ENABLE | PLANE_CTL_ARB_SLOTS_4BPP | PLANE_CTL_FORMAT_XRGB_8888;
...
pub(crate) fn ui4_handoff_active() -> bool {
    SCANOUT_LATCHED.load(Ordering::Acquire) && super::claimed_device().is_some_and(is_target)
}
...
fn prepare_frame(dev: Dev) -> Option<Frame> {
    ...
    for y in 0..HEIGHT as usize {
        for x in 0..WIDTH as usize {
            ...
            let old_extent =
                (x < 2560 && (1436..1440).contains(&y)) || (y < 1440 && (2556..2560).contains(&x));
...
pub(crate) fn init_once(dev: Dev) -> bool {
    ...
    if ctl_before & PLANE_CTL_ENABLE != 0 {
        ...
        if !wait_frame(dev) || !wait_frame(dev) {
            ...
            mmio_write(dev, UNI_PLANE_BASE + UNI_PLANE_SURF_OFF, surf_before);
            crate::log!(
                "intel/tgl-native-panel: stopped reason=primary-disable-frame-timeout old-plane-rearmed=1 backing=retained\n"
            );
            return false;
        }
    }
    ...
    crate::log!(
        ...
        live,
        readback_ok as u8,
        scalers_detached as u8,
        SPIRIT_RESEALED as u8,
    );
    if readback_ok {
