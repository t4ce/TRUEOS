    ...
    let result = loop {
        ...
        if first_frame {
            ...
            if gpu_transform_enabled {
                crate::log_info!(
                    target: "helio";
                    "helio instance={} example=2 name=churn-benchmark native=1 online artifact_bytes={} active_objects={} max_instances={} draw_groups=12 geometry=3x-posnormal-cube frame={} window={} extent={}x{} plane={} background=transparent-rgba controls=WASD+Space+Shift,left-drag-look,+/-,C-collision-burst producer_pacing=absolute-deadline period_us=16667 target_fps=60 missed_deadline=skip-extra-delay path=helioa-churn-forward-v1->build-folded-affine-template->dirty-local+dirty-world+dirty-row->gpu-world-matrices+compaction->artifact-native-vs+ps->12-indexed-indirect->one-guc-schedule->ui4-triple-direct buildtime_constant_ops=2 folded_affines=1 cpu_vertex_projection=0 cpu_matrix_expansion=0 mutable_upload=camera+dirty-64b-transform-seeds+dirty-worklists gpu_outputs=208b-instances+compacted-indices+20b-indexed-indirect immutable_geometry=retained cpu_readback=0 cpu_frame_copy=0\n",
                    context.instance_id,
...
async fn run_pendulum_native(context: InstanceContext) -> Result<(), GameError> {
    ...
    let result = loop {
        ...
        if first_frame {
            ...
            crate::log_info!(
                target: "helio";
                "helio instance={} example=4 name=pendulum-bigcloth native=1 online artifact_bytes={} segments={} instances={} draw_groups=12 geometry=one-retained-posnormal-box frame={} window={} extent={}x{} plane={} controls=WASD+Space+Shift,left-drag-look,C-pause,R-reset producer_pacing=absolute-deadline period_us={} path=helioa-pendulum-v1->build-folded-affine-template->dirty-local+dirty-world+dirty-row->gpu-world-matrices+compaction->artifact-native-vs+ps->gpu-indexed-indirect->one-guc-schedule->ui4-triple-direct buildtime_constant_ops=2 folded_affines=1 cpu_vertex_projection=0 cpu_matrix_expansion=0 cpu_winding_repair=0 mutable_upload=camera+dirty-64b-transform-seeds+dirty-worklists gpu_outputs=208b-instances+compacted-indices+20b-indexed-indirect immutable_geometry=retained cpu_readback=0 cpu_frame_copy=0\n",
                context.instance_id,
                CHURN_FORWARD_ARTIFACT.len(),
