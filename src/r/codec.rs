...
pub(crate) static CODEC_COMPUTE: crate::cpu_task_pool::CpuTaskPool =
    crate::cpu_task_pool::CpuTaskPool::new(
...
/// Existing 7z callers retain the same content identity. The LZ4 path selects
...
impl ArchiveFormat {
    ...
    fn content_type(self) -> crate::r::fs::trueosfs::ContentTypeId {
        match self {
            Self::SevenZ => ARCHIVE_CONTENT_TYPE,
            Self::TarLz4 => crate::r::fs::trueosfs::ContentTypeId::LZ4,
        }
    }
}
fn codec_now_ms() -> u64 {
...
impl core::fmt::Display for CodecError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            Self::Lz4(err) => write!(f, "lz4: {:?}", err),
            Self::Tar(err) => write!(f, "tar: {:?}", err),
...
pub fn enqueue_archive_pack(
    ...
    enqueue_operation(owner, |id| CodecRequest::PackPath {
        owner,
        id,
        source_path,
        archive_path,
...
/// differs from `enqueue_archive_pack`, which accepts one file or recursively packs
/// one directory.
pub fn enqueue_archive_pack_many(
    owner: u32,
    source_paths: Vec<String>,
    archive_path: String,
) -> Result<u32, CodecError> {
    ...
    enqueue_operation(owner, |id| CodecRequest::PackPaths {
        owner,
        id,
...
pub fn enqueue_archive_unpack(
    ...
    enqueue_operation(owner, |id| CodecRequest::UnpackPath {
        owner,
        id,
        archive_path,
        output_path,
...
async fn compress_entries(
    ...
    let prepared = CODEC_COMPUTE
        .run("archive/pack", move |_| {
            ...
            match format {
                ArchiveFormat::SevenZ => {
                    crate::z7::compress_files_to_vec(&sources).map_err(CodecError::Archive)
                }
                ArchiveFormat::TarLz4 => {
                    super::tar::pack(&sources, MAX_TAR_BYTES).map_err(CodecError::Tar)
                }
            }
        })
        .await
        .map_err(|_| CodecError::NotReady)??;
    let out = match format {
        ArchiveFormat::SevenZ => prepared,
        ArchiveFormat::TarLz4 => super::lz4::compress_frame(prepared)
            .await
            .map_err(CodecError::Lz4)?,
    };
    crate::log_important!(target: "storage"; "codec: phase=encode elapsed_ms={} output_bytes={}\n", codec_now_ms().saturating_sub(started), out.len());
    Ok(out)
}
...
async fn pack_path_job(source_path: &str, archive_path: &str) -> Result<CodecReport, CodecError> {
    ...
    let format = ArchiveFormat::for_output(archive_path);
    let archive = compress_entries(entries, format).await?;
...
async fn unpack_path_job(archive_path: &str, output_path: &str) -> Result<CodecReport, CodecError> {
    ...
    let entries = if archive.starts_with(&super::lz4::MAGIC) {
        ...
        CODEC_COMPUTE
            ...
            .await
            .map_err(|_| CodecError::NotReady)?
            .map_err(CodecError::Tar)?
    } else {
        CODEC_COMPUTE
            .run("archive/7z-unpack", move |_| {
    ...
    if legacy_blob_members != 0 {
        crate::log_important!(target: "storage";
            "codec/archive: unpack content_identity=absent fallback=legacy-blob members={} decision=accepted\n",
            legacy_blob_members,
        );
    }
