...
pub(crate) fn enabled() -> bool { SURFACE.get().is_some() }
...
impl Pen {
    ...
    fn flush(&mut self) {
        ...
        let _ = crate::intel::dma_flush_strided_rows(
            ...
            self.surface.pitch_bytes as usize, microfont::FHEIGHT);
...
pub(crate) async fn microfont_log_task(assigned_slot: u32) {
    ...
    loop {
        ...
        if let Some((count, lost)) = crate::log_os::logtotcp::copy_for_screen(&mut cursor, &mut batch) {
            if lost != 0 {
                let _ = writeln!(pen, "[screen] {} bytes overwritten before this reader caught up", lost);
            }
            for &byte in &batch[..count] { pen.byte(byte); }
            pen.flush();
        }
        if pen.full() {
            pen.finish();
