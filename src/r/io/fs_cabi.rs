...
#[unsafe(no_mangle)]
pub extern "C" fn trueos_cabi_ssh_shell_open(cols: u32, rows: u32) -> i64 {
    if crate::hv::current_hull_guest_context_vm_id().is_none() {
        return -1;
    }
    let (status, session) = trueos_vm::vmcall::call(
        trueos_vm::vmcall::OP_BP_SSH_SHELL_OPEN,
        u64::from(cols),
        u64::from(rows),
    );
    if status == trueos_vm::vmcall::STATUS_OK {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn trueos_cabi_ssh_shell_write(
    session: u32,
    data_ptr: *const u8,
    data_len: usize,
) -> isize {
    ...
    let data = unsafe { core::slice::from_raw_parts(data_ptr, data_len) };
    let (status, written) = trueos_vm::vmcall::call_with_payload(
        trueos_vm::vmcall::OP_BP_SSH_SHELL_WRITE,
        u64::from(session),
        0,
        data,
        &mut [],
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn trueos_cabi_ssh_shell_read(
    session: u32,
    out_ptr: *mut u8,
    out_cap: usize,
) -> isize {
    ...
    let output = unsafe { core::slice::from_raw_parts_mut(out_ptr, out_cap) };
    let (status, read) = trueos_vm::vmcall::call_with_payload(
        trueos_vm::vmcall::OP_BP_SSH_SHELL_READ,
        u64::from(session),
        out_cap as u64,
        &[],
        output,
...
pub extern "C" fn trueos_cabi_ssh_shell_resize(
    ...
    let (status, rc) = trueos_vm::vmcall::call(
        trueos_vm::vmcall::OP_BP_SSH_SHELL_RESIZE,
        u64::from(session),
        packed,
    );
    if status == trueos_vm::vmcall::STATUS_OK {
...
#[unsafe(no_mangle)]
pub extern "C" fn trueos_cabi_ssh_shell_close(session: u32) -> i32 {
    if crate::hv::current_hull_guest_context_vm_id().is_none() {
        return -1;
    }
    let (status, rc) = trueos_vm::vmcall::call(
