...
pub(crate) fn broker_retained_frame_submit(
    ...
    broker_retained_frame_submit_inner(principal, device, queue, submit, None, None, None)
}
...
pub(crate) fn broker_retained_frame_submit_v3(
    ...
    broker_retained_frame_submit_inner(
        ...
        queue,
        submit.frame.frame,
        Some(submit.frame.material_parameters),
        Some(submit),
        None,
    )
}
pub(crate) fn broker_retained_frame_submit_v4(
    principal: Principal,
    device: u64,
    queue: u64,
    submit: v::vgpu::RetainedFrameSubmitV4,
) -> Result<v::vgpu::TimelinePoint, i32> {
    broker_retained_frame_submit_inner(
        principal,
        device,
...
fn broker_retained_frame_submit_inner(
    principal: Principal,
    device: u64,
    queue: u64,
    submit: v::vgpu::RetainedFrameSubmit,
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn trueos_cabi_vgpu_retained_frame_submit_v2(
    device: u64,
    queue: u64,
    submit: *const v::vgpu::RetainedFrameSubmitV2,
    out_point: *mut v::vgpu::TimelinePoint,
) -> i32 {
    if submit.is_null() || out_point.is_null() {
        return -14;
    ...
    let payload = unsafe {
        core::slice::from_raw_parts(
            (&submit as *const v::vgpu::RetainedFrameSubmitV2).cast::<u8>(),
            core::mem::size_of::<v::vgpu::RetainedFrameSubmitV2>(),
        )
    };
    let result = if crate::hv::current_hull_guest_context_vm_id().is_some() {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn trueos_cabi_vgpu_retained_frame_submit_v4(
    device: u64,
    queue: u64,
    submit: *const v::vgpu::RetainedFrameSubmitV4,
    out_point: *mut v::vgpu::TimelinePoint,
) -> i32 {
    if submit.is_null() || out_point.is_null() {
        return -14;
    ...
    let payload = unsafe {
        core::slice::from_raw_parts(
            (&submit as *const v::vgpu::RetainedFrameSubmitV4).cast::<u8>(),
            core::mem::size_of::<v::vgpu::RetainedFrameSubmitV4>(),
        )
    };
    let result = if crate::hv::current_hull_guest_context_vm_id().is_some() {
        guest_record(trueos_vm::vmcall::OP_BP_VGPU_RETAINED_FRAME_SUBMIT_V4, device, queue, payload)
    } else {
        broker_retained_frame_submit_v4(direct_principal(), device, queue, submit)
    };
    match result {
        Ok(point) => {
            unsafe { out_point.write(point) };
...
pub(crate) fn broker_retained_textured_frame_v1(
    principal: Principal,
    device: u64,
    queue: u64,
    submit: v::vgpu::RetainedTexturedFrameV1,
) -> Result<v::vgpu::TimelinePoint, i32> {
    let owner = ui4_owner(principal)?;
    let completed = vgpu::submit_ui4_retained_frame(
        principal,
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn trueos_cabi_vgpu_retained_textured_frame_v1(
    device: u64,
    queue: u64,
    submit: *const v::vgpu::RetainedTexturedFrameV1,
    out_point: *mut v::vgpu::TimelinePoint,
) -> i32 {
    if submit.is_null() || out_point.is_null() {
        return -14;
    ...
    let payload = unsafe {
        core::slice::from_raw_parts(
            (&submit as *const v::vgpu::RetainedTexturedFrameV1).cast::<u8>(),
            core::mem::size_of::<v::vgpu::RetainedTexturedFrameV1>(),
        )
    };
    let result = if crate::hv::current_hull_guest_context_vm_id().is_some() {
        guest_record(
            trueos_vm::vmcall::OP_BP_VGPU_RETAINED_TEXTURED_FRAME_V1,
            device,
            queue,
            payload,
        )
    } else {
        broker_retained_textured_frame_v1(direct_principal(), device, queue, submit)
    };
    match result {
        Ok(point) => {
            unsafe { out_point.write(point) };
