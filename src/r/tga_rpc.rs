...
//! delivers the result to a Rust future or callback. Only LED heartbeat is exposed.
...
pub async fn service_task() {
    ...
    crate::log_important!(
        "tga-rpc: worker started functions=heartbeat transport=bar0 completion=msi\n"
    );
    loop {
...
pub async fn heartbeat_task() {
    crate::log_important!("tga-rpc: heartbeat client started\n");
            ...
            match heartbeat().await {
                Ok(true) => {
                    ...
                    if completed % 120 == 0 {
                        ...
                        crate::log_important!(
                            "tga-rpc: LED heartbeat alive completed={} interrupts={}\n",
                            completed,
                            crate::tga::completion_interrupt_count()
                        );
                    }
                    if announced_generation != Some(generation) {
                        crate::log_important!("tga-rpc: heartbeat online\n");
                        announced_generation = Some(generation);
                    }
                }
                result => {
                    crate::log_warn!(
                        "tga-rpc: heartbeat failed result={:?}; rediscovering\n",
                        result
                    );
                    announced_generation = None;
                    if crate::tga::connection_generation() == generation {
                        crate::tga::disconnect();
                    }
                }
            }
        }
