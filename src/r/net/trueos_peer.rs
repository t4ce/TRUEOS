...
const TRUEOS_HOST_ADVERTISEMENT_QUEUE_CAP: usize = TRUEOS_SWARM_HOST_CAP * 2;
const TRUEOS_PEER_ADVERTISEMENT_QUEUE_CAP: usize = 4;

static HOST_ADVERTISEMENTS: Mutex<VecDeque<trueos_esp::gate::TrueOsHostAdvertisement>> =
    Mutex::new(VecDeque::new());
static OUTBOUND_PEER_ADVERTISEMENTS: Mutex<VecDeque<Vec<u8>>> = Mutex::new(VecDeque::new());
...
pub(crate) fn publish_host_advertisement(advertisement: trueos_esp::gate::TrueOsHostAdvertisement) {
    ...
    {
        let mut queue = HOST_ADVERTISEMENTS.lock();
        if queue.len() >= TRUEOS_HOST_ADVERTISEMENT_QUEUE_CAP {
            let _ = queue.pop_front();
        }
        queue.push_back(advertisement);
    }
...
pub(crate) fn request_lumen_work_capacity_probe() -> u32 {
    #[cfg(feature = "trueos_lumen")]
    {
        ...
        crate::log!(
            "trueos-peer: lumen work capacity probe requested seq={} timeout_ms={}\n",
            seq,
    ...
    #[cfg(feature = "trueos_lumen")]
    {
        ...
        loop {
            if LUMEN_WORK_PROBE_RESULT_SEQ.load(Ordering::Acquire) == seq {
                ...
                crate::log!(
                    "trueos-peer: lumen prompt offload gate seq={} sent={} replies={} best={} enabled={}\n",
                    seq,
                    sent,
                    replies,
                    best,
                ...
                && embassy_time_driver::now().saturating_sub(start) > timeout_ticks
            {
                crate::lumen::lumen_net::set_remote_bf16_route_available(false);
                crate::log!(
                    "trueos-peer: lumen prompt offload gate seq={} result=timeout enabled=0\n",
                    seq
                );
                return false;
            }
...
pub(crate) fn take_peer_advertisement() -> Option<Vec<u8>> {
    OUTBOUND_PEER_ADVERTISEMENTS.lock().pop_front()
}
...
fn process_lumen_app_text_payload(
    ...
    if let Some(line) = trueos_lumen_matvec_shadow_received(payload) {
        crate::log!(
            "trueos-peer: lumen matvec shadow received handle={} bytes={} {}\n",
            handle.0,
            payload.len(),
            line
        );
    ...
    for line in text.lines().filter(|line| {
        ...
        {
            crate::log!(
                "trueos-peer: lumen matvec result chunk received handle={} job={} chunk={} offset={} bytes={} total={} got={} chunks={} complete={} copied_rows={} checksum=0x{:016X}\n",
                handle.0,
                chunk.job_id,
                chunk.chunk_index,
                chunk.offset,
    ...
    for line in text.lines().filter(|line| {
        ...
        if let Some(chunk) = parse_lumen_matvec_xchunk(line) {
            if let Some(update) = record_shadow_x_chunk(shadow_x_reassemblies, chunk) {
                crate::log!(
                    "trueos-peer: lumen matvec xchunk received handle={} job={} chunk={} offset={} bytes={} total={} got={} chunks={} complete={}\n",
                    handle.0,
                    chunk.job_id,
                    chunk.chunk_index,
                    chunk.offset,
                ...
                if update.complete {
                            ...
                            )
                    {
                        ...
                        crate::log!(
                            "trueos-peer: lumen matvec x reassembled job={} bytes={} chunks={} checksum=0x{:016X} proof=remote-bf16-matvec rows={} matrix=0x{:016X} result_checksum=0x{:016X} first={:.6} last={:.6} result_bytes={} result_frames={}\n",
                            chunk.job_id,
                            update.total_bytes,
                            update.chunks,
                            update.checksum,
                        ...
                        crate::log!(
                            "trueos-peer: lumen matvec x reassembled job={} bytes={} chunks={} checksum=0x{:016X} proof=unavailable\n",
                            chunk.job_id,
                            update.total_bytes,
                            update.chunks,
                            update.checksum
                ...
                crate::log!(
                    "trueos-peer: lumen matvec xchunk rejected handle={} payload_bytes={}\n",
                    handle.0,
                    payload.len()
                );
            }
...
fn submit_trueos_lumen_work_capacity(vnet: &VNet, handle: v::vnet::NetHandle) {
    ...
    crate::log!(
        "trueos-peer: lumen work capacity reply handle={} n={} proto={} caps=0x{:08X} workers={} pending={} min_rows={} online={} running={}\n",
        handle.0,
        telemetry.capacity_lanes,
        telemetry.protocol_version,
        telemetry.caps,
...
fn submit_trueos_lumen_shadow_frame_to_first_peer(
    ...
    crate::log!(
        "trueos-peer: lumen app frame sent handle={} opcode={} payload_bytes={} wire_bytes={} pending={}\n",
        handle.0,
...
fn submit_trueos_lumen_result_frames(
    ...
    for (chunk_index, offset) in (0..result_bytes.len()).step_by(chunk_bytes).enumerate() {
        ...
        if frame.len() > v::vnet::MAX_MSG {
            crate::log!(
                "trueos-peer: lumen matvec result send skipped job={} chunk={} reason=frame-too-large wire_bytes={}\n",
                job_id,
                chunk_index,
                frame.len()
            );
...
fn queue_trueos_peer_advertisement(node_id: u64) {
    let hello = trueos_peer_hello(node_id);
    let bytes = hello.as_bytes();
    let len = bytes.len().min(TRUEOS_PEER_HELLO_MAX);
    let mut queue = OUTBOUND_PEER_ADVERTISEMENTS.lock();
    if queue.len() >= TRUEOS_PEER_ADVERTISEMENT_QUEUE_CAP {
        let _ = queue.pop_front();
    }
    ...
    F: FnMut(LumenAppFrame<'_>),
{
    ...
    if link.lumen_rx.len().saturating_add(data.len()) > TRUEOS_LUMEN_APP_RX_BUF_BYTES {
        link.lumen_rx.clear();
        crate::log!(
            "trueos-peer: lumen app rx reset handle={} reason=overflow cap={}\n",
            handle.0,
...
fn take_host_advertisements(max: usize) -> Vec<trueos_esp::gate::TrueOsHostAdvertisement> {
    let mut out = Vec::new();
    let mut queue = HOST_ADVERTISEMENTS.lock();
    for _ in 0..max {
        let Some(advertisement) = queue.pop_front() else {
            break;
        };
...
pub async fn trueos_peer_task() {
    ...
    loop {
        ...
        crate::log!(
            "trueos-peer: starting trueos peer protocol trueos_magic={} peer_tcp={} node=0x{:016X} peer_slots={} rx_buf_bytes={}\n",
            trueos_esp::gate::TRUEOS_SWARM_MAGIC_TEXT,
            trueos_esp::gate::TRUEOS_PEER_TCP_PORT,
            local_node_id,
        ...
        loop {
            if let Some(ev) = vnet.pop_event() {
                if let v::vnet::Event::Error { msg } = ev {
                    crate::log!("trueos-peer: error {}\n", msg);
                    if msg == "bad handle" {
                        let cleared = clear_trueos_peer_links(peer_links.as_mut_slice());
                        crate::lumen::lumen_net::set_remote_bf16_route_available(false);
                        crate::log!(
                            "trueos-peer: peer links cleared reason=bad-handle count={}\n",
                            cleared
                        );
                    }
                }
                ...
                match ev {
                    v::vnet::Event::Opened {
                        ...
                        crate::log!(
                            "trueos-peer: tcp listener bound handle={} port={}\n",
                            handle.0,
                            trueos_esp::gate::TRUEOS_PEER_TCP_PORT
                        );
                    }
                    v::vnet::Event::TcpEstablished { handle, .. } => {
                        if !ensure_trueos_peer_link(peer_links.as_mut_slice(), handle) {
                            crate::log!(
                                "trueos-peer: tcp rejected handle={} reason=peer-slot-cap cap={}\n",
                                handle.0,
                                TRUEOS_PEER_LINK_CAP
                            );
                        ...
                        crate::log!(
                            "trueos-peer: tcp established handle={} active_links={} sending hello\n",
                            handle.0,
                    ...
                    v::vnet::Event::TcpData { handle, data } => {
                        ...
                        if let Some(advertisement) = record_trueos_peer_data(
                            ...
                            crate::log!(
                                "trueos-peer: hello received handle={} node=0x{:016X} bytes={}\n",
                                handle.0,
                            ...
                            if let Some(retained) = trueos_peer_duplicate_handle(
                                ...
                                crate::log!(
                                    "trueos-peer: duplicate handle={} retained_handle={} node=0x{:016X} action=close-duplicate\n",
                                    handle.0,
                                    retained.0,
                                    advertisement.node_id
                                );
                        ...
                        if !close_duplicate_peer {
                            ...
                            drain_lumen_app_data_for_handle(
                                ...
                                |frame| {
                                    if frame.opcode == TRUEOS_LUMEN_APP_OP_TEXT {
                                        ...
                                        crate::log!(
                                            "trueos-peer: lumen app frame ignored handle={} opcode={} bytes={}\n",
                                            handle.0,
                                            frame.opcode,
                                            frame.payload.len()
                                        );
                                ...
                                parse_trueos_lumen_work_capacity(data.as_slice())
                            {
                                ...
                                crate::log!(
                                    "trueos-peer: lumen work capacity received handle={} node=0x{:016X} n={} proto={} caps=0x{:08X} workers={} pending={} min_rows={} counted={}\n",
                                    handle.0,
                                    node_id,
                                    capacity.lanes,
                    ...
                    v::vnet::Event::Closed { handle } if peer_listener == Some(handle) => {
                        ...
                        let _ = remove_trueos_peer_link(peer_links.as_mut_slice(), handle);
                        crate::log!(
                            "trueos-peer: tcp listener closed, reopening port={} retained_peer_node=0x{:016X} active_links={}\n",
                            trueos_esp::gate::TRUEOS_PEER_TCP_PORT,
                            retained_peer_node,
                            trueos_peer_link_count(peer_links.as_slice())
                        );
                    ...
                    v::vnet::Event::Closed { handle } => {
                        if remove_trueos_peer_link(peer_links.as_mut_slice(), handle) {
                            ...
                            crate::log!(
                                "trueos-peer: tcp closed handle={} active_links={}\n",
                                handle.0,
                                trueos_peer_link_count(peer_links.as_slice())
                            );
                        }
            ...
            let now_ms = monotonic_ms();
            for advertisement in take_host_advertisements(TRUEOS_SWARM_HOST_CAP) {
                if advertisement.node_id != 0 && advertisement.node_id == local_node_id {
                    continue;
                }
                crate::log!(
                    "trueos-peer: host advertisement from {}.{}.{}.{} peer_tcp={} node=0x{:016X} caps=0x{:08X}\n",
                ...
                if trueos_peer_link_known(peer_links.as_slice(), advertisement.node_id) {
                    continue;
                }
                if trueos_peer_link_has_room_or_known(peer_links.as_slice(), advertisement.node_id)
                {
                    ...
                    crate::log!(
                        "trueos-peer: dial skipped node=0x{:016X} reason=peer-slot-cap cap={}\n",
                        advertisement.node_id,
                        TRUEOS_PEER_LINK_CAP
                    );
                }
            ...
            if now_ms >= next_peer_advertise_ms {
                queue_trueos_peer_advertisement(local_node_id);
            ...
            if requested_lumen_work_probe_seq != seen_lumen_work_probe_seq {
                ...
                if lumen_work_probe_sent == 0 {
                    ...
                    crate::log!(
                        "trueos-peer: lumen work capacity probe seq={} skipped reason=no-peers\n",
                        lumen_work_probe_seq
                    );
                } else {
                    ...
                    crate::log!(
                        "trueos-peer: lumen work capacity probe seq={} sent={} timeout_ms={}\n",
                        lumen_work_probe_seq,
                        lumen_work_probe_sent,
                        TRUEOS_LUMEN_WORK_PROBE_TIMEOUT_MS
                    );
            ...
            if lumen_work_probe_deadline_ms != 0 && now_ms >= lumen_work_probe_deadline_ms {
                ...
                crate::log!(
                    "trueos-peer: lumen work capacity probe seq={} complete sent={} replies={} best={}\n",
                    lumen_work_probe_seq,
                    lumen_work_probe_sent,
                    lumen_work_probe_replies,
                    lumen_work_probe_best
            ...
            if trueos_peer_link_count(peer_links.as_slice()) != 0
                && let Some(frame) = crate::lumen::lumen_net::take_shadow_bf16_matvec_frame()
            ...
            {
                crate::log!("trueos-peer: lumen matvec shadow send failed bytes={}\n", frame.len());
            }
