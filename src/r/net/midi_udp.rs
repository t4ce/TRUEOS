//! UDP ingress for the network-attached MIDI piano controller.
//!
//! The wire sender happens to be a microcontroller, but this service is an
...
pub const MIDI_PIANO_UDP_PORT: u16 = crate::allports::services::MIDI_PIANO_UDP_PORT;
...
impl PianoUdpReceiver {
    ...
    /// Parse one piano datagram without coupling the wire parser to a vnet handle.
...
pub async fn midi_piano_udp_task() {
    ...
    loop {
        ...
        let mut piano = PianoUdpReceiver::new();
        let _ = vnet.submit(piano.bootstrap_command());
        crate::log!(
            "midi-udp: starting piano listener port={} keys={}\n",
            MIDI_PIANO_UDP_PORT,
            PIANO_KEY_COUNT
        );
            ...
            if let Some(ev) = vnet.pop_event() {
                match ev {
                    api::Event::Opened { handle, kind } if kind == api::SocketKind::Udp => {
                        piano.bind(handle);
                        crate::log!(
                            "midi-udp: piano listener bound handle={} port={}\n",
                            handle.0,
                            MIDI_PIANO_UDP_PORT
                        );
                    }
                    api::Event::Closed { handle } if piano.unbind(handle) => {
                        crate::aud::live_piano::all_notes_off();
                        crate::log!("midi-udp: piano listener closed, reopening\n");
                        let _ = vnet.submit(piano.bootstrap_command());
                    }
                    api::Event::UdpPacket { handle, from, data } => {
                        let handled = piano.on_packet(handle, data.as_slice(), |event| {
                            let kind = match event.kind {
                                PianoNoteEventKind::Down => "down",
                                PianoNoteEventKind::Up => "up",
                            };
                            crate::log!(
                                "midi-udp: note {} key={} note={} velocity={} delta={} from={}.{}.{}.{}\n",
                                kind,
                                event.key_index,
                                event.note,
                                event.velocity,
