...
function rawLabel(li){var s=folderSummary(li);var n=firstTextNode(s||li);return n?n.textContent||"":"";}
...
function isAppsFolder(path){var parts=path.split("/");return parts.length>0&&parts[parts.length-1].toLowerCase()==="apps";}
...
function wireFolder(root,li,path){if(li.getAttribute("data-trueosfs-folder")==="1"){return;}li.setAttribute("data-trueosfs-folder","1");var details=ensureFolderDetails(li);if(details){details.open=isAppsFolder(path);}var summary=folderSummary(li)||li;summary.style.fontWeight="bold";var host=document.createElement("span");var del=document.createElement("button");del.type="button";del.textContent="x";var uploadBtn=document.createElement("button");uploadBtn.type="button";uploadBtn.textContent="upload";var createBtn=document.createElement("button");createBtn.type="button";createBtn.textContent="+";var picker=document.createElement("input");picker.type="file";picker.hidden=true;var status=document.createElement("small");host.appendChild(document.createTextNode(" "));host.appendChild(del);host.appendChild(document.createTextNode(" "));host.appendChild(uploadBtn);host.appendChild(document.createTextNode(" "));host.appendChild(createBtn);host.appendChild(document.createTextNode(" "));host.appendChild(status);summary.appendChild(host);li.appendChild(picker);del.addEventListener("click",function(ev){ev.preventDefault();ev.stopPropagation();deleteFile(root,path,cleanLabel(li),status);});uploadBtn.addEventListener("click",function(){picker.click();});picker.addEventListener("change",function(){if(picker.files&&picker.files[0]){uploadFile(root,path,picker.files[0],status);}picker.value="";});createBtn.addEventListener("click",function(){var name=window.prompt("Folder name");if(!name){return;}setStatus(status,"creating "+name+" ...");fetch(mkdirHref(root,path,name),{method:"POST"}).then(function(resp){if(!resp.ok){throw new Error(String(resp.status));}setStatus(status,"created "+name);window.setTimeout(function(){window.location.reload();},250);}).catch(function(err){setStatus(status,"create failed: "+err.message);});});}
                            ...
                            && path_only.starts_with("/dl/")
                        {
                            ...
                            for r in roots.iter() {
                                ...
                                let tree = match crate::disc::block::device_handle(r.disk_id) {
                                    None => None,
                                    Some(disk) => {
                                        match crate::r::fs::fs_html::html_tree_async(disk, HTTP_TRUEOSFS_MAX_ENTRIES)
                                            .await
                                        {
                                            Ok(v) => v,
                                            Err(_) => None,
                                ...
                                trees_html.push_str(
                                    format!(
                                        "<details open><summary>{}</summary><div class=\"tree\" data-root=\"{}\">",
                                        r.disk_id, raw
                                    )
                                    .as_str(),
                                );
