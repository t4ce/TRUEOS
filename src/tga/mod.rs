...
//!
//! TRUEOS owns only discovery, hotplug, BAR0 mapping, MSI completion, and LED
...
// 0x42 now belongs to the hypervisor lifecycle kick; MSI uses its own vector.
pub(crate) const TGA_COMPLETION_VECTOR: u8 = 0x44;
const _: () = assert!(TGA_COMPLETION_VECTOR != crate::hv::control_kick::LIFECYCLE_KICK_VECTOR);
const _: () = assert!(TGA_COMPLETION_VECTOR != crate::live_update::RENDEZVOUS_VECTOR);
const _: () = assert!(TGA_COMPLETION_VECTOR != crate::remote_work_wake::REMOTE_WORK_WAKE_VECTOR);
const _: () = assert!(TGA_COMPLETION_VECTOR != crate::chronos::CHRONOS_TIMER_VECTOR);
...
fn is_present(tga: &Tga) -> bool {
    crate::pci::config_read_u16(tga.bus, tga.slot, tga.function, 0) == TGA_VENDOR_ID
        && crate::pci::config_read_u16(tga.bus, tga.slot, tga.function, 2) == TGA_DEVICE_ID
}
...
fn bring_online(device: &PciDevice) -> Option<Tga> {
    ...
    let last_map = *TGA_LAST_MAP.lock();
    let mapped = match last_map {
...
pub fn try_init() -> bool {
    ...
    let found = crate::pci::with_devices(|devices| devices.iter().copied().find(is_tga));
    let Some(device) = found else {
        return false;
    };
    try_init_device(device)
}

fn try_init_device(device: PciDevice) -> bool {
...
pub(crate) fn disconnect() {
    ...
    if !is_present(&old) {
        crate::pci::forget_discovered_device(
            old.bus,
            old.slot,
            old.function,
            TGA_VENDOR_ID,
...
/// Probe one slot per yield; never run a synchronous whole-bus scan here.
async fn discover() -> Option<PciDevice> {
    for bus in 0u8..=255 {
        for slot in 0u8..32 {
            let vendor = crate::pci::config_read_u16(bus, slot, 0, 0);
            if vendor != 0xffff {
                let functions = if crate::pci::config_read_u8(bus, slot, 0, 0x0e) & 0x80 != 0 {
                    8
                } else {
                    1
                };
                for function in 0..functions {
                    if crate::pci::config_read_u16(bus, slot, function, 0) == TGA_VENDOR_ID
                        && crate::pci::config_read_u16(bus, slot, function, 2) == TGA_DEVICE_ID
                    {
                        return Some(PciDevice {
                            bus,
                            slot,
                            function,
                            vendor: TGA_VENDOR_ID,
                            device: TGA_DEVICE_ID,
                            vendor_id: TGA_VENDOR_ID,
                            device_id: TGA_DEVICE_ID,
                            class: crate::pci::config_read_u8(bus, slot, function, 0x0b),
                            subclass: crate::pci::config_read_u8(bus, slot, function, 0x0a),
                            prog_if: crate::pci::config_read_u8(bus, slot, function, 9),
                        });
                    }
                }
            }
...
pub(crate) async fn tga_task() {
    crate::log_important!(
        "tga: PCI heartbeat discovery started vid=22c2 did=1100 scan=incremental\n"
    );
    ...
    loop {
        if !is_online() {
            if !try_init() {
                if let Some(device) = discover().await {
                    crate::pci::publish_discovered_device(device);
                    try_init_device(device);
                    missing_logged = false;
                } else if !missing_logged {
                    crate::log_important!(
                        "tga: no responding 22c2:1100 endpoint after incremental scan; retrying\n"
                    );
                    missing_logged = true;
                }
            }
