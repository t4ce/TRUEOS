...
pub(crate) fn status() -> Result<CameraStatus, &'static str> {
    ...
    if !super::is_present(dev) {
        return Err("FPGA disappeared");
    }
    let read = |offset| super::Tga::read_reg(dev.mmio_base + offset);
    if read(0x280) != 0x43414d31 {
        return Err("camera firmware not loaded");
    }
    let dimensions = read(0x288);
    Ok(CameraStatus {
        flags: read(0x284),
        width: dimensions as u16,
        height: (dimensions >> 16) as u16,
        frames: read(0x28c),
        edid_reads: read(0x290),
    })
}
...
pub(crate) async fn read_tile() -> Result<Vec<u8>, &'static str> {
    let state = status()?;
    if state.flags & 4 == 0 {
        return Err("waiting for HDMI pixels (tile not ready)");
    }
    ...
    for row in 0..64u32 {
        {
            ...
            for col in 0..64u32 {
                ...
                pixels
                    .extend_from_slice(&super::Tga::read_reg(dev.mmio_base + 0x304).to_le_bytes());
            }
        }
