...
pub(crate) async fn reset_uas_skhynix_transport_for_bench(
    ...
    let result = async {
        ...
        uas_skhynix_reset_transport(&mut rt, stage).await?;
...
pub(super) fn uas_bench_now_ms() -> u64 {
...
fn uas_skhynix_clear_stream_epoch(rt: &mut UsbMassRuntime, stage: &'static str) {
    let dead = rt.uas_dead_stream_mask.count_ones();
    if dead == 0 {
        rt.uas_next_stream_tag = 1;
        return;
    }
    ...
    rt.uas_next_stream_tag = 1;
    crate::log_info!(target: "usb";
        "crabusb: mass {:04X}:{:04X} uas-window stream epoch reset stage={} cleared_dead={} faults={}\n",
        rt.vendor_id,
        rt.product_id,
        stage,
        dead,
...
async fn uas_skhynix_reset_reclaim_epoch(
    rt: &mut UsbMassRuntime,
    stage: &'static str,
) -> block::Result<()> {
    uas_skhynix_reset_transport(rt, stage).await?;
    uas_skhynix_clear_stream_epoch(rt, stage);
    Ok(())
}
...
async fn uas_skhynix_abort_pending_window(
    ...
    let _ = uas_skhynix_reset_reclaim_epoch(rt, stage).await;
}

async fn uas_skhynix_abort_pending_write_window(
    rt: &mut UsbMassRuntime,
    ...
    crate::log_info!(target: "usb";
        "crabusb: mass {:04X}:{:04X} uas-write-window abort pending={} stage={} dead_streams={}\n",
        rt.vendor_id,
        rt.product_id,
        pending,
        stage,
...
fn uas_write_poll_flights(
    ...
    while idx < flights.len() {
        ...
        let mut step_err = None;
        {
            ...
            if !timed_out && step_err.is_none() {
                if flight.write_ready_seen && !flight.data_done && flight.data_ticket.is_none() {
                    let start = flight.data_offset;
                    let end = start.saturating_add(flight.bytes);
                    if end > source.len() {
        ...
        if timed_out {
            ...
            return Ok(Some(UasWriteStep::TimedOut {
                ...
                bytes: flight.bytes,
                timing,
                write_ready_seen: flight.write_ready_seen,
                data_done: flight.data_done,
                status_good_seen: flight.status_good_seen,
            }));
        }
...
async fn uas_write_wait_one(
    ...
    let mut tick = core::pin::pin!(Timer::after(EmbassyDuration::from_millis(UAS_BENCH_TICK_MS)));
    poll_fn(|cx| {
        match uas_write_poll_flights(status_in, data_out, source, flights, cx) {
            Ok(Some(step)) => return Poll::Ready(Ok(Some(step))),
            Err(err) => return Poll::Ready(Err(err)),
            Ok(None) => {}
        }
...
pub(super) fn uas_runtime_alloc_stream(rt: &mut UsbMassRuntime) -> Option<u16> {
    for _ in 0..mass::UAS_XHCI_MAX_STREAM_ID {
        let tag = rt
            .uas_next_stream_tag
...
pub(super) fn uas_error_retires_stream(err: mass::MassProbeError) -> bool {
    matches!(
...
pub(super) fn uas_runtime_note_error(
    rt: &mut UsbMassRuntime,
    stage: &'static str,
    tag: u16,
...
pub(super) fn uas_runtime_log_exhausted(rt: &UsbMassRuntime, stage: &'static str) {
    crate::globalog::log_with_level(
        log::Level::Warn,
        format_args!(
...
fn uas_runtime_note_transport_timeout(
    rt: &mut UsbMassRuntime,
    stage: &'static str,
    tag: u16,
    err: mass::MassProbeError,
    ...
    crate::globalog::log_with_level(
        log::Level::Warn,
        format_args!(
            "crabusb: mass {:04X}:{:04X} uas-transport timeout stage={} tag=0x{:04X} err={:?} dead={} faults={} io_limit={}\n",
            rt.vendor_id,
            rt.product_id,
            stage,
            tag,
...
pub(super) async fn read_blocks_uas_skhynix_windowed(
    rt: &mut UsbMassRuntime,
    lba: u64,
    blocks: usize,
    out: &mut [u8],
    ...
    while completed_bytes < total_bytes as u64 || !flights.is_empty() {
        ...
        match step {
            ...
            UasBenchStep::TimedOut(flight) => {
                ...
                timeouts = timeouts.saturating_add(1);
                uas_runtime_note_transport_timeout(
                    rt,
                    "read-window",
                    lost_tag,
                ...
                if !had_pending {
                    ...
                    let _ = uas_skhynix_reset_reclaim_epoch(rt, "read-window-timeout").await;
                }
                if let Some(route) = route.as_mut() {
                    route.phase = UasRoutePhase::Reset;
                    route.counters.dead_streams = rt.uas_dead_stream_mask.count_ones();
                }
                if timeouts >= UAS_SKHYNIX_WINDOW_TIMEOUT_LIMIT {
                    crate::globalog::log_with_level(
                        log::Level::Warn,
                        format_args!(
                            "crabusb: mass {:04X}:{:04X} uas-window timeout-limit stage=read-window lba={} completed={} submitted={} timeouts={} io_limit={}\n",
                            rt.vendor_id,
                            rt.product_id,
                            retry_lba,
                            completed_bytes,
...
pub(super) async fn write_blocks_uas_skhynix_windowed(
    rt: &mut UsbMassRuntime,
    lba: u64,
    buf: &[u8],
    block_size: usize,
    ...
    let mut cwnd = if rt.skhynix_write_probe_fail_fast {
        max_inflight
    } else {
        1
    };
    ...
    while completed_bytes < total_bytes as u64 || !flights.is_empty() {
        ...
        match step {
            ...
            UasWriteStep::TimedOut {
                ...
                uas_skhynix_abort_pending_write_window(
                    rt,
                    &mut flights,
                ...
                .await;
                if !had_pending {
                    let _ = uas_skhynix_reset_reclaim_epoch(rt, "write-window-timeout").await;
                }
                if let Some(route) = route.as_mut() {
                    route.phase = UasRoutePhase::Reset;
                    route.counters.dead_streams = rt.uas_dead_stream_mask.count_ones();
                ...
                crate::log_info!(target: "usb";
                    "crabusb: mass {:04X}:{:04X} uas-write-window timeout tag=0x{:04X} lba={} bytes={} completed={} submitted={} ready={} data_done={} status_good={}\n",
                    rt.vendor_id,
                    rt.product_id,
                    tag,
                    lba,
                    ...
                    completed_bytes,
                    submitted_bytes,
                    write_ready_seen as u8,
                    data_done as u8,
                    status_good_seen as u8
                );
                return Err(block::Error::Timeout);
            }
        }
...
pub(super) async fn mass_storage_uas_skhynix_task(
    mut device: Device,
    controller_id: u32,
    target: UasTarget,
