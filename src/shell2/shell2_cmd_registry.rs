...
const TOOL_JSON_HELIO: &str = r#"{"type":"object","properties":{"action":{"type":"string","enum":["start","status"],"description":"Launch the embedded Helio simple-cube game or inspect its runtime state."}},"required":[],"additionalProperties":false}"#;
...
const BUILTIN_CMD_REGISTRY: &[BuiltinShell2CmdEntry] = &[
    ...
    BuiltinShell2CmdEntry {
        ...
        tool_description: Some(
            "Launch the embedded build-produced Helio simple-cube game through Render/GuC and UI4, or inspect its state.",
        ),
...
mod tests {
    ...
    fn helio_is_advertised_as_a_launchable_runtime_use_case() {
        ...
        assert!(registry.contains("Launch the embedded build-produced Helio simple-cube game"));
