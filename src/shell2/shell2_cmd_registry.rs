...
const TOOL_JSON_FONT: &str = r#"{"type":"object","properties":{"text":{"type":"string","minLength":1,"maxLength":256,"description":"One UTF-8 row tessellated and stamped once at the center of the native primary framebuffer."},"rows":{"type":"array","minItems":1,"items":{"type":"string","minLength":1,"maxLength":256},"description":"Explicit UTF-8 rows combined into one transient centered font draw and framebuffer stamp."},"font_id":{"type":"integer","minimum":1,"maximum":2,"description":"Embedded font: 1=L_10646, 2=Noto Sans SC."},"size":{"type":"integer","minimum":1,"maximum":100,"default":100,"description":"Percentage of the aspect-preserving full-scanout fit."},"color":{"type":"string","pattern":"^[0-9A-Fa-f]{8}$","description":"Static stamp color as RRGGBBAA."}},"oneOf":[{"required":["text"]},{"required":["rows"]}],"additionalProperties":false}"#;
...
const BUILTIN_CMD_REGISTRY: &[BuiltinShell2CmdEntry] = &[
    ...
    BuiltinShell2CmdEntry {
        ...
        tool_description: Some(
            "Tessellate UTF-8 text and aspect-fit it at the center of the native display.",
        ),
