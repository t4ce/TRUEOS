...
const TOOL_JSON_LUMEN: &str = r#"{"type":"object","properties":{"command":{"type":"string","enum":["hello","decode"],"description":"Run the sealed FFN proof or one fixed LFM2.5 decode token through TRUEGA."},"token":{"type":"integer","minimum":0,"maximum":65535,"description":"Input token required by command=decode."}},"required":["command"],"additionalProperties":false}"#;
