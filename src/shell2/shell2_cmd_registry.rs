...
const TOOL_JSON_CPP: &str = r#"{"type":"object","properties":{"action":{"type":"string","enum":["start","list","status","stop","spirit"],"description":"Launch, inspect, or stop the C++/IGC demo suite, or select Spirit's C++ repass."},"mode":{"type":"string","enum":["gallery","aurora","julia","sdf","voronoi"],"description":"Foundational C++ for OpenCL workload to display."},"duration_ms":{"type":"integer","minimum":0,"description":"Demo lifetime in milliseconds; zero runs until stopped."},"cadence_ms":{"type":"integer","minimum":1,"maximum":60000,"description":"Target GPU launch cadence in milliseconds."},"publish_every":{"type":"integer","minimum":1,"maximum":1024,"description":"Publish every Nth retired GPU frame."},"background_id":{"type":"integer","enum":[0,2,3,4,5,6,7,8,9,10],"description":"Spirit background ID when action is spirit."},"shader_id":{"type":"integer","minimum":0,"maximum":15,"description":"Spirit sprite shader ID when action is spirit."}},"required":[],"additionalProperties":false}"#;
...
fn dispatch_tga(spawner: &Spawner, io: &'static dyn ShellBackend2, rest: &str) -> ParseOutcome {
    super::cmds::tga::try_parse(spawner, io, rest)
}
...
const BUILTIN_CMD_REGISTRY: &[BuiltinShell2CmdEntry] = &[
    ...
    BuiltinShell2CmdEntry {
        ...
        tool_description: Some(
            "Launch five UI4 demos or select Spirit's 9x16 visual suite from reproducibly baked C++ for OpenCL and Intel IGC Zebins.",
        ),
    ...
    BuiltinShell2CmdEntry {
        ...
        tool_description: Some(
            "Launch and control UI4-backed GPGPU previews, SVG outline probes, and buffer-frame tests.",
        ),
