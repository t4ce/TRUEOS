...
mod tests {
    ...
    #[test]
    fn cpp_tool_schema_advertises_font_rush_start_and_stop() {
        assert!(TOOL_JSON_CPP.contains(
            "\"font_action\":{\"type\":\"string\",\"enum\":[\"stamp\",\"present\",\"rush\",\"status\",\"release\"]"
        ));
        assert!(
            TOOL_JSON_CPP
                .contains("\"rush_action\":{\"type\":\"string\",\"enum\":[\"start\",\"stop\"]")
        );
        assert!(command_registry_json().contains("shared-slot Unicode glyph rush"));
    }
}
