...
const TOOL_JSON_GPGPU: &str = r#"{"type":"object","properties":{"subcommand":{"type":"string","enum":["preview","probe"],"description":"GPGPU command family."},"action":{"type":"string","enum":["start","status","stop"],"description":"Preview lifecycle action."},"preset":{"type":"string","enum":["all","static","static30","mandelbrot","chart","plasma"],"description":"Preview to launch. All launches Mandelbrot, chart, and plasma together; static30 opens the 30-frame UI4 buffer test."},"probe":{"type":"string","enum":["font-tessel"],"description":"Diagnostic probe family."},"stage":{"type":"string","enum":["artifact","audit","flatten","mesh","all"],"description":"Font-tessellation diagnostic stage."},"duration_ms":{"type":"integer","minimum":0,"description":"Preview lifetime in milliseconds; zero runs until stopped."},"cadence_ms":{"type":"integer","minimum":1,"maximum":60000,"description":"Target compute-launch cadence in milliseconds."},"publish_every":{"type":"integer","minimum":1,"maximum":1024,"description":"Publish every Nth completed compute frame."}},"required":["subcommand"],"additionalProperties":false}"#;
...
const BUILTIN_CMD_REGISTRY: &[BuiltinShell2CmdEntry] = &[
    ...
    BuiltinShell2CmdEntry {
        ...
        tool_description: Some(
            "Launch and control UI4-backed GPGPU previews and buffer-frame tests.",
        ),
