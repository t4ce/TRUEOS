...
const TOOL_JSON_VGPU: &str = r#"{"type":"object","properties":{"command":{"type":"string","enum":["status","test","material"],"description":"Inspect the vGPU broker, run a runtime test, or select a Picasso material diagnostic view."},"test":{"type":"string","enum":["broker","abi","guc","compute","blit","all"],"description":"Runtime test selected when command=test."},"view":{"type":"string","enum":["pbr","base","normal","uv","solid"],"description":"Next-frame Picasso output selected when command=material; pbr restores ordinary shading."}},"required":["command"],"additionalProperties":false}"#;
