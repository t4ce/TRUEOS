...
const TOOL_JSON_HELIO: &str = r#"{"type":"object","properties":{"action":{"type":"string","enum":["start","list","status","monitor","perf","logger"],"description":"Launch, list, or inspect embedded Helio examples, or control Helio's temporary Spirit GPU logger (monitor, perf, and logger are aliases)."},"id":{"type":"integer","minimum":1,"maximum":4,"description":"Example id; 1 is simple-cube, 2 is churn-benchmark, 3 is shape-battle-royale, and 4 is pendulum-bigcloth."},"monitor_action":{"type":"string","enum":["start","status","off"],"description":"Start, inspect, or stop the Spirit 256x256 direct GPU logger."},"seconds":{"type":"integer","minimum":1,"maximum":300,"description":"Temporary logger lifetime in seconds; defaults to 30 and auto-restores Spirit afterward."}},"required":[],"additionalProperties":false}"#;
...
const BUILTIN_CMD_REGISTRY: &[BuiltinShell2CmdEntry] = &[
    ...
    BuiltinShell2CmdEntry {
        ...
        tool_description: Some(
            "Launch numbered build-produced Helio examples through Render/GuC and UI4, inspect their state, or temporarily show low-level stats through Spirit's 256x256 direct GPU logger while bypassing UI4/composition.",
        ),
...
mod tests {
    ...
    fn helio_is_advertised_as_a_launchable_runtime_use_case() {
        ...
        assert!(registry.contains("Launch numbered build-produced Helio examples"));
