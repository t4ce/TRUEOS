...
const TOOL_JSON_GPGPU: &str = r#"{"type":"object","properties":{"subcommand":{"type":"string","enum":["preview","probe","chart","pixel","artifacts"],"description":"GPGPU command family."},"action":{"type":"string","enum":["start","status","stop","probe","reload"],"description":"Preview lifecycle or artifact action. Preview start launches Mandelbrot, chart, and plasma together in dedicated UI4 broker frames."},"probe":{"type":"string","enum":["font-tessel"],"description":"Diagnostic probe family."},"stage":{"type":"string","enum":["artifact","audit","flatten","mesh","all"],"description":"Artifact or font-tessellation diagnostic stage."},"duration_ms":{"type":"integer","minimum":0,"description":"Preview lifetime in milliseconds; zero runs until stopped."},"cadence_ms":{"type":"integer","minimum":1,"maximum":60000,"description":"Target compute-launch cadence in milliseconds for all preview members."},"publish_every":{"type":"integer","minimum":1,"maximum":1024,"description":"Publish every Nth completed compute frame per preview member."},"kernel":{"type":"string","description":"Known kernel name, video alias, or all for artifact actions."}},"required":["subcommand"],"additionalProperties":false}"#;
...
const TOOL_JSON_VID: &str = r#"{"type":"object","properties":{"source":{"type":"string","enum":["embedded","online"],"description":"Use the embedded validated Annex-B asset or download the fixed online AVC1 MP4 asset."},"loop":{"type":"boolean","description":"Repeat playback while retaining the same UI4 Frame and window lifetime."}},"required":[],"additionalProperties":false}"#;
...
const BUILTIN_CMD_REGISTRY: &[BuiltinShell2CmdEntry] = &[
    ...
    BuiltinShell2CmdEntry {
        ...
        tool_description: Some(
            "Play the embedded or fixed online H.264 asset through VDBOX and the UI4 double-Frame path.",
        ),
