...
const TOOL_JSON_GPGPU: &str = r#"{"type":"object","properties":{"subcommand":{"type":"string","enum":["preview","probe","chart","pixel","artifacts"],"description":"GPGPU command family."},"action":{"type":"string","enum":["start","status","stop","reload"],"description":"Preview lifecycle or artifact action."},"preset":{"type":"string","enum":["mandelbrot","chart","plasma"],"description":"UI4-backed live-preview compute node."},"probe":{"type":"string","enum":["font-tessel"],"description":"Diagnostic probe family."},"stage":{"type":"string","enum":["artifact","audit","flatten","mesh","all"],"description":"Artifact or font-tessellation diagnostic stage."},"duration_ms":{"type":"integer","minimum":0,"description":"Preview lifetime in milliseconds; zero runs until stopped."},"cadence_ms":{"type":"integer","minimum":1,"maximum":60000,"description":"Target compute-launch cadence in milliseconds."},"publish_every":{"type":"integer","minimum":1,"maximum":1024,"description":"Publish every Nth completed compute frame to UI4."},"kernel":{"type":"string","description":"Known kernel name or all for artifact reload."}},"required":["subcommand"],"additionalProperties":false}"#;
...
const TOOL_JSON_VID: &str = r#"{"type":"object","properties":{"fps":{"type":"integer","minimum":1,"maximum":144,"description":"Playback speed in frames per second."},"path":{"type":"string","description":"Optional TRUEOSFS path to a raw Annex-B H.264 or AVC/H.264 MP4 video. Omit it for the built-in local asset."},"source":{"type":"string","enum":["trueosfs","online"],"description":"Read path from TRUEOSFS or play the built-in online MP4 probe."},"loop":{"type":"boolean","description":"Repeat playback until the video task is replaced."},"direction":{"type":"string","enum":["forward","reverse"],"description":"Forward only or chained forward then reverse playback."},"cache":{"type":"string","enum":["off","tail","full"],"description":"Decoded-frame cache strategy for reverse playback experiments. Default is off, including reverse."},"alpha":{"type":"integer","minimum":0,"maximum":255,"description":"Decoded video overlay plane alpha."},"study":{"type":"boolean","description":"Write stripe-study artifacts from the full decoded cache."},"fill":{"type":"boolean","description":"Show frames while filling reverse GOP cache."},"warm":{"type":"boolean","description":"Use noreset-lite decode mode. Default is true."}},"required":["fps"],"additionalProperties":false}"#;
