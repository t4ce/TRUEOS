...
const TOOL_JSON_FONT: &str = r#"{"type":"object","properties":{"text":{"type":"string","minLength":1,"maxLength":256,"description":"One UTF-8 row to tessellate and present."},"rows":{"type":"array","minItems":1,"items":{"type":"string","minLength":1},"description":"Explicit UTF-8 row groups for one combined draw; total characters are limited to 256."},"demo":{"type":"integer","minimum":1,"maximum":9,"description":"Built-in persistent-font scene and restrained animation preset."},"font_id":{"type":"integer","minimum":1,"maximum":2,"description":"Embedded font: 1=L_10646, 2=Noto Sans SC."},"scale":{"type":"integer","minimum":1,"maximum":8,"description":"Native post-tessellation render scale; defaults to 5."},"persist":{"type":"boolean","description":"Keep the indexed geometry resident in the render PPGTT."},"color":{"type":"string","pattern":"^[0-9A-Fa-f]{8}$","description":"Static persistent-font color as RRGGBBAA."},"from":{"type":"string","pattern":"^[0-9A-Fa-f]{8}$","description":"Transition start color as RRGGBBAA."},"to":{"type":"string","pattern":"^[0-9A-Fa-f]{8}$","description":"Transition end color as RRGGBBAA."},"channels":{"type":"string","enum":["r","g","b","a","rgb","rgba"],"description":"Color components changed by the transition."},"duration_ms":{"type":"integer","minimum":16,"maximum":600000,"description":"Monotonic transition duration in milliseconds."},"timing":{"type":"string","enum":["linear","sine"],"description":"Transition timing function."},"iteration":{"type":"string","enum":["once","loop","alternate"],"description":"Transition iteration policy."}},"oneOf":[{"required":["text"]},{"required":["rows"]},{"required":["demo"]}],"additionalProperties":false}"#;
...
fn dispatch_font(spawner: &Spawner, io: &'static dyn ShellBackend2, rest: &str) -> ParseOutcome {
    super::cmds::font::try_parse(spawner, io, rest)
}
...
const BUILTIN_CMD_REGISTRY: &[BuiltinShell2CmdEntry] = &[
    ...
    BuiltinShell2CmdEntry {
        ...
        tool_description: Some(
            "Tessellate UTF-8 text from warmed font outlines and present one grouped draw.",
        ),
