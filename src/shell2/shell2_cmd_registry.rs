...
const TOOL_JSON_GPGPU: &str = r#"{"type":"object","properties":{"subcommand":{"type":"string","enum":["status","walkrow","tilewalker","rowburst","variants"],"description":"Debug/status subcommand. Omit for normal row painting; use variants for compact artifact recipe sweep."},"row":{"type":"integer","minimum":1,"maximum":1440,"description":"1-based scanout row. Normal shell form is `gpgpu ROW`."},"rows":{"type":"integer","minimum":1,"maximum":1440,"description":"Optional row count. Defaults to 5 for normal row painting."},"x":{"type":"integer","minimum":0,"description":"Optional scanout x coordinate for debug commands."},"color":{"type":"integer","minimum":0,"maximum":16777215,"description":"Debug-only 24-bit RGB scanout color as 0xRRGGBB."},"stamps":{"oneOf":[{"type":"integer","minimum":1,"maximum":512},{"type":"string","enum":["full","row","all","max"]}],"description":"Optional walkrow/tilewalker stamp count, or full to stamp to the scanout edge. Defaults to 33."},"bands":{"oneOf":[{"type":"integer","minimum":1,"maximum":8},{"type":"string","enum":["full","row","all","max"]}],"description":"Optional rowburst band count. Defaults to 1."},"mode":{"type":"string","enum":["strict","loose","repair2","repair4","repair8","repair16","repair32","raw","raw-loose"],"description":"rowburst debug mode. strict/loose use one chunkstamp pass, repair modes add shifted chunkstamp passes, raw modes use the experimental artifact."},"verify":{"type":"boolean","description":"When true, poison and read back each stamp. Defaults to false for speed."}},"required":[],"additionalProperties":false}"#;
