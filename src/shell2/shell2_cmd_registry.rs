...
const TOOL_JSON_FONT: &str = r#"{"type":"object","properties":{"text":{"type":"string","minLength":1,"maxLength":256,"description":"One UTF-8 row tessellated locally and added to the Draw3D TCP scene."},"rows":{"type":"array","minItems":1,"items":{"type":"string","minLength":1},"description":"Explicit UTF-8 row groups for one scene mesh; total characters are limited to 256."},"demo":{"type":"integer","minimum":1,"maximum":9,"description":"Built-in font scene and restrained animation preset."},"font_id":{"type":"integer","minimum":1,"maximum":2,"description":"Embedded font: 1=L_10646, 2=Noto Sans SC."},"scale":{"type":"integer","minimum":1,"maximum":8,"description":"Native post-tessellation render scale; defaults to 5."},"color":{"type":"string","pattern":"^[0-9A-Fa-f]{8}$","description":"Static scene-mesh color as RRGGBBAA."},"from":{"type":"string","pattern":"^[0-9A-Fa-f]{8}$","description":"Transition start color as RRGGBBAA."},"to":{"type":"string","pattern":"^[0-9A-Fa-f]{8}$","description":"Transition end color as RRGGBBAA."},"channels":{"type":"string","enum":["r","g","b","a","rgb","rgba"],"description":"Color components changed by the transition."},"duration_ms":{"type":"integer","minimum":16,"maximum":600000,"description":"Monotonic transition duration in milliseconds."},"timing":{"type":"string","enum":["linear","sine"],"description":"Transition timing function."},"iteration":{"type":"string","enum":["once","loop","alternate"],"description":"Transition iteration policy."}},"oneOf":[{"required":["text"]},{"required":["rows"]},{"required":["demo"]}],"additionalProperties":false}"#;
...
const BUILTIN_CMD_REGISTRY: &[BuiltinShell2CmdEntry] = &[
    ...
    BuiltinShell2CmdEntry {
        ...
        tool_description: Some("Tessellate UTF-8 text and manage it as Draw3D TCP scene meshes."),
