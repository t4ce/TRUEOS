...
const TOOL_JSON_GPGPU: &str = r#"{"type":"object","properties":{"subcommand":{"type":"string","enum":["status","walkrow"],"description":"GPGPU command to run."},"row":{"type":"integer","minimum":1,"maximum":1440,"description":"1-based scanout row for walkrow."},"x":{"type":"integer","minimum":0,"description":"Optional scanout x coordinate for the 16-pixel stamp."},"color":{"type":"integer","description":"Optional 32-bit pixel color for the SIMD16 scanout store. Defaults to white."}},"required":["subcommand"],"additionalProperties":false}"#;
