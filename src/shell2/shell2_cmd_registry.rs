...
const TOOL_JSON_FONT: &str = r#"{"type":"object","properties":{"text":{"type":"string","minLength":1,"maxLength":256,"description":"One UTF-8 row to tessellate and present."},"rows":{"type":"array","minItems":1,"items":{"type":"string","minLength":1},"description":"Explicit UTF-8 row groups for one combined draw; total characters are limited to 256."},"scale":{"type":"integer","minimum":1,"maximum":8,"description":"Native post-tessellation render scale; defaults to 5."}},"oneOf":[{"required":["text"]},{"required":["rows"]}],"additionalProperties":false}"#;
...
const BUILTIN_CMD_REGISTRY: &[BuiltinShell2CmdEntry] = &[
    ...
    BuiltinShell2CmdEntry {
        ...
        tool_description: Some("Tessellate UTF-8 text from warmed font outlines and present one grouped draw."),
