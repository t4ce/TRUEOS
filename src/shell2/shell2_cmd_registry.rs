...
const TOOL_JSON_GPGPU: &str = r#"{"type":"object","properties":{"subcommand":{"type":"string","enum":["preview","probe","chart","pixel","artifacts"],"description":"GPGPU command family."},"action":{"type":"string","enum":["start","status","stop","reload"],"description":"Preview lifecycle or artifact action."},"preset":{"type":"string","enum":["mandelbrot"],"description":"UI4-backed live-preview preset."},"probe":{"type":"string","enum":["font-tessel"],"description":"Diagnostic probe family."},"stage":{"type":"string","enum":["artifact","audit","flatten","mesh","all"],"description":"Artifact or font-tessellation diagnostic stage."},"duration_ms":{"type":"integer","minimum":0,"description":"Preview lifetime in milliseconds; zero runs until stopped."},"cadence_ms":{"type":"integer","minimum":1,"maximum":60000,"description":"Target compute-launch cadence in milliseconds."},"publish_every":{"type":"integer","minimum":1,"maximum":1024,"description":"Publish every Nth completed compute frame to UI4."},"kernel":{"type":"string","description":"Known kernel name or all for artifact reload."}},"required":["subcommand"],"additionalProperties":false}"#;
