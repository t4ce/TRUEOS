...
const TOOL_JSON_GPGPU: &str = r#"{"type":"object","properties":{"subcommand":{"type":"string","enum":["status","walkrow","tilewalker"],"description":"GPGPU command to run."},"row":{"type":"integer","minimum":1,"maximum":1440,"description":"1-based scanout row for walkrow or tilewalker start row."},"rows":{"type":"integer","minimum":1,"maximum":1440,"description":"Optional tilewalker row count. Defaults to 16."},"x":{"type":"integer","minimum":0,"description":"Optional scanout x coordinate for the first 16-pixel stamp."},"color":{"type":"integer","description":"Optional 32-bit pixel color for the SIMD16 scanout store. Defaults to white."},"stamps":{"type":"integer","minimum":1,"maximum":256,"description":"Optional number of 16-pixel stamps per row. Defaults to 33."},"verify":{"type":"boolean","description":"When true, poison and read back each stamp. Defaults to false for speed."}},"required":["subcommand"],"additionalProperties":false}"#;
