...
fn print_intro(target: &MatrixTarget) {
    print_matrix_target_line(target, "Ample 0: text mode boot");
    print_matrix_target_line(target, "You are standing in a clean white test chamber.");
...
fn print_startup_menu(io: &'static dyn ShellBackend2) {
    print_shell_line(io, "Choose a world or save:");
    print_shell_line(io, "Worlds");
    print_shell_line(
        io,
        "  1. AMBLE: An Absurd Adventure (by Dave, v0.67.0-alpha)",
    );
    print_shell_line(
        io,
        "      A surreal sci-fi comedy adventure about cake, curiosity,",
    );
    print_shell_line(io, "      and improbable facilities.");
    print_shell_line(io, "  2. AMBLE: An Absurd Adventure (by Dave, v0.66.0)");
    print_shell_line(
        io,
        "      A surreal sci-fi comedy adventure about cake, curiosity,",
    );
    print_shell_line(io, "      and improbable facilities.");
    print_shell_line(io, "  3. Hospital Game TBD (by pygmy-twylyte, v0.1.0)");
    print_shell_line(
        io,
        "      A mystery game set in a nearly abandoned small hospital.",
    );
    print_shell_line(io, "");
    print_shell_line(io, "Select a world or save [1-3] (Enter=1): ");
}
        ...
        AmplePhase::Selecting => {
            ...
            match selected {
                ...
                _ => {
                    ...
                    print_matrix_target_line(target, "Select a world or save [1-3] (Enter=1): ");
                    CommandSessionInputResult::KeepRunning
                }
            }
        }
...
fn handle_game_input(target: &MatrixTarget, submitted: &str) -> CommandSessionInputResult {
    let trimmed = submitted.trim();
    if trimmed.is_empty() {
        print_matrix_target_line(target, "ample: type `help`");
        ...
        || trimmed.eq_ignore_ascii_case("q")
    {
        ...
        print_matrix_target_line(target, "ample: session closed");
        return CommandSessionInputResult::CompleteIdle;
    }

    if trimmed.eq_ignore_ascii_case("help") || trimmed == "?" {
        print_matrix_target_line(target, "ample: available commands");
    ...
    if trimmed.eq_ignore_ascii_case("inventory") || trimmed.eq_ignore_ascii_case("inv") {
        print_matrix_target_line(target, "ample: inventory -> visitor badge, dry humor");
        return CommandSessionInputResult::KeepRunning;
    }
    ...
    if trimmed.eq_ignore_ascii_case("north") || trimmed.eq_ignore_ascii_case("go north") {
        ...
        print_matrix_target_line(target, "ample: maybe the badge reader wants attention.");
        return CommandSessionInputResult::KeepRunning;
    }

    if trimmed.eq_ignore_ascii_case("south") || trimmed.eq_ignore_ascii_case("go south") {
        print_matrix_target_line(target, "ample: you drift back toward the terminal bench.");
        return CommandSessionInputResult::KeepRunning;
    }

    if trimmed.eq_ignore_ascii_case("take badge") || trimmed.eq_ignore_ascii_case("get badge") {
        print_matrix_target_line(target, "ample: you pick up the visitor badge.");
        return CommandSessionInputResult::KeepRunning;
    }
