...
pub(crate) fn store(spawner: &Spawner, io: &'static dyn ShellBackend2) {
    ...
    match rapl_store_task(target.clone()) {
        ...
        Err(_) => {
            ...
            print_shell_line(io, "tlb rapl store: task unavailable");
        }
    }
}
...
async fn rapl_store_task(target: MatrixTarget) {
    match crate::power::rapl::store_history_to_trueosfs().await {
        ...
        Err(crate::power::rapl::RaplStoreError::RootNotMounted) => {
            print_matrix_target_line(&target, "tlb rapl store: TRUEOSFS root is not mounted")
        }
        Err(crate::power::rapl::RaplStoreError::RootUnavailable) => {
            print_matrix_target_line(&target, "tlb rapl store: TRUEOSFS root is unavailable")
        }
        Err(crate::power::rapl::RaplStoreError::NoSpaceOrFs) => {
            print_matrix_target_line(&target, "tlb rapl store: no space or filesystem unavailable")
        }
        Err(crate::power::rapl::RaplStoreError::Io(err)) => print_matrix_target_line(
            &target,
            alloc::format!("tlb rapl store: write failed: {:?}", err).as_str(),
        ),
    }
    set_matrix_target_active(&target, false);
}
