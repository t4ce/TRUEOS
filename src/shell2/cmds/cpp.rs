...
fn print_list(io: &'static dyn ShellBackend2) {
    ...
    print_shell_line(
        io,
        "cpp font rush: Lucida-only finite plane probe; after T/R/U/E/O/S and TrueOS it clears once, holds a minimum two-second blank, pulses § § §, then runs the terminal no-clear storm through raw prepared glyph producers",
    );
    print_shell_line(
        io,
