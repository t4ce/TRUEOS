...
fn queue_font_service_rush(io: &'static dyn ShellBackend2) {
    match crate::ui4::request_cpp_font_rush_start() {
        Ok(serial) => print_shell_line(
            io,
            alloc::format!(
                "cpp font rush: queued=1 request={} font=1 font_name=lucida-sans-unicode font_policy=pinned-warm-required base_cadence_ms=250 stage_ms=3000 stages=base-ranks-0..3+expanded-ranks-0..3-once+title-letters-T/R/U/E/O/S-150ms-each+TrueOS-tight-rgba-scaled-max-fit-hold-1000ms+transparent-clear-once+blank-min-2000ms+section-pulse-3x-100msx3000ms+raw-producer-storm showcase_sources=one-time-tight-white-rgba+primary-expanded-chargeup+run-owned+release-after-last-use showcase_path=transparent-clear->guc-font-rcs-scaled-tinted-sprite terminal_stage=raw-plan-pool-32 glyph_layout_base=1+2+4+16 glyph_layout_expanded=4+8+16+64 producer_storm=64-raw-planned-glyphs/32-producers/8x4/full-region-integer-anchors/two-diagonally-separated-anchors-each buffer_history=deterministic-mirrored-double clear_policy=one-transparent-blank+transparent-section+transparent-storm-prime/no-clear-storm backdrop=pipe-a-bottom-color pixel_alpha=premultiplied planes=ui4-display-capability-enumerated-then-primary-only consumer_pending_limit=1 service_model=plan-pool-32+font-fifo-32+one-font-context-in-flight final_path=prepared-raw-glyph-plan->font-r8-coverage-and-mask-batch->guc-font-rcs-ordered-source-over->ui4-rgba8->display-plane-direct final_planning=32-workers final_skrifa=plan-stage final_tessellation=plan-stage final_coverage=font-rcs final_shading=font-mask-batch compositor_jobs=0 rgba_cpu_readback=0 coverage_audit_cpu_readback=cache-miss-only duration=until-stopped stop=\"cpp font rush stop\"",
                serial,
            )
            .as_str(),
        ),
