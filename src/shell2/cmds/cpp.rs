...
fn print_list(io: &'static dyn ShellBackend2) {
    ...
    print_shell_line(
        io,
        "cpp font rush: Lucida-only finite plane probe; after T/R/U/E/O/S and TrueOS it clears once, charges four temporary 128-entry RGBA8 size atlases behind a minimum two-second blank, pulses § § §, then runs the terminal no-clear storm from cached pixels only",
    );
    print_shell_line(
        io,
...
fn queue_font_service_rush(io: &'static dyn ShellBackend2) {
    match crate::ui4::request_cpp_font_rush_start() {
        Ok(serial) => print_shell_line(
            io,
            alloc::format!(
                "cpp font rush: queued=1 request={} font=1 font_name=lucida-sans-unicode font_policy=pinned-warm-required base_cadence_ms=250 stage_ms=3000 stages=base-ranks-0..3+expanded-ranks-0..3-once+title-letters-T/R/U/E/O/S-150ms-each+TrueOS-tight-rgba-scaled-max-fit-hold-1000ms+transparent-clear-once+rgba8-charge-4sizesx4batchesx32glyphs-behind-blank+blank-min-2000ms+section-pulse-3x-100msx3000ms+producer-storm showcase_sources=one-time-tight-white-rgba+primary-expanded-chargeup+run-owned+release-after-last-use showcase_path=transparent-clear->guc-font-rcs-scaled-tinted-sprite terminal_stage=sealed-rgba8-cache-blast glyph_layout_base=1+2+4+16 glyph_layout_expanded=4+8+16+64 rgba8_cache=4x2048x1024/8MiB/128-fixed-tiles/size-specific/run-owned/no-eviction charge_workers=32 producer_storm=64-cached-blits/32-regions/8x4/full-region-integer-anchors/two-diagonally-separated-anchors-each buffer_history=deterministic-mirrored-double clear_policy=one-transparent-blank+transparent-section+transparent-storm-prime/no-clear-storm backdrop=pipe-a-bottom-color pixel_alpha=premultiplied planes=ui4-display-capability-enumerated-then-primary-only consumer_pending_limit=1 service_model=plan-pool-32-during-charge+font-fifo-32+one-font-context-in-flight final_path=sealed-premultiplied-rgba8->guc-font-rcs-ordered-source-over->ui4-rgba8->display-plane-direct final_planning=0 final_skrifa=0 final_tessellation=0 final_coverage=0 final_shading=0 compositor_jobs=0 rgba_cpu_readback=0 coverage_audit_cpu_readback=charge-miss-only duration=until-stopped stop=\"cpp font rush stop\"",
                serial,
            )
            .as_str(),
        ),
