...
fn print_list(io: &'static dyn ShellBackend2) {
    ...
    print_shell_line(
        io,
        "cpp font rush: Lucida-only finite plane probe; its title shows six isolated fullscreen letters T/R/U/E/O/S then a large exact TrueOS, charges four temporary 128-entry RGBA8 size atlases, seals them, then runs the terminal no-clear 32-producer storm from cached pixels only",
    );
    print_shell_line(
        io,
