...
fn entry_hint_offset(entry: u64) -> u32 {
    entry as u32
}
...
fn elf_type_name(bytes: &[u8]) -> Option<&'static str> {
...
fn unpack_blueprint(module: &BlueprintModule<'_>) -> Result<Vec<u8>, &'static str> {
    match module.flags {
...
async fn run_command_task(target: MatrixTarget, archive: String, app_args: Vec<String>) {
    ...
    async move {
        ...
        let module = match parse_blueprint(module_bytes.as_slice()) {
            Ok(module) => module,
            Err(err) => {
        ...
        let unpacked = match unpack_blueprint(&module) {
            Ok(bytes) => bytes,
            Err(err) => {
                log(alloc::format!("run: {}", err).as_str());
                return;
        ...
        if unpacked.starts_with(b"\x7fELF") {
            if let Some(kind) = elf_type_name(unpacked.as_slice()) {
            ...
            && matches!(elf_type_name(unpacked.as_slice()), Some("REL"))
        {
            match load_rel_image(unpacked.as_slice()) {
                Ok(image) => match parse_sections(unpacked.as_slice()).and_then(|sections| {
                    ...
                    Err(err) => {
                        log(alloc::format!("run: main lookup failed: {}", err).as_str());
                    }
                },
                Err(err) => {
                    log(alloc::format!("run: REL load failed: {}", err).as_str());
                }
            }
        }
    }
