...
async fn capture(spawner: Spawner, io: &'static dyn ShellBackend2) {
    ...
    let pixels = match crate::tga::camera::read_tile().await {
        Ok(p) => p,
        Err(e) => {
            print_shell_line(io, &alloc::format!("cam: {e}"));
            return;
        }
    };
    let png = match crate::graphics::encoder::png::encode_rgba8_png(64, 64, &pixels, 256) {
        Ok(p) => p,
        Err(e) => {
            print_shell_line(io, &alloc::format!("cam: PNG encode failed: {e:?}"));
            return;
        }
    };
    let Some(disk) = crate::r::fs::trueosfs::primary_root_handle() else {
        print_shell_line(io, "cam: no TRUEOSFS root");
        return;
    };
    let path = alloc::format!("camera/hdmi-tile-{}.png", crate::chronos::monotonic_nanos());
    match crate::r::fs::trueosfs::file_in_typed_async(disk, &path, &png, infer::ContentTypeId::PNG)
        .await
    {
        Ok(true) => {
            print_shell_line(
                io,
                &alloc::format!("cam: captured frozen 64x64 HDMI tile to trueosfs:/{path}"),
            );
            crate::log_important!(
                "cam: captured path={} bytes={} source=hdmi-rx tile=64x64\n",
                path,
                png.len()
            );
