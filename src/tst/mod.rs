...
pub(crate) async fn boot_gfx_virtio_sw_prepare_task() {
    ...
    async fn wait_for_gfx_ready_stable(min_epoch: u64, timeout_ms: u64, settle_ms: u64) -> bool {
        ...
        loop {
            let kind_ok = matches!(
                ...
                Some(crate::gfx::BackendKind::Virgl)
                    | Some(crate::gfx::BackendKind::VirtioSw)
            );
    ...
    crate::log!("qjs-gfx-prepare: waiting for stable gfx (virgl/virtio_sw)\n");
    let epoch0 = crate::gfx::backend_epoch();

    // First chance: if another path already selected a ready backend, wait for brief stability.
    if wait_for_gfx_ready_stable(epoch0, 1500, 250).await {
        crate::v::readiness::set(crate::v::readiness::GFX_VIRTIO_SW_READY);
        crate::log!("qjs-gfx-prepare: ready (backend already active)\n");
        return;
    }

    crate::log!("qjs-gfx-prepare: forcing gfx switch (prefer virgl)\n");
    let mut virgl_ok = false;
    for _ in 0..8 {
    ...
    if !virgl_ok {
        crate::log!("qjs-gfx-prepare: virgl unavailable; fallback to virtio_sw\n");
        if !crate::gfx::switch_to_virtio_sw() {
            crate::log!("qjs-gfx-prepare: failed (no backend switch succeeded)\n");
            return;
        }
    }

    let target_epoch = epoch0.saturating_add(1);
    if !wait_for_gfx_ready_stable(target_epoch, 2500, 300).await {
        crate::log!("qjs-gfx-prepare: failed (backend not stable)\n");
        return;
    }
    crate::v::readiness::set(crate::v::readiness::GFX_VIRTIO_SW_READY);
    crate::log!(
