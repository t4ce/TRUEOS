...
const SVG_SMOKE_TEX_ID_BASE: u32 = 2_000;

const SVG_FIXTURES: &[SvgFixture] = &[
    SvgFixture {
        name: "sunrise_layers",
        svg: r##"
...
fn run_inner() {
    for (index, fixture) in SVG_FIXTURES.iter().enumerate() {
        let tex_id = SVG_SMOKE_TEX_ID_BASE + index as u32;
        match crate::gfx::svg::upload_svg_text_to_texture(tex_id, fixture.svg) {
            Ok(info) => crate::log!(
                "usvg-smoke[{}]: helper-ok tex_id={} {}x{}\n",
                fixture.name,
                tex_id,
                info.width,
                info.height
            ...
            Err(err) => crate::log!(
                "usvg-smoke[{}]: helper-err tex_id={} rc={}\n",
                fixture.name,
                tex_id,
                err
            ),
