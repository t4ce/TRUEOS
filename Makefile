...
QEMU_SERIAL ?= tcp:127.0.0.1:5555,server,wait
QEMU_VFIO_HOST ?=
...
intel-gpu-bake-lz4-cpp:
	PYTHON="$(INTEL_GPU_BAKERY_PYTHON)" bash "$(INTEL_GPU_BAKERY_DIR)/bake_adls_cpp_lz4.sh"

intel-gpu-bake-cpp-artifacts:
	$(MAKE) --no-print-directory intel-gpu-bake-lz4-cpp
