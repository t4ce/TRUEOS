    ...
    S: DnsClientStream + Unpin + 'static,
{
    ...
    fn next_random_query_id(&self) -> Result<u16, ProtoError> {
        ...
        Err(ProtoError::from("id space exhausted, consider filing an issue"))
    }
...
mod test {
    ...
    impl Stream for MockClientStream {
        ...
        fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
            ...
            if let Some(mut message) = self.messages.pop() {
                message.set_id(id);
                Poll::Ready(Some(Ok(SerialMessage::new(message.to_bytes().unwrap(), self.addr))))
            } else {
                Poll::Pending
            }
        }
    ...
    fn a_query_answer() -> (DnsRequest, Vec<Message>) {
        ...
        msg.set_message_type(MessageType::Response).add_answer(
            Record::from_rdata(name, 86400, RData::A(Ipv4Addr::new(93, 184, 215, 14).into()))
                .set_dns_class(DNSClass::IN)
                .clone(),
        );
        (DnsRequest::new(query, DnsRequestOptions::default()), vec![msg])
    }
    ...
    fn axfr_response() -> Vec<Record> {
        ...
        vec![
            ...
            Record::from_rdata(
                ...
                86400,
                RData::AAAA(
                    Ipv6Addr::new(0x2606, 0x2800, 0x21f, 0xcb07, 0x6820, 0x80da, 0xaf6b, 0x8b2c)
                        .into(),
                ),
            )
            .set_dns_class(DNSClass::IN)
    ...
    fn axfr_query_answer_multi() -> (DnsRequest, Vec<Message>) {
        ...
        msg2.set_message_type(MessageType::Response)
            .insert_answers(rr2);
        (DnsRequest::new(query, DnsRequestOptions::default()), vec![msg1, msg2])
    }
