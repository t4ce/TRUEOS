...
mod tests {
    ...
    fn get_data() -> Vec<(Name, Vec<u8>)> {
        vec![
            ...
            (Name::from_str("a.bc.").unwrap(), vec![1, b'a', 2, b'b', b'c', 0]), // two labels, 'a.bc'
            (
    ...
    fn test_partial_cmp() {
        let comparisons: Vec<(Name, Name)> = vec![
            (Name::from_str("example.").unwrap(), Name::from_str("a.example.").unwrap()),
            (Name::from_str("a.example.").unwrap(), Name::from_str("yljkjljk.a.example.").unwrap()),
            (
            ...
            (Name::from_ascii("zABC.a.EXAMPLE.").unwrap(), Name::from_str("z.example.").unwrap()),
            (
    ...
    fn test_cmp_ignore_case() {
        let comparisons: Vec<(Name, Name)> = vec![
            (Name::from_ascii("ExAmPle.").unwrap(), Name::from_ascii("example.").unwrap()),
            (Name::from_ascii("A.example.").unwrap(), Name::from_ascii("a.example.").unwrap()),
        ];
    ...
    fn test_from_str() {
        ...
        assert_eq!(Name::from_str(".").unwrap(), Name::from_labels(Vec::<&str>::new()).unwrap());
    }
    ...
    #[test]
    fn test_to_string() {
        assert_eq!(Name::from_str("www.example.com.").unwrap().to_string(), "www.example.com.");
        assert_eq!(Name::from_str("www.example.com").unwrap().to_string(), "www.example.com");
    }
    ...
    fn test_length_limits() {
        ...
        assert!(matches!(long_label_error.kind(), ProtoErrorKind::LabelBytesTooLong(64)));
        let long_name_error =
            Name::parse(&format!("a.{}", &expected_name_str[1..]), None).unwrap_err();
        assert!(matches!(long_name_error.kind(), ProtoErrorKind::DomainNameTooLong(256)))
    }
