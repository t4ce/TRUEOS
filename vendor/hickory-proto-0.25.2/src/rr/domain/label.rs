...
mod tests {
    ...
    fn test_partial_cmp() {
        let comparisons: Vec<(Label, Label)> = vec![
            (Label::from_raw_bytes(b"yljkjljk").unwrap(), Label::from_raw_bytes(b"Z").unwrap()),
            (Label::from_raw_bytes(b"Z").unwrap(), Label::from_raw_bytes(b"zABC").unwrap()),
            (Label::from_raw_bytes(&[1]).unwrap(), Label::from_raw_bytes(b"*").unwrap()),
            (Label::from_raw_bytes(b"*").unwrap(), Label::from_raw_bytes(&[200]).unwrap()),
        ];
