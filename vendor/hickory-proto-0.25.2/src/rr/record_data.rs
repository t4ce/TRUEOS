...
mod tests {
    ...
    fn get_data() -> Vec<(RData, Vec<u8>)> {
        vec![
            ...
            (RData::MX(MX::new(256, Name::from_str("n.").unwrap())), vec![1, 0, 1, b'n', 0]),
            (
            ...
            (
                RData::SRV(SRV::new(1, 2, 3, Name::from_str("www.example.com.").unwrap())),
                vec![
    ...
    fn test_read() {
        for (_test_pass, (expect, binary)) in get_data().into_iter().enumerate() {
            ...
            assert_eq!(
                RData::read(&mut decoder, record_type_from_rdata(&expect), Restrict::new(length))
                    .unwrap(),
                expect
            );
        }
