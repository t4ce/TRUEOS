...
impl fmt::Debug for EchConfigList {
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> Result<(), fmt::Error> {
        write!(f, "\"EchConfig ({})\"", data_encoding::BASE64.encode(&self.0))
    }
}
...
mod tests {
    ...
    fn test_encode_decode_svcb() {
        test_encode_decode(SVCB::new(0, Name::from_utf8("www.example.com.").unwrap(), vec![]));
    ...
    fn test_unrestricted_output_size() {
        let svcb = SVCB::new(
            8224,
            Name::from_utf8(".").unwrap(),
            vec![(SvcParamKey::Unknown(8224), SvcParamValue::Unknown(Unknown(vec![32; 257])))],
        );
    ...
    fn test_unknown_value_round_trip() {
        let svcb = SVCB::new(
            8224,
            Name::from_utf8(".").unwrap(),
            vec![(SvcParamKey::Unknown(8224), SvcParamValue::Unknown(Unknown(vec![32; 10])))],
        );
        ...
        let decoded =
            SVCB::read_data(&mut decoder, Restrict::new(u16::try_from(buf.len()).unwrap()))
                .unwrap();
