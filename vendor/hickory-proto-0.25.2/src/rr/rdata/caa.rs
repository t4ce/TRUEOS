...
fn emit_tag(buf: &mut [u8], tag: &[u8]) -> ProtoResult<u8> {
    ...
    if buf.len() < len {
        return Err(
            format!("insufficient capacity in CAA buffer: {} for tag: {}", buf.len(), len).into()
        );
    }
...
mod tests {
    ...
    fn test_from_str_property() {
        ...
        assert_eq!(Property::from("unknown".to_string()), Property::Unknown("unknown".to_string()));
    }
    ...
    fn test_encode_decode_iodef() {
        test_encode_decode(CAA::new_iodef(true, Url::parse("https://www.example.com").unwrap()));
    ...
    fn test_encode_non_fqdn() {
        ...
        test_encode(
            CAA::new_issue(true, Some(Name::parse("example.com", None).unwrap()), vec![]),
            &encoded,
        );
    }
    ...
    fn test_to_string() {
        ...
        let empty_options =
            CAA::new_issue(false, Some(Name::parse("example.com", None).unwrap()), vec![]);
        ...
        assert_eq!(two_options.to_string(), "0 issue \"example.com; one=1; two=2\"");
        ...
        assert_eq!(flag_set.to_string(), "128 issue \"example.com; one=1; two=2\"");

        let empty_domain =
            CAA::new_issue(false, None, vec![KeyValue::new("one", "1"), KeyValue::new("two", "2")]);
