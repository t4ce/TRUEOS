...
mod tests {
    ...
    fn test_read_empty_option_at_end_of_opt() {
        ...
        assert!(read_rdata.is_ok(), "error decoding: {:?}", read_rdata.unwrap_err());

        let opt = read_rdata.unwrap();
        let options = vec![
            (EdnsCode::Subnet, EdnsOption::Subnet("0.0.0.0/0".parse().unwrap())),
            (
                EdnsCode::Cookie,
    ...
    fn test_multiple_options_with_same_code() {
        ...
        let opt = read_rdata.unwrap();
        let options = vec![
            (EdnsCode::Unknown(15u16), EdnsOption::Unknown(15u16, vec![0x00, 0x06])),
            (
