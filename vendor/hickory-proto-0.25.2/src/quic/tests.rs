...
async fn test_quic_stream() {
    ...
    let quic_ns =
        QuicServer::new(SocketAddr::from(([127, 0, 0, 1], 0)), Arc::new(certificate_and_key))
            .await
    ...
    message.add_query(Query::query(Name::from_str("www.example.test.").unwrap(), RecordType::AAAA));
