...
pub(crate) fn parse<'i>(mut tokens: impl Iterator<Item = &'i str>) -> ParseResult<DNSKEY> {
    ...
    if protocol != 3 {
        return Err(ParseError::from(ParseErrorKind::Message("protocol field must be 3")));
    }
    ...
    if public_key_str.is_empty() {
        return Err(ParseError::from(ParseErrorKind::Message("public key not present")));
    }

    let public_key = data_encoding::BASE64.decode(public_key_str.as_bytes())?;

    Ok(DNSKEY::with_flags(flags, PublicKeyBuf::new(public_key, algorithm)))
}
