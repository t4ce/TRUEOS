...
pub(crate) fn parse<'i, I: Iterator<Item = &'i str>>(mut tokens: I) -> ParseResult<SVCB> {
    ...
    for token in tokens {
        ...
        let key = key_value.next().ok_or_else(|| {
            ParseError::from(ParseErrorKind::MissingToken("SVCB SvcbParams missing".to_string()))
        })?;
...
fn parse_char_data(value: &str) -> Result<String, ParseError> {
    ...
    match ch_data {
        ...
        _ => Err(ParseError::from(ParseErrorKind::Message("expected character data"))),
    }
}
...
fn parse_mandatory(value: Option<&str>) -> Result<SvcParamValue, ParseError> {
    let value = value.ok_or_else(|| {
        ParseError::from(ParseErrorKind::Message("expected at least one Mandatory field"))
    })?;
...
fn parse_ech_config(value: Option<&str>) -> Result<SvcParamValue, ParseError> {
    let value = value.ok_or_else(|| {
        ParseError::from(ParseErrorKind::Message("expected a base64 encoded string for EchConfig"))
    })?;

    let value = parse_char_data(value)?;
    let ech_config_bytes = data_encoding::BASE64.decode(value.as_bytes())?;
    Ok(SvcParamValue::EchConfigList(EchConfigList(ech_config_bytes)))
}
