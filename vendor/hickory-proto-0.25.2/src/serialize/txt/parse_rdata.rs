    ...
    fn parse<'i, I: Iterator<Item = &'i str>>(
        ...
        let rdata = match record_type {
            ...
            RecordType::NSEC3PARAM => {
                return Err(ParseError::from("NSEC3PARAM should be dynamically generated"));
            }
            RecordType::RRSIG => {
...
mod tests {
    ...
    fn test_aaaa() {
        ...
        let name = Name::from_str("example.com.").unwrap();
        let record =
            RData::parse(RecordType::AAAA, tokens.iter().map(AsRef::as_ref), Some(&name)).unwrap();
    ...
    fn test_ns_parse() {
        ...
        assert_eq!(record, RData::NS(NS(Name::from_str("ns.example.com").unwrap())));
    }
    ...
    fn test_csync() {
        ...
        let name = Name::from_str("example.com.").unwrap();
        let record =
            RData::parse(RecordType::CSYNC, tokens.iter().map(AsRef::as_ref), Some(&name)).unwrap();

        assert_eq!(
            record,
            RData::CSYNC(CSYNC::new(123, true, false, [RecordType::A, RecordType::NS]))
        );
    }
    ...
    fn test_ds() {
        ...
        let record =
            RData::parse(RecordType::DS, tokens.iter().map(AsRef::as_ref), Some(&name)).unwrap();
    ...
    fn test_any() {
        let tokens = ["test"];
        let name = Name::from_str("example.com.").unwrap();
        let result = RData::parse(RecordType::ANY, tokens.iter().map(AsRef::as_ref), Some(&name));
