    ...
    C: FnMut(&mut [f32]) + Send + 'static,
{
    ...
    {
        return Ok(OutputDevice::new(directsound::DirectSoundDevice::new(params, data_callback)?));
    }

    #[cfg(target_os = "android")]
    {
        return Ok(OutputDevice::new(aaudio::AAudioOutputDevice::new(params, data_callback)?));
    }
    ...
    #[cfg(target_os = "linux")]
    {
        #[cfg(feature = "alsa")]
        {
            return Ok(OutputDevice::new(alsa::AlsaSoundDevice::new(params, data_callback)?));
        }

        #[cfg(all(feature = "pulse", not(feature = "alsa")))]
        {
            return Ok(OutputDevice::new(pulse::PulseSoundDevice::new(params, data_callback)?));
        }
    ...
    #[cfg(all(target_os = "unknown", target_arch = "wasm32"))]
    {
        return Ok(OutputDevice::new(web::WebAudioDevice::new(params, data_callback)?));
    }

    #[cfg(any(target_os = "macos", target_os = "ios"))]
    {
        return Ok(OutputDevice::new(coreaudio::CoreaudioSoundDevice::new(params, data_callback)?));
    }

    #[cfg(target_os = "trueos")]
    {
        return Ok(OutputDevice::new(trueos::TrueOsAudioDevice::new(params, data_callback)?));
    }
