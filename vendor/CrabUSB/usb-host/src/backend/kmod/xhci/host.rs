...
impl EventHandler {
    ...
    fn clean_event_ring(&self) -> Event {
        ...
        while let Some(allowed) = self.event_ring().next() {
            match allowed {
                ...
                Allowed::TransferEvent(c) => {
                    ...
                    if matches!(code, Ok(CompletionCode::Success | CompletionCode::ShortPacket)) {
                        if let Some(seen) = TRANSFER_EVENT_TRACE.sample() {
                            trace!(
                                "xhci: event transfer seen={} slot={} ep={} ptr={:#x} code={:?} remaining={} event_data={}",
                                seen,
                                slot_id,
                                ep_id,
                                ptr,
                        ...
                        warn!(
                            "xhci: abnormal transfer completion slot={} ep={} ptr={:#x} code={:?} remaining={} event_data={}",
                            slot_id,
                            ep_id,
                            ptr,
                            code,
        ...
        if command_events + port_events + transfer_events + other_events > 0 {
            let interesting = command_events + port_events + other_events > 0;
            let sample = EVENT_DRAIN_TRACE.sample();
            if interesting || sample.is_some() {
                trace!(
                    "xhci: event ring drained seen={} command={} port={} transfer={} other={} erdp={:#x}",
                    sample.unwrap_or_else(|| EVENT_DRAIN_TRACE.total()),
                    command_events,
                    port_events,
                    transfer_events,
                    other_events,
