...
use super::{DirectionExt, TraceSampler, reg::SlotBell, ring::SendRing, transfer::TransferId};
use crate::{
    BusAddr,
    backend::{
        Dci,
...
static TRANSFER_LENGTH_TRACE: TraceSampler = TraceSampler::new();
...
impl Endpoint {
    ...
    fn transfer_from_completion(
        ...
        if let SubmittedTdKind::Iso { packets } = &submitted.kind {
            ...
            if let Some(seen) = TRANSFER_LENGTH_TRACE.sample() {
                trace!(
                    "xhci: transfer complete kind=iso seen={} bytes={}",
                    seen, transfer.transfer_len
                );
            }
            return Ok(transfer);
        }
        ...
        if let Some(seen) = TRANSFER_LENGTH_TRACE.sample() {
            trace!(
                "xhci: transfer complete kind=normal seen={} bytes={}",
                seen, transfer.transfer_len
            );
        }
        Ok(transfer)
