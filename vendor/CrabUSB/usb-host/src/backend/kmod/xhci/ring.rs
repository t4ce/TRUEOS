...
static TRANSFER_ENQUEUE_TRACE: TraceSampler = TraceSampler::new();
...
impl Ring {
    ...
    pub fn enque_command(&mut self, mut trb: command::Allowed) -> BusAddr {
        ...
        trace!("xhci: command enqueue trb={trb:X?} addr={addr:X?}");
        addr
    }
    ...
    pub fn enque_transfer(&mut self, mut trb: transfer::Allowed) -> BusAddr {
        ...
        let addr = self.enque_trb(trb.into());
        if let Some(seen) = TRANSFER_ENQUEUE_TRACE.sample() {
            trace!("xhci: transfer enqueue seen={} trb={trb:X?} addr={addr:X?}", seen);
        }
        addr
    }
    ...
    fn enque_transfer_deferred_cycle(
        ...
        if let Some(seen) = TRANSFER_ENQUEUE_TRACE.sample() {
            trace!(
                "xhci: transfer enqueue deferred=true seen={} trb={visible_trb:X?} addr={addr:X?}",
                seen
            );
        }
