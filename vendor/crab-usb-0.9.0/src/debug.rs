...
#[derive(Clone, Copy, Debug, Default)]
pub struct DebugUsbTelemetrySnapshot {
    pub sequence: u64,
    pub last_submit: DebugLastSubmit,
    pub last_event: DebugLastEvent,
    pub last_stream_config: DebugLastStreamConfig,
    pub probe_progress: DebugUsbProbeProgress,
}

static DEBUG_SEQUENCE: AtomicU64 = AtomicU64::new(0);
...
fn bump_sequence() {
    DEBUG_SEQUENCE.fetch_add(1, Ordering::AcqRel);
}
...
pub fn debug_usb_telemetry_snapshot() -> DebugUsbTelemetrySnapshot {
    DebugUsbTelemetrySnapshot {
        sequence: DEBUG_SEQUENCE.load(Ordering::Acquire),
        last_submit: debug_last_submit(),
        last_event: debug_last_event(),
        last_stream_config: debug_last_stream_config(),
        probe_progress: debug_usb_probe_progress(),
    }
}
