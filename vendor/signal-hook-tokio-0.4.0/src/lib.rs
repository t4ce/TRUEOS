...
pub struct SignalsInfo<E: Exfiltrator = SignalOnly>(signal_hook::iterator::SignalsInfo<E>);
...
impl<E: Exfiltrator> SignalsInfo<E> {
    ...
    where
        ...
        S: Borrow<c_int>,
    {
        signal_hook::iterator::SignalsInfo::with_exfiltrator(signals, exfiltrator).map(Self)
    }
...
#[cfg(all(feature = "futures-v0_3", not(target_os = "trueos")))]
...
#[cfg_attr(docsrs, doc(cfg(feature = "futures-v0_3")))]
#[cfg(all(feature = "futures-v0_3", target_os = "trueos"))]
impl<E: Exfiltrator + Unpin> Stream for SignalsInfo<E> {
    type Item = E::Output;

    fn poll_next(mut self: Pin<&mut Self>, _ctx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        if let Some(signal) = self.0.pending().next() {
