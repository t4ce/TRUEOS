...
unsafe fn format_from_waveformatex_ptr(
    ...
    let sample_format =
        match ((*waveformatex_ptr).wBitsPerSample, (*waveformatex_ptr).wFormatTag as u32) {
            ...
            (n_bits, KernelStreaming::WAVE_FORMAT_EXTENSIBLE) => {
                let waveformatextensible_ptr =
                    waveformatex_ptr as *const Audio::WAVEFORMATEXTENSIBLE;
    ...
    pub fn name(&self) -> Result<String, DeviceNameError> {
        unsafe {
            ...
            if prop_variant.vt != VT_LPWSTR.0 {
                let description =
                    format!("property store produced invalid data: {:?}", prop_variant.vt);
                let err = BackendSpecificError { description };
                return Err(err.into());
            }
