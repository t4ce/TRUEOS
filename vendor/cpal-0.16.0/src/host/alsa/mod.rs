...
impl DeviceTrait for Device {
        ...
        E: FnMut(StreamError) + Send + 'static,
    {
        ...
        let stream =
            Stream::new_input(Arc::new(stream_inner), data_callback, error_callback, timeout);
        Ok(stream)
    }
        ...
        E: FnMut(StreamError) + Send + 'static,
    {
        ...
        let stream =
            Stream::new_output(Arc::new(stream_inner), data_callback, error_callback, timeout);
        Ok(stream)
    }
}
    ...
    loop {
        ...
        match flow {
            ...
            PollDescriptorsFlow::Ready {
                ...
                if let Err(err) =
                    process_input(stream, &mut ctxt.buffer, status, delay_frames, data_callback)
                {
                    error_callback(err.into());
                }
            }
...
impl Stream {
        ...
        E: FnMut(StreamError) + Send + 'static,
    {
        ...
        let thread = thread::Builder::new()
            ...
            .spawn(move || {
                input_stream_worker(rx, &stream, &mut data_callback, &mut error_callback, timeout);
            })
            .unwrap();
        Stream {
            thread: Some(thread),
        ...
        E: FnMut(StreamError) + Send + 'static,
    {
        ...
        let thread = thread::Builder::new()
            ...
            .spawn(move || {
                output_stream_worker(rx, &stream, &mut data_callback, &mut error_callback, timeout);
            })
            .unwrap();
        Stream {
            thread: Some(thread),
