    ...
    E: FnMut(StreamError) + Send + 'static,
{
    ...
    let stream = builder
        ...
        .error_callback(Box::new(move |stream, error| (error_callback)(StreamError::from(error))))
        .open_stream()?;
...
impl DeviceTrait for Device {
        ...
        match &self.0 {
            ...
            Some(info) => {
                let name = if info.address.is_empty() {
                    ...
                    format!("{}:{:?}:{}", info.product_name, info.device_type, info.address)
                };
                Ok(name)
            }
        }
        ...
        E: FnMut(StreamError) + Send + 'static,
    {
        ...
        build_input_stream(self, config, data_callback, error_callback, builder, sample_format)
    }
        ...
        E: FnMut(StreamError) + Send + 'static,
    {
        ...
        build_output_stream(self, config, data_callback, error_callback, builder, sample_format)
    }
}
