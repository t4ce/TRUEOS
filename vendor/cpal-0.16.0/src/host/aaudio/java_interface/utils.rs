...
pub fn call_method_string_arg_ret_bool<'j>(
    ...
    env.call_method(subject, name, "(Ljava/lang/String;)Z", &[(&env.new_string(arg)?).into()])?
        .z()
}
...
pub fn get_package_manager<'j>(
    ...
    env.call_method(subject, "getPackageManager", "()Landroid/content/pm/PackageManager;", &[])?
        .l()
}
...
pub fn get_devices<'j>(
    ...
    env.call_method(subject, "getDevices", "(I)[Landroid/media/AudioDeviceInfo;", &[flags.into()])?
        .l()
        .map(From::from)
}
