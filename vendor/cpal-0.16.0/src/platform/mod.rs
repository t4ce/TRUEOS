...
macro_rules! impl_platform_host {
    ($($(#[cfg($feat: meta)])? $HostVariant:ident $host_mod:ident $host_name:literal),*) => {
        ...
        impl crate::traits::DeviceTrait for Device {
            ...
            fn name(&self) -> Result<alloc::string::String, crate::DeviceNameError> {
                match self.0 {
                    $(
                        $(#[cfg($feat)])?
        ...
        pub fn available_hosts() -> alloc::vec::Vec<HostId> {
            let mut host_ids = alloc::vec![];
            $(
                $(#[cfg($feat)])?
...
mod platform_impl {
    pub use crate::host::trueos::{
        Device as TrueOsDevice, Devices as TrueOsDevices, Host as TrueOsHost,
        Stream as TrueOsStream, SupportedInputConfigs as TrueOsSupportedInputConfigs,
        SupportedOutputConfigs as TrueOsSupportedOutputConfigs,
    };

    impl_platform_host!(TrueOs trueos "TRUEOS");
...
struct NotSendSyncAcrossAllPlatforms(PhantomData<*mut ()>);

impl Default for NotSendSyncAcrossAllPlatforms {
    fn default() -> Self {
        NotSendSyncAcrossAllPlatforms(PhantomData)
    }
}
