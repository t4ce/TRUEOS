...
pub type InputDevices<I> = core::iter::Filter<I, fn(&<I as Iterator>::Item) -> bool>;

/// A host's device iterator yielding only *output* devices.
pub type OutputDevices<I> = core::iter::Filter<I, fn(&<I as Iterator>::Item) -> bool>;
...
impl Data {
    ...
    pub fn bytes_mut(&mut self) -> &mut [u8] {
        ...
        unsafe { core::slice::from_raw_parts_mut(self.data as *mut u8, len) }
    }
        ...
        T: SizedSample,
    {
        if T::FORMAT == self.sample_format {
            // The safety of this block relies on correct construction of the `Data` instance. See
            // the unsafe `from_parts` constructor for these requirements.
            unsafe { Some(core::slice::from_raw_parts(self.data as *const T, self.len)) }
        } else {
            None
        }
    }
...
impl SupportedStreamConfigRange {
    ...
    /// - 44100 (cd quality)
    /// - Max sample rate
    pub fn cmp_default_heuristics(&self, other: &Self) -> core::cmp::Ordering {
