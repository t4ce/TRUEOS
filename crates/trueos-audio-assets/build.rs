...
fn main() {
    println!("cargo:rerun-if-changed=assets/demo.mp3");

    let manifest_dir = PathBuf::from(env::var_os("CARGO_MANIFEST_DIR").unwrap());
    let mp3_path = manifest_dir.join("assets").join("demo.mp3");
    let out_dir = PathBuf::from(env::var_os("OUT_DIR").unwrap());

    if !mp3_path.exists() {
        println!("cargo:warning=trueos-audio-assets: missing {} (embedding empty audio)", mp3_path.display());
    ...
    loop {
        match decoder.next_frame() {
            Ok(frame) => {
                ...
                // If the stream changes format mid-way, we still keep decoding, but
                // report the initial metadata. (Good enough for a demo asset.)
                pcm_i16.extend_from_slice(&frame.data);
            }
            Err(minimp3::Error::Eof) => break,
            Err(e) => {
                println!("cargo:warning=trueos-audio-assets: mp3 decode error: {e:?} (embedding decoded prefix)");
                break;
            }
        }
    }
...
fn write_generated(out_dir: &Path, meta: Option<GeneratedMeta>) -> std::io::Result<()> {
    ...
    match meta {
        None => {
            writeln!(
                f,
                "pub const DEMO: trueos_audio::Pcm<'static> = trueos_audio::Pcm {{ sample_rate_hz: 48_000, channels: 2, samples_interleaved_i16: &[] }};"
            )?;
        }
        Some(meta) => {
            ...
            writeln!(f, "pub static PCM_BYTES: AlignedBytes<{{ include_bytes!(\"{}\").len() }}> = AlignedBytes(*include_bytes!(\"{}\"));", pcm_path.display(), pcm_path.display())?;
            writeln!(f, "const fn samples_from_bytes(bytes: &'static [u8]) -> &'static [i16] {{")?;
            writeln!(f, "    // Safety: PCM_BYTES is 2-byte aligned and produced as little-endian i16.")?;
            writeln!(f, "    let len = bytes.len() / 2;")?;
            writeln!(f, "    unsafe {{ core::slice::from_raw_parts(bytes.as_ptr() as *const i16, len) }}")?;
            writeln!(f, "}}")?;
            writeln!(
                f,
                "pub const DEMO: trueos_audio::Pcm<'static> = trueos_audio::Pcm {{ sample_rate_hz: {}, channels: {}, samples_interleaved_i16: samples_from_bytes(&PCM_BYTES.0) }};",
