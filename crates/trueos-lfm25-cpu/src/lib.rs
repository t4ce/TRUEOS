...
pub const Q8_BLOCK_VALUES: usize = trueos_lfm25_model::lfm25::Q8_0_BLOCK_VALUES;
pub const Q8_BLOCK_BYTES: usize = trueos_lfm25_model::lfm25::Q8_0_BLOCK_BYTES;
pub const HEAD_DIMENSION: usize = trueos_lfm25_model::lfm25::MODEL_HEAD_DIMENSION as usize;
...
impl F32Sidecar {
    pub fn from_artifact(artifact: &[u8]) -> Result<Self, Error> {
        if artifact.len() != F32_SIDECAR_BYTES
            ...
            || artifact.get(96..128)
                != Some(
                    trueos_lfm25_model::lfm25::generated::MODEL_SEAL
                        .tensor_table_sha256
                        .as_slice(),
                )
        {
        ...
        let expected = trueos_lfm25_model::lfm25::generated::TENSORS
            .iter()
            .filter(|descriptor| {
                trueos_lfm25_model::lfm25::TensorFormat::from_raw(descriptor.format)
                    == Some(trueos_lfm25_model::lfm25::TensorFormat::Bf16Le)
            });
...
impl Lfm25Tokenizer {
    pub fn from_artifact(artifact: &[u8]) -> Result<Self, Error> {
        ...
        if version != TOKENIZER_VERSION
            || vocabulary != trueos_lfm25_model::lfm25::MODEL_VOCABULARY_SIZE as usize
            || artifact[40..72] != trueos_lfm25_model::lfm25::PINNED_GGUF_SHA256
        {
            return Err(Error::Artifact);
        }
...
fn packed_q8x16_admitted_shape(columns: usize, rows: usize) -> bool {
    if columns == trueos_lfm25_model::lfm25::MODEL_HIDDEN_SIZE as usize {
        matches!(rows, 512 | 1_024 | 3_072 | 4_608 | 65_536)
    } else {
        columns == trueos_lfm25_model::lfm25::MODEL_FEED_FORWARD_SIZE as usize && rows == 1_024
    }
}
...
pub fn pack_q8x16_model_in_place(model: &mut [u8]) -> Result<PackedQ8x16Stats, Error> {
    ...
    let maximum_tile_bytes = trueos_lfm25_model::lfm25::generated::TENSORS
        .iter()
        .filter(|descriptor| {
            trueos_lfm25_model::lfm25::TensorFormat::from_raw(descriptor.format)
    ...
    for descriptor in trueos_lfm25_model::lfm25::generated::TENSORS {
        if trueos_lfm25_model::lfm25::TensorFormat::from_raw(descriptor.format)
            != Some(trueos_lfm25_model::lfm25::TensorFormat::Q8_0)
        {
            continue;
        }
...
pub fn packed_q8x16_activation_bytes(columns: usize) -> Result<usize, Error> {
    if columns != trueos_lfm25_model::lfm25::MODEL_HIDDEN_SIZE as usize
        && columns != trueos_lfm25_model::lfm25::MODEL_FEED_FORWARD_SIZE as usize
    {
        return Err(Error::Shape);
    }
...
mod tests {
    ...
    fn synthetic_f32_sidecar() -> Vec<u8> {
        ...
        artifact[32..64].copy_from_slice(&trueos_lfm25_model::lfm25::PINNED_GGUF_SHA256);
        artifact[64..96].copy_from_slice(&trueos_lfm25_model::lfm25::PINNED_NATIVE_IMAGE_SHA256);
        artifact[96..128]
            .copy_from_slice(&trueos_lfm25_model::lfm25::generated::MODEL_SEAL.tensor_table_sha256);
        let mut payload_offset = F32_SIDECAR_PAYLOAD_OFFSET;
        let mut value_index = 0usize;
        for (entry_index, descriptor) in trueos_lfm25_model::lfm25::generated::TENSORS
            .iter()
            .filter(|descriptor| {
                trueos_lfm25_model::lfm25::TensorFormat::from_raw(descriptor.format)
                    == Some(trueos_lfm25_model::lfm25::TensorFormat::Bf16Le)
    ...
    fn f32_sidecar_has_exact_generated_ids_elements_and_source_bits() {
        ...
        let expected_ids: Vec<u16> = trueos_lfm25_model::lfm25::generated::TENSORS
            .iter()
            .filter(|descriptor| {
                trueos_lfm25_model::lfm25::TensorFormat::from_raw(descriptor.format)
                    == Some(trueos_lfm25_model::lfm25::TensorFormat::Bf16Le)
