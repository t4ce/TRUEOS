...
fn main() -> Result<(), String> {
    ...
    if gguf.len() != trueos_fpga_abi::lfm25::PINNED_GGUF_BYTES as usize {
        return Err(format!(
            "GGUF bytes {} != pinned {}",
            gguf.len(),
            trueos_fpga_abi::lfm25::PINNED_GGUF_BYTES
        ));
    }
...
fn seal(metadata: TokenizerMetadata) -> Result<Vec<u8>, String> {
    ...
    if encoded_tokens.len() != trueos_fpga_abi::lfm25::MODEL_VOCABULARY_SIZE as usize
    ...
    artifact.extend_from_slice(&trueos_fpga_abi::lfm25::PINNED_GGUF_SHA256);
