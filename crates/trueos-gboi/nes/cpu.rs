...
impl Cpu {
    ...
    fn zp_r(&mut self, bus: &mut impl CpuBus) -> u8 {
        let a = self.read8(bus) as u16;
        bus.cpu_read(a)
    }
    fn zpx_r(&mut self, bus: &mut impl CpuBus) -> u8 {
        let a = self.read8(bus).wrapping_add(self.x) as u16;
        bus.cpu_read(a)
    }
    fn zpy_r(&mut self, bus: &mut impl CpuBus) -> u8 {
        let a = self.read8(bus).wrapping_add(self.y) as u16;
        bus.cpu_read(a)
    }
    fn abs_r(&mut self, bus: &mut impl CpuBus) -> u8 {
        let a = self.read16(bus);
        bus.cpu_read(a)
    }
    fn abx_r(&mut self, bus: &mut impl CpuBus) -> (u8, u32) {
        let base = self.read16(bus);
        let a = base.wrapping_add(self.x as u16);
        let p = if (base & 0xFF00) != (a & 0xFF00) {
            1
        } else {
            0
        };
    ...
    fn zp_a(&mut self, bus: &mut impl CpuBus) -> u16 {
        self.read8(bus) as u16
    }
    fn zpx_a(&mut self, bus: &mut impl CpuBus) -> u16 {
        self.read8(bus).wrapping_add(self.x) as u16
    }
    fn zpy_a(&mut self, bus: &mut impl CpuBus) -> u16 {
        self.read8(bus).wrapping_add(self.y) as u16
    }
    fn abs_a(&mut self, bus: &mut impl CpuBus) -> u16 {
        self.read16(bus)
    }
    fn abx_a(&mut self, bus: &mut impl CpuBus) -> u16 {
        let b = self.read16(bus);
        b.wrapping_add(self.x as u16)
    }
    fn aby_a(&mut self, bus: &mut impl CpuBus) -> u16 {
        let b = self.read16(bus);
    ...
    fn branch(&mut self, bus: &mut impl CpuBus, cond: bool) -> u32 {
        ...
        if cond {
            ...
            let penalty = if (self.pc & 0xFF00) != (new_pc & 0xFF00) {
                1
            } else {
                0
            };
    ...
    pub fn step(&mut self, bus: &mut impl CpuBus) -> u32 {
        ...
        let cycles = match opcode {
            ...
            0xB5 => {
                let v = self.zpx_r(bus);
                self.a = v;
                self.set_zn(v);
                4
            }
            0xAD => {
                let v = self.abs_r(bus);
                self.a = v;
                self.set_zn(v);
                4
            }
            0xBD => {
                let (v, p) = self.abx_r(bus);
                self.a = v;
                self.set_zn(v);
                4 + p
            }
            0xB9 => {
                let (v, p) = self.aby_r(bus);
                self.a = v;
                self.set_zn(v);
                4 + p
            }
            0xA1 => {
                let v = self.izx_r(bus);
                self.a = v;
                self.set_zn(v);
                6
            }
            0xB1 => {
                let (v, p) = self.izy_r(bus);
                self.a = v;
                self.set_zn(v);
                5 + p
            }
            ...
            0x9D => {
                let a = self.abx_a(bus);
                bus.cpu_write(a, self.a);
                5
            }
            0x99 => {
                let a = self.aby_a(bus);
                bus.cpu_write(a, self.a);
                5
            }
            0x81 => {
