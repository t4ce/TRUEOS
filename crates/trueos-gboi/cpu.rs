...
impl Cpu {
    ...
    fn execute(&mut self, op: u8, bus: &mut impl GbBus) -> u32 {
        match op {
            ...
            0x08 => {
                ...
                bus.write(a, self.sp as u8);
                bus.write(a.wrapping_add(1), (self.sp >> 8) as u8);
                5
            }
            0x09 => {
                self.add_hl(self.bc());
            ...
            0x34 => {
                let hl = self.hl();
                let v = self.inc8(bus.read(hl));
                bus.write(hl, v);
                3
            }
            0x35 => {
                let hl = self.hl();
                let v = self.dec8(bus.read(hl));
                bus.write(hl, v);
                3
            }
            0x36 => {
            ...
            0x3F => {
                let c = !self.cf();
                self.f = (self.f & FLAG_Z) | if c { FLAG_C } else { 0 };
                1
            }
    ...
    fn execute_cb(&mut self, bus: &mut impl GbBus) -> u32 {
        ...
        match cb {
            0x00..=0x07 => {
                let res = self.cb_rlc(v);
                self.set_reg(r, res, bus);
            }
            0x08..=0x0F => {
                let res = self.cb_rrc(v);
                self.set_reg(r, res, bus);
            }
            0x10..=0x17 => {
                let res = self.cb_rl(v);
                self.set_reg(r, res, bus);
            }
            0x18..=0x1F => {
                let res = self.cb_rr(v);
                self.set_reg(r, res, bus);
            }
            0x20..=0x27 => {
                let res = self.cb_sla(v);
                self.set_reg(r, res, bus);
            }
            0x28..=0x2F => {
                let res = self.cb_sra(v);
                self.set_reg(r, res, bus);
            }
            0x30..=0x37 => {
                let res = self.cb_swap(v);
                self.set_reg(r, res, bus);
            }
            0x38..=0x3F => {
                let res = self.cb_srl(v);
                self.set_reg(r, res, bus);
            }
            0x40..=0x7F => {
                let bit = (cb >> 3) & 7;
