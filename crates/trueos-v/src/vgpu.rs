...
pub struct IndexedBatchDraw {
    ...
    pub first_index: u32,
    pub base_vertex: i32,
    /// Linear fragment color encoded to the target's RGBA8 sRGB bytes.
    pub rgba8_srgb: u32,
}

/// Bounded render-pass batch used by custom WGPU backends. All ranges share
/// one surface, pipeline and vertex/index binding, matching ordinary repeated
/// `draw_indexed` calls inside one WebGPU render pass.
#[derive(Copy, Clone, Debug, Default, Eq, PartialEq)]
#[repr(C)]
...
/// Version/layout 9 adds a material showcase when bits 13 and 14 are both set;
/// bits 0..2 select one of six built-in metallic/roughness finish records.
/// It retains custom opaque RGB555 (bit 15 set, RGB in bits 0..14).
/// Custom color overrides all other low-bit modes and requires draw group 0.
...
/// default materials. Older kernels must reject layout 9.
pub const RETAINED_VERTEX_LAYOUT_CUBE_PATCH_SEED: u32 = 9;
...
/// artifact. Matrices, compaction, and indirect records remain GPU-authored.
