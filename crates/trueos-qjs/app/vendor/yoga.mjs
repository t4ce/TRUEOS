...
//
// Exposes a yoga-layout-like API surface while delegating layout operations to
// the native TRUEOS Yoga bridge (`trueos:yoga`). This avoids WebAssembly
// requirements in QJS runtimes that do not provide global WebAssembly.
...
export class Node {
	...
	// Native callback plumbing is not wired yet; keep API-compatible behavior.
	// We approximate by applying an immediate unconstrained measure once.
	setMeasureFunc(fn) {
		...
		if (this._measure) {
			try {
				const m = this._measure(
					NaN,
					native.MEASURE_MODE_UNDEFINED ?? 0,
					NaN,
					native.MEASURE_MODE_UNDEFINED ?? 0,
				);
