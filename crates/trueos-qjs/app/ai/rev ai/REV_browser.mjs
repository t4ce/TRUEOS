...
const SURFER_BOOT_STATE = {
  bridgeInstalled: false,
  fullApiInstalled: false,
  runtimeInitStarted: false,
  runtimeInitDone: false,
  runtimeInitError: '',
};
...
installBrowserBootBridge();
ensureBrowserRuntimeBootstrap();
...
const assetManager = {
  ...
  applyResourcesToRows(_rows = []) {
    return true;
  },
  requestAssetsForRows(_rows = []) {
    return true;
  },
  primeHtmlImageUrls(_html = '') {
    return true;
  },
};

function logBrowserBootstrap(line) {
  try {
    console.log(`[surfer bootstrap] ${line}`);
  } catch (_) {}
}

function ensureBrowserRuntimeBootstrap() {
  if (SURFER_BOOT_STATE.runtimeInitDone) return true;
  if (SURFER_BOOT_STATE.runtimeInitStarted) return !SURFER_BOOT_STATE.runtimeInitError;
  SURFER_BOOT_STATE.runtimeInitStarted = true;
  logBrowserBootstrap('runtime init start');
  try {
    SURFER_BOOT_STATE.runtimeInitDone = true;
    logBrowserBootstrap('runtime init ok');
    return true;
  } catch (err) {
    SURFER_BOOT_STATE.runtimeInitError = String(err && err.stack ? err.stack : err);
    logBrowserBootstrap(`runtime init failed ${SURFER_BOOT_STATE.runtimeInitError}`);
    throw err;
  }
}

function installBrowserBootBridge() {
  if (SURFER_BOOT_STATE.bridgeInstalled) return runtime.host.__trueosBrowser;
  SURFER_BOOT_STATE.bridgeInstalled = true;
  runtime.host.__trueosBrowser = {
    setHtml(nextHtml) {
      runtime.host.__trueosUiHtml = String(nextHtml || '');
      return true;
    },
    getHtml() {
      return cachedHtml;
  ...
  logBrowserBootstrap(`boot bridge installed browser=${Number(runtime.host.__trueosBrowserInstanceId || 0) || 0}`);
  return runtime.host.__trueosBrowser;
}
...
function stripTagBodiesForTreeMode(html, tagName) {
  const source = String(html || '');
  const tag = String(tagName || '').trim().toLowerCase();
  if (!source || !tag) return source;
  ...
  while (cursor < source.length) {
    ...
    if (openEnd < 0) {
      out += source.slice(openIdx);
      break;
    }
    const closeIdx = lower.indexOf(closeNeedle, openEnd + 1);
    if (closeIdx < 0) {
      out += source.slice(openIdx, openEnd + 1);
      break;
    }
    out += source.slice(openIdx, openEnd + 1);
    out += source.slice(closeIdx, closeIdx + closeNeedle.length);
    cursor = closeIdx + closeNeedle.length;
  }
  return out;
}

function preprocessHtmlForTreeMode(html) {
  let next = String(html || '');
  if (!next) return next;
  const before = next.length;
  next = stripTagBodiesForTreeMode(next, 'script');
  const afterScript = next.length;
  next = next.replace(/<!--[\s\S]*?-->/g, '');
  const after = next.length;
  try {
    const browserId = Number(runtime.host.__trueosBrowserInstanceId || 0) | 0;
    console.log(
      `[surfer stage] browser=${browserId > 0 ? browserId : 0} preprocessHtmlForTreeMode bytes=${before}->${afterScript}->${after}`,
    );
  } catch (_) {}
  return next;
}
...
function buildDocFromParsed(parsed, vw, context = 'document') {
  try {
    const browserId = Number(runtime.host.__trueosBrowserInstanceId || 0) | 0;
    console.log(`[surfer stage] browser=${browserId > 0 ? browserId : 0} buildDocFromParsed enter vw=${Math.max(1, Number(vw || 1) | 0)} context=${context}`);
  } catch (_) {}
  ...
  try {
    const browserId = Number(runtime.host.__trueosBrowserInstanceId || 0) | 0;
    console.log(`[surfer stage] browser=${browserId > 0 ? browserId : 0} buildDocFromParsed ok rows=${rows.length} content=${layout.contentW}x${layout.contentH}`);
  } catch (_) {}
  return {
    dom: doc,
    css: cssSection,
...
function buildDocFromHtml(html, vw, context = 'document') {
  const source = preprocessHtmlForTreeMode(html);
  let parsed;
  try {
    const browserId = Number(runtime.host.__trueosBrowserInstanceId || 0) | 0;
    console.log(`[surfer stage] browser=${browserId > 0 ? browserId : 0} buildDocFromHtml enter bytes=${source.length} vw=${Math.max(1, Number(vw || 1) | 0)} context=${context}`);
  } catch (_) {}
  try {
    try {
      const browserId = Number(runtime.host.__trueosBrowserInstanceId || 0) | 0;
      console.log(`[surfer stage] browser=${browserId > 0 ? browserId : 0} parse5 enter bytes=${source.length}`);
    } catch (_) {}
    parsed = parse5.parse(source);
    try {
      const browserId = Number(runtime.host.__trueosBrowserInstanceId || 0) | 0;
      console.log(`[surfer stage] browser=${browserId > 0 ? browserId : 0} parse5 ok`);
    } catch (_) {}
  } catch (err) {
    try {
      const browserId = Number(runtime.host.__trueosBrowserInstanceId || 0) | 0;
      console.log(`[surfer stage] browser=${browserId > 0 ? browserId : 0} parse5 failed ${String(err && err.stack ? err.stack : err)}`);
    } catch (_) {}
    raiseBrowserError(
...
function ensureDoc(vw) {
  ...
  try {
    const browserId = Number(runtime.host.__trueosBrowserInstanceId || 0) | 0;
    console.log(`[surfer stage] browser=${browserId > 0 ? browserId : 0} ensureDoc enter vw=${Math.max(1, Number(vw || 1) | 0)} cached=${cachedDoc ? 1 : 0} html_bytes=${cachedHtml.length}`);
  } catch (_) {}
  if (!cachedDoc) {
  ...
  try {
    const browserId = Number(runtime.host.__trueosBrowserInstanceId || 0) | 0;
    const rowCount = Array.isArray(cachedDoc && cachedDoc.rows) ? cachedDoc.rows.length : 0;
    console.log(`[surfer stage] browser=${browserId > 0 ? browserId : 0} ensureDoc ok rows=${rowCount}`);
  } catch (_) {}
  return cachedDoc;
}
...
function setHtml(nextHtml) {
  ...
  try {
    const browserId = Number(runtime.host.__trueosBrowserInstanceId || 0) | 0;
    console.log(`[surfer ok] browser=${browserId > 0 ? browserId : 0} html_bytes=${cachedHtml.length} url=${currentPageUrl || '(unknown)'}`);
  } catch (_) {}
  try {
    const { vw, vh } = computeViewport();
    try {
      const browserId = Number(runtime.host.__trueosBrowserInstanceId || 0) | 0;
      console.log(`[surfer stage] browser=${browserId > 0 ? browserId : 0} setHtml direct-build enter vw=${vw} vh=${vh} bytes=${cachedHtml.length}`);
    } catch (_) {}
    cachedDoc = buildDocFromHtml(effectiveBrowserHtmlSource(), vw, 'setHtml handover document');
    clampScrollForDoc(cachedDoc, vw, vh);
    publishThemeLayoutInteractives(cachedDoc && cachedDoc.themeLayout ? cachedDoc.themeLayout : null);
    try {
      const browserId = Number(runtime.host.__trueosBrowserInstanceId || 0) | 0;
      const rowCount = Array.isArray(cachedDoc && cachedDoc.rows) ? cachedDoc.rows.length : 0;
      console.log(`[surfer stage] browser=${browserId > 0 ? browserId : 0} setHtml direct-build ok rows=${rowCount}`);
    } catch (_) {}
  } catch (err) {
    try {
      const browserId = Number(runtime.host.__trueosBrowserInstanceId || 0) | 0;
      console.log(`[surfer stage] browser=${browserId > 0 ? browserId : 0} setHtml direct-build failed ${String(err && err.stack ? err.stack : err)}`);
    } catch (_) {}
    throw err;
  }
  markBrowserRenderDirty();
  try {
    ...
    try {
      const browserId = Number(runtime.host.__trueosBrowserInstanceId || 0) | 0;
      console.log(`[surfer stage] browser=${browserId > 0 ? browserId : 0} paint after direct-build failed ${String(err && err.stack ? err.stack : err)}`);
    } catch (_) {}
  }
  return true;
}
...
function consumePendingBootHtml() {
  const pending = String(runtime.host.__trueosUiHtml || '');
  const pendingUrl = String(
    runtime.host.__trueosUiHtmlUrl
    || runtime.host.__trueosBrowserCurrentUrl
    || runtime.host.__trueosBrowserUrl
    || currentPageUrl
    || '',
  ...
  delete runtime.host.__trueosUiHtmlUrl;
  if (pendingUrl) {
    setCurrentPageUrl(pendingUrl);
  }
  try {
    console.log(`${surferLogPrefix()} consumePendingBootHtml bytes=${pending.length} url=${currentPageUrl || '(unknown)'}`);
  } catch (_) {}
  try {
    setHtml(pending);
    try {
      const browserId = Number(runtime.host.__trueosBrowserInstanceId || 0) | 0;
      console.log(`[surfer ok] browser=${browserId > 0 ? browserId : 0} handover bytes=${pending.length} url=${currentPageUrl || '(unknown)'}`);
    } catch (_) {}
    return true;
  } catch (err) {
    logBrowserBootstrap(`consumePendingBootHtml failed ${String(err && err.stack ? err.stack : err)}`);
    runtime.host.__trueosUiHtml = pending;
    runtime.host.__trueosUiHtmlUrl = pendingUrl;
    return false;
  }
}
...
function paint() {
  const { vw, vh } = computeViewport();
  try {
    console.log(`${surferLogPrefix()} paint enter viewport=${vw}x${vh} html_bytes=${cachedHtml.length}`);
  } catch (_) {}
  try {
    const browserId = Number(runtime.host.__trueosBrowserInstanceId || 0) | 0;
    console.log(`[surfer stage] browser=${browserId > 0 ? browserId : 0} paint before ensureDoc vw=${vw} vh=${vh} html_bytes=${cachedHtml.length}`);
  } catch (_) {}
  ...
  try {
    ...
    try {
      const browserId = Number(runtime.host.__trueosBrowserInstanceId || 0) | 0;
      const rowCount = Array.isArray(doc && doc.rows) ? doc.rows.length : 0;
      console.log(`[surfer stage] browser=${browserId > 0 ? browserId : 0} paint after ensureDoc rows=${rowCount}`);
    } catch (_) {}
...
logBrowserBootstrap('full api install start');
runtime.host.__trueosBrowser = {
  paint,
  setHtml,
  setNodeHtml,
  ...
  keyboard(input = null, options = null) {
    const parsed = parseKeyboardInput(input, options, (message) => {
      raiseBrowserError('TRUEOS_BROWSER_KEYBOARD_INVALID', message);
    });
...
logBrowserBootstrap('full api object assigned');
SURFER_BOOT_STATE.fullApiInstalled = true;
try {
  const browserId = Number(runtime.host.__trueosBrowserInstanceId || 0) | 0;
  console.log(`[surfer bootstrap] browser api installed browser=${browserId > 0 ? browserId : 0}`);
} catch (err) {
  try {
    console.log(`[surfer bootstrap] browser api installed log failed ${String(err && err.stack ? err.stack : err)}`);
  } catch (_) {}
}
try {
  logBrowserBootstrap('consume pending boot html start');
  consumePendingBootHtml();
  logBrowserBootstrap('consume pending boot html done');
} catch (err) {
  try {
    console.log(`[surfer bootstrap] consumePendingBootHtml top-level failed ${String(err && err.stack ? err.stack : err)}`);
  } catch (_) {}
}
