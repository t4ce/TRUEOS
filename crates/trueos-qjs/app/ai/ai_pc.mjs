...
const AI_PC_TOOL_POLICY = [
  "The autogenerated shell1 command registry tools are currently disabled unless TRUEOS_AI_ENABLE_SHELL1_COMMAND_REGISTRY_TOOLS is enabled.",
  ...
  "Do not invent shell-command capabilities through the registry tool path when those tools are disabled in the current runtime.",
].join(" ");
...
function shellPrint(text) {
  ...
  shellPrintPending += value.replace(/\r\n/g, "\n").replace(/\r/g, "\n");
  for (;;) {
    const newlineIndex = shellPrintPending.indexOf("\n");
    if (newlineIndex < 0) {
      break;
    }
    const line = shellPrintPending.slice(0, newlineIndex);
    shellPrintPending = shellPrintPending.slice(newlineIndex + 1);
    shellEmitLine(line);
  }
}
