...
const AI_PC_TOOL_POLICY = [
  "The autogenerated shell1 command registry tools are enabled by default and should be used when a task clearly maps to a shell1 command.",
  ...
  "For DOM replacement or page-update requests, do not detour through surf, cmd, tlb.*, or other shell registry commands unless the user explicitly asked for shell output instead of a browser DOM change.",
  ...
  "Use the shell1 command registry tools deliberately when they are the cleanest way to act, but prefer driverdev_* and browser_* tools for structured hardware and browser state work.",
].join(" ");
...
function shellEmitLine(text) {
  ...
  if (typeof globalThis.__trueosShell2PrintLine === "function") {
    try {
      globalThis.__trueosShell2PrintLine(value);
      return;
    } catch (_err) {}
  }
...
function buildTools(entry, options = null) {
  ...
  if (computerModeEligible && normalToolCallCount >= DEFAULT_COMPUTER_HANDOFF_AFTER_TOOL_CALLS) {
