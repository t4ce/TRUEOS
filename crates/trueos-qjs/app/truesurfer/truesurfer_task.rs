...
pub struct HostedBrowserInteractiveState {
    pub interactives: Vec<HostedBrowserInteractiveItem>,
}
...
struct BrowserInstanceState {
    ...
    interactive_state: HostedBrowserInteractiveState,
    text_state: HostedBrowserTextState,
    keyboard_events: VecDeque<HostedKeyboardEvent>,
}
...
pub fn hosted_interactive_state_for_browser(
    ...
    with_browser_state(browser_instance_id, |state| state.interactive_state.clone())
        .unwrap_or_default()
}

pub fn hosted_text_state_for_browser(browser_instance_id: u32) -> HostedBrowserTextState {
    with_browser_state(browser_instance_id, |state| state.text_state).unwrap_or_default()
}
...
pub fn queue_hosted_keyboard_events(browser_window_id: u32, events: &[HostedKeyboardEvent]) -> bool {
    ...
    with_browser_state_mut(browser_instance_id, |state| {
        for event in events {
            state.keyboard_events.push_back(event.clone());
        }
        state.interactive_seq = state.interactive_seq.wrapping_add(1);
        true
    })
    .unwrap_or(false)
}
...
unsafe fn dispatch_pending_html(ctx: *mut qjs::JSContext, browser_instance_id: u32) -> bool {
    let Some(pending) = with_browser_state(browser_instance_id, |state| state.pending_html.clone())
        .flatten()
    else {
        return false;
    };
    ...
    let html_js = qjs::JS_NewStringLen(
        ctx,
        pending.html.as_ptr() as *const c_char,
        pending.html.len(),
    );
    ...
    if dispatched {
        let html_len = pending.html.len();
        let _ = with_browser_state_mut(browser_instance_id, |state| {
            state.pending_html = None;
            state.surface_state.content_width = state.surface_state.viewport_width.max(1);
            state.surface_state.content_height = state.surface_state.viewport_height.max(1);
            state.surface_seq = state.surface_seq.wrapping_add(1);
            state.text_seq = state.text_seq.wrapping_add(1);
        });
        log_line(format!(
            "qjs-truesurfer[{}]: dispatched html bytes={} url={}\n",
            browser_instance_id, html_len, pending.url
        ));
    }
...
pub async fn ui2_gfx_browser_task(browser_instance_id: u32) {
    ...
    log_line(format!(
        "qjs-truesurfer[{}]: starting truesurfer bootstrap\n",
        browser_instance_id
    ));
    ...
    log_line(format!(
        "qjs-truesurfer[{}]: runtime loop ended\n",
        browser_instance_id
    ));
}
