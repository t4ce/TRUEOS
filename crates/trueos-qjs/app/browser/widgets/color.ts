...
import { getOrCreateGraphics, getOrCreateText, clearGraphics } from '../pixiReuse';

let sharedVertexColorBuffer: Buffer | null = null;
function getSharedVertexColorBuffer(): Buffer {
  if (sharedVertexColorBuffer) return sharedVertexColorBuffer;
...
export function applyYogaDefaultsColor(yogaNode: any, node: { attrs?: Record<string, string> }, Yoga: any): void {
...
function barycentric(px: number, py: number, ax: number, ay: number, bx: number, by: number, cx: number, cy: number): { w0: number; w1: number; w2: number } {
  const v0x = bx - ax;
  const v0y = by - ay;
  const v1x = cx - ax;
  const v1y = cy - ay;
...
let cachedShader: Shader | null = null;
function getColorMeshShader(): Shader {
...
function makeHexPositions(cx: number, cy: number, r: number): Float32Array {
...
export function sampleColorPickerAtLocal(opts: { lx: number; ly: number; w: number; h: number }): Rgb | null {
  ...
  for (let ti = 0; ti < TRI_INDICES.length; ti += 3) {
    ...
    if (!pointInTri(lx, ly, ax, ay, bx, by, cx2, cy2)) continue;
...
export function renderColorPicker(opts: {
  ...
  setPick: (p: { x: number; y: number } | null) => void;

  requestPaint: (() => void) | null;

  getPointerId: (ev: any) => number;
  setDraggingPointerId: (pid: number | null) => void;
}): void {
  const {
    node,
    container,
  ...
  const getByLabel = (container as any).getChildByLabel as ((l: string) => any) | undefined;
  let mesh = (getByLabel ? getByLabel.call(container, '__mesh') : container.children.find((c: any) => c?.label === '__mesh')) as Mesh | null;
  if (!mesh) {
    ...
    (mesh as any).label = '__mesh';
    container.addChild(mesh);
    (mesh as any).__sizeKey = sizeKey;
  } else if ((mesh as any).__sizeKey !== sizeKey) {
  ...
  if (setAlpha) setAlpha(clamp255(alpha));
