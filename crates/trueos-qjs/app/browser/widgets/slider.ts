...
export type SliderBounds = { x: number; y: number; w: number; h: number; innerPad: number };

export function getOrInitSliderState(map: Map<string, SliderState>, key: string, attrs?: Record<string, string>): SliderState {
  const existing = map.get(key);
  if (existing) return existing;

  const raw = Number(attrs?.value ?? '0');
  const v = Number.isFinite(raw) ? raw : 0;
  const state: SliderState = { value: Math.max(0, Math.min(1, v)) };
  map.set(key, state);
  return state;
}

export function applyYogaDefaultsSlider(yogaNode: any, Yoga: any): void {
...
export function createYogaNodeForSliderLabel(opts: {
  ...
  Yoga: any;
  measurer: { measure: (text: string, maxWidth?: number) => { width: number; height: number } };
}): { yogaNode: any; buildBox: () => any } {
...
export function renderSliderLabel(opts: {
  ...
  container: Container;
  theme: { fontFamily: string; fontSize: number; text: number };
  sliderStates: Map<string, SliderState>;
}): void {
  const { node, container, theme, sliderStates } = opts;
  const sliderKey = node.attrs?.['data-slider-key'];

  let st: SliderState | null = null;
  if (sliderKey) {
...
export function renderSlider(opts: {
  ...
  theme: {
    text: number;
    control: { progress: { border: number; background: number; fill: number } };
  };
  sliderStates: Map<string, SliderState>;
  ...
  // Optional pointerId override for test harnesses (e.g. map mouse to cursor #3).
  getPointerId?: (ev: any) => number;
}): void {
  const { node, container, graphics: g, w, h, absX, absY, theme, sliderStates, sliderBounds, sliderDrags, requestPaint } =
    opts;
  ...
  const innerPad = 3;
  if (key) sliderBounds.set(key, { x: absX, y: absY, w: bw, h: bh, innerPad });
  ...
  container.on('pointerdown', (ev: any) => {
    ...
    const pid = opts.getPointerId ? opts.getPointerId(ev) : Number(ev?.pointerId ?? ev?.data?.pointerId ?? 0);
      if (pid <= 0) return; // Ignore pointerdown when effective pointerId is 0 (noop mode)
    for (const [otherPid, d] of sliderDrags.entries()) {
      if (d.key === key && otherPid !== pid) sliderDrags.delete(otherPid);
    }
