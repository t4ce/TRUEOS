...
export function renderImg(opts) {
    ...
    const svgString = stripUnsupportedSvgText(svgFromSrc ?? (hasSrc ? makeImgPlaceholderSvg(w, h) : makeNeonOrbSvg({ ring: 34, core: 14 })));
    ...
    if (typeof svgFn === 'function') {
        ...
        if (hasSrc && label.trim().length > 0) {
            ...
            t.style.wordWrapWidth = Math.max(0, Math.ceil(w - 16) + WRAP_EPSILON_PX);
            t.position.set(8, 8 + TEXT_BASELINE_NUDGE_Y);
            t.visible = true;
        }
        else {
            const t = container.getChildByLabel
                ? container.getChildByLabel('__label')
                : container.children.find((c) => c?.label === '__label');
            if (t)
                t.visible = false;
        }
        return;
...
export function applyYogaDefaultsImg(yogaNode, node, Yoga) {
