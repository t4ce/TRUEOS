...
function parseIntOrNull(s, lo, hi) {
    const n = Number(s);
...
function parseDateTimeLocalValue(v) {
    ...
    if (y2 == null || mo == null || dd == null || hh == null || mm == null || ss == null)
        return null;
    const wi = clampInt(Math.floor((dd - 1) / 7) + 1, 1, 4);
...
export function applyYogaDefaultsTemporalInput(yogaNode, Yoga, kind) {
...
function drawControlBorder(g, theme, w, h, focusColor) {
...
function drawChevronDown(g, x, y, size, color) {
    const midX = x + size / 2;
    const midY = y + size / 2;
    g.moveTo(x, midY - 2);
...
export function renderTemporalInput(opts) {
    const { node, container, graphics: g, w, h, absX, absY, theme, uiState, getPointerId, getCursorColor, temporalStates, yearSliderOwners, getOrInitInputValue, requestPaint, popupSink, } = opts;
    const key = node.key;
    if (!key)
        return;
    if (!node.tagName)
    ...
    if (kind === 'time')
        inputValue.value = formatTimeValue(st);
    else if (kind === 'month')
        inputValue.value = formatMonthValue(st);
    else if (kind === 'week')
        inputValue.value = formatWeekValue(st);
    else if (kind === 'date')
        inputValue.value = formatDateValue(st);
    else
        inputValue.value = formatDateTimeLocalValue(st);
    ...
    if (kind !== 'datetime-local') {
        ...
        const dt = container.getChildByLabel
            ? container.getChildByLabel('__date')
            : container.children.find((c) => c?.label === '__date');
        const tt = container.getChildByLabel
            ? container.getChildByLabel('__time')
            : container.children.find((c) => c?.label === '__time');
        if (dt)
            dt.visible = false;
        if (tt)
            tt.visible = false;
        ...
        const shownT = container.getChildByLabel
            ? container.getChildByLabel('__shown')
            : container.children.find((c) => c?.label === '__shown');
        if (shownT)
            shownT.visible = false;
...
function drawPanelFrame(panelG, theme, w, h) {
    panelG.rect(0, 0, w, h);
...
function renderYearSlider(opts) {
    const { stage, theme, popup, st, viewportW, viewportH, sliders, sliderBounds, sliderDrags, getPointerId, requestPaint, onChange, } = opts;
    const pad = 10;
    const panelW = 250;
    const panelH = 78;
    let px = popup.absX;
...
export function renderTemporalPopups(opts) {
    const { popups, stage, theme, viewportW, viewportH, temporalStates, getOrInitInputValue, sliders, sliderBounds, sliderDrags, selects, selectPopups, getCursorColor, uiFocus, getPointerId, requestPaint, } = opts;
    const subPopups = [];
    for (const popup of popups) {
        ...
        if (popup.kind === 'month-panel') {
            ...
            if (st.openYear)
                subPopups.push({ kind: 'year-panel', inputKey: popup.inputKey, absX: px, absY: py, anchorW: panelW, anchorH: 0 });
        }
            ...
            panel.on('pointerdown', (ev) => {
                ...
                for (const wh of weekHits) {
                    if (hit(wh)) {
                        ...
                        if (st.kind === 'week')
                            iv.value = formatWeekValue(st);
                        else if (st.kind === 'date')
                            iv.value = formatDateValue(st);
                        else
                            iv.value = formatDateTimeLocalValue(st);
                        // Close.
                        st.openPanel = null;
                        st.openYear = false;
                        st.openMonthGrid = false;
        ...
        if (popup.kind === 'time-panel') {
            ...
            const mkOptions = (count) => Array.from({ length: count }, (_, i) => pad2(i)).join('\n');
            ...
            const rowY = 32;
            const gap = 8;
            const renderSel = (key, x, optsCount) => {
        ...
        if (popup.kind === 'month-grid') {
            renderMonthGrid({
                ...
                onPick: (month) => {
                    ...
                    const iv = getOrInitInputValue(popup.inputKey, { type: 'text' });
                    if (st.kind === 'month')
                        iv.value = formatMonthValue(st);
                    else if (st.kind === 'week')
                        iv.value = formatWeekValue(st);
                    else if (st.kind === 'date')
                        iv.value = formatDateValue(st);
                    else
...
export function closeAllTemporalPopups(temporalStates) {
    let didClose = false;
