...
export function getOrInitSelectState(map, key, initSelectedIndex) {
    ...
    if (existing)
        return existing;
    const st = { selectedIndex: Math.max(0, initSelectedIndex | 0), open: false };
    map.set(key, st);
    return st;
}
export function applyYogaDefaultsSelect(yogaNode, Yoga) {
...
function drawDownChevron(g, x, y, w, h, color) {
    const pad = 4;
    const x0 = x + pad;
    const x1 = x + w - pad;
    const y0 = y + pad;
...
function parseInitSelectedIndex(attrs) {
...
export function renderSelect(opts) {
    const { node, container, graphics: g, w, h, absX, absY, theme, selectStates, uiState, getPointerId, getCursorColor, requestPaint, popupSink } = opts;
    const key = node.key;
    if (!key)
        return;
...
export function renderSelectPopup(opts) {
    ...
    panel.on('pointerdown', (ev) => {
        ...
        const lx = local?.x ?? -1;
        const ly = local?.y ?? -1;
        if (lx < 0 || lx > popup.w || ly < 0 || ly > panelH)
            return;
