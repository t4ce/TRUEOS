...
export function applyYogaDefaultsNumber(yogaNode, Yoga) {
...
function parseNumberAttr(attrs, name, fallback) {
    const v = Number(attrs?.[name] ?? '');
    return Number.isFinite(v) ? v : fallback;
}
function drawChevronUp(g, x, y, w, h, color) {
    const pad = 3;
    const x0 = x + pad;
    const x1 = x + w - pad;
    const y0 = y + pad;
...
function drawChevronDown(g, x, y, w, h, color) {
    const pad = 3;
    const x0 = x + pad;
    const x1 = x + w - pad;
    const y0 = y + pad;
...
export function renderNumberSpinner(opts) {
    ...
    hit.on('pointerdown', (ev) => {
        ...
        const lx = p?.x ?? 0;
        const ly = p?.y ?? 0;
        const dir = hitUp.contains(lx, ly) ? 1 : hitDown.contains(lx, ly) ? -1 : null;
        if (!dir)
            return;
        // Immediate step.
        inc(dir);
        ...
        if (holds && key) {
            ...
            if (prev) {
                if (prev.timeoutId != null)
                    window.clearTimeout(prev.timeoutId);
                if (prev.intervalId != null)
                    window.clearInterval(prev.intervalId);
            }
            const hold = { key, timeoutId: null, intervalId: null };
