...
import { BLOCK_TAGS } from './htmlDefaults';
...
function getRenderMeasure(theme: { fontSize: number; fontFamily: string }): (s: string) => number {
  ...
  return (s: string) => renderMeasureCtx!.measureText(s).width;
}
...
function isElement(node: any): node is { nodeName: string; tagName?: string; childNodes?: any[] } {
  return node && typeof node === 'object' && typeof node.nodeName === 'string' && Array.isArray(node.childNodes);
}

function isText(node: any): node is { nodeName: '#text'; value: string } {
  return node && typeof node === 'object' && node.nodeName === '#text' && typeof node.value === 'string';
}

function getBody(doc: any): any | undefined {
  const html = (doc.childNodes ?? []).find((n: any) => isElement(n) && n.tagName === 'html');
  if (!html) return undefined;
  return (html.childNodes ?? []).find((n: any) => isElement(n) && n.tagName === 'body');
}
...
function toRenderTree(node: any, path = '0'): RenderNode[] {
  ...
  if (tagName === 'input') {
    ...
    if (t === 'time') return [{ kind: 'block', key: `${path}:input`, tagName: 'timeinput', attrs, children: [] }];
    if (t === 'month') return [{ kind: 'block', key: `${path}:input`, tagName: 'monthinput', attrs, children: [] }];
    if (t === 'week') return [{ kind: 'block', key: `${path}:input`, tagName: 'weekinput', attrs, children: [] }];
    if (t === 'date') return [{ kind: 'block', key: `${path}:input`, tagName: 'dateinput', attrs, children: [] }];
    if (t === 'datetime-local')
  ...
  if (tagName === 'svg') {
    ...
    try {
      a['data-svg'] = (parse5 as any).serialize(node as any);
    } catch {
      a['data-svg'] = '';
    }
    return [{ kind: 'block', key: `${path}:${tagName}`, tagName, attrs: a, children: [] }];
  ...
  if (tagName === 'iframe') {
    ...
    if (srcdoc.trim().length > 0) {
      try {
        const doc = parse5.parse(srcdoc) as any;
  ...
  if (tagName === 'select') {
    ...
    const childrenArr: any[] = Array.isArray(node.childNodes) ? node.childNodes : [];
    for (const ch of childrenArr) {
      ...
      const oAttrs = (ch as any).attrs as Array<{ name: string; value: string }> | undefined;
      const hasSelected = Array.isArray(oAttrs) && oAttrs.some((a) => String(a?.name ?? '').toLowerCase() === 'selected');
      if (hasSelected) selectedIndex = Math.max(0, options.length - 1);
    }
  ...
  if (tagName === 'color') {
    const picker: BlockNode = { kind: 'block', key: `${path}:color`, tagName: 'color', attrs, children: [] };

    const mkSpin = (ch: 'r' | 'g' | 'b' | 'a'): BlockNode => ({
      kind: 'block',
      key: `${path}:color-${ch}`,
      tagName: 'number',
      attrs: {
  ...
  if (tagName === 'search') {
    ...
    const inputAttrs: Record<string, string> = { ...(attrs ?? {}) };
  ...
  if (tagName === 'details' || tagName === 'stub') {
    ...
    const summaryEl = (node.childNodes ?? []).find((c: any) => isElement(c) && (c.tagName ?? c.nodeName) === 'summary');
    const summaryTextFallback =
      summaryEl
    ...
    const buildSummaryChildren = (): RenderNode[] => {
      if (!summaryEl) return summaryTextFallback.length > 0 ? [{ kind: 'text', text: summaryTextFallback }] : [];

      const keep: RenderNode[] = [];
      const trailing: RenderNode[] = [];
      let inlineText = '';
      ...
      for (const ch of summaryEl.childNodes ?? []) {
        ...
        if (isElement(ch)) {
          ...
          if (t === 'input' || t === 'button' || t === 'select' || t === 'textarea') {
            ...
            if (isCheckboxOrRadio) trailing.push(...nodes);
      ...
      const textNode: RenderNode[] = txt.length > 0 ? [{ kind: 'text', text: txt }] : [];
    ...
    for (const child of node.childNodes ?? []) {
      ...
      if (BLOCK_TAGS.has(childTag)) children.push(...toRenderTree(child, childPath));
    }
  ...
  for (const child of node.childNodes ?? []) {
    ...
    if (isElement(child)) {
      ...
      if (BLOCK_TAGS.has(childTag)) {
        ...
        if (t.length > 0) childBlocks.push({ kind: 'text', text: t });
        inlineText = '';
  ...
  if (tail.length > 0) childBlocks.push({ kind: 'text', text: tail });

  // Wrap: for BODY/HTML just return its children.
  if (tagName === 'html' || tagName === 'body') return childBlocks;

  out.push({ kind: 'block', key: `${path}:${tagName}`, tagName, attrs, children: childBlocks });
  return out;
}

function attrsToMap(node: any): Record<string, string> | undefined {
...
async function buildLayoutTree(renderNodes: RenderNode[], viewportWidth: number, viewportHeight: number): Promise<LayoutBox> {
  const padding = 12;
  const gap = 8;
  ...
  function yogaForNode(node: RenderNode): { yogaNode: any; buildBox: () => LayoutBox } {
    ...
    if (isHeadingTag(node.tagName)) applyYogaDefaultsHeading(yogaNode, Yoga);

    if (node.tagName === 'hr') applyYogaDefaultsHr(yogaNode, Yoga);
    ...
    if (node.tagName === 'table') applyYogaDefaultsTable(yogaNode, Yoga);
    if (node.tagName === 'tr') applyYogaDefaultsTr(yogaNode, Yoga);
    if (node.tagName === 'td' || node.tagName === 'th') applyYogaDefaultsCell(yogaNode, Yoga);

    if (node.tagName === 'input') applyYogaDefaultsInput(yogaNode, node, Yoga);
    if (node.tagName === 'textarea') applyYogaDefaultsTextarea(yogaNode, Yoga);
    if (node.tagName === 'select') applyYogaDefaultsSelect(yogaNode, Yoga);
    ...
    if (node.tagName === 'img') applyYogaDefaultsImg(yogaNode, node, Yoga);
    if (node.tagName === 'svg') applyYogaDefaultsSvg(yogaNode, node, Yoga);
    if (node.tagName === 'canvas') applyYogaDefaultsCanvas(yogaNode, node, Yoga);
    if (node.tagName === 'iframe') applyYogaDefaultsIframe(yogaNode, node, Yoga);
    if (node.tagName === 'button') applyYogaDefaultsButton(yogaNode, Yoga);
    if (node.tagName === 'dialog') applyYogaDefaultsDialog(yogaNode, Yoga);
    if (node.tagName === 'number') applyYogaDefaultsNumber(yogaNode, Yoga);
    if (node.tagName === 'color') applyYogaDefaultsColor(yogaNode, node, Yoga);
    if (node.tagName === 'searchrow') applyYogaDefaultsSearchRow(yogaNode, Yoga);
    if (node.tagName === 'searchbutton') applyYogaDefaultsSearchButton(yogaNode, Yoga);

    if (node.tagName === 'summary') applyYogaDefaultsSummary(yogaNode, Yoga);
    if (node.tagName === 'details') applyYogaDefaultsDetails(yogaNode, Yoga);

    if (node.tagName === 'barrow') applyYogaDefaultsBarrow(yogaNode, Yoga);
    if (node.tagName === 'progress' || node.tagName === 'meter') applyYogaDefaultsProgressOrMeter(yogaNode, Yoga);
    if (node.tagName === 'slider') applyYogaDefaultsSlider(yogaNode, Yoga);
...
function renderToPixi(app: Application, box: LayoutBox, sceneRoot?: Container) {
  ...
  for (let i = 0; i < box.children.length; i++) {
    ...
    else drawNode(child, contentRoot, baseTextCtx, 0, contentRoot.position.y, rootDialogs, stageClampRect, `root.${i}`, rootOrder++);
  }
...
async function main() {
  ...
  const html = await fetch('/input.html').then((r) => r.text());
