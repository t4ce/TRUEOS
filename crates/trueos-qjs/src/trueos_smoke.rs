...
pub unsafe fn run_pixi_rgbtri_smoke() {
    ...
    if mod_ret.is_exception() {
        ...
        // This smoke intentionally starts a long-running render loop.
        // Do only a short drain so we don't pin BSP waiting for quiescence.
        let _ = drain_jobs_and_promises(rt, ctx, 250);
    }

    drop(vm);
}

/// Persistent Pixi runtime owner.
///
/// This keeps one QuickJS VM alive and continuously pumps jobs/async operations so
/// long-running render loops (RAF/setTimeout/nextTick) do not hit runtime teardown asserts.
pub unsafe fn run_pixi_rgbtri_runtime_forever() -> ! {
    let Some(vm) = qjs::vm::QjsVm::new_node() else {
        log_str("quickjs: pixi-runtime: JS_NewRuntime failed\n");
        loop {
            trueos_cabi_poll_once();
        }
    };
    ...
    log_str("quickjs: pixi-runtime: vm ok\n");

    install_print(ctx);
    qjs::node::install_globals(ctx);

    let mod_filename = b"<runtime-pixi-rgbtri>\0";
    let mut owned: alloc::vec::Vec<u8> = include_str!("../app/pixi/main.mjs")
        .as_bytes()
        .to_vec();
    owned.push(0);

    log_str("quickjs: pixi-runtime: JS_Eval begin\n");
    let mod_ret = qjs::JS_Eval(
        ctx,
        owned.as_ptr() as *const c_char,
        owned.len() - 1,
    ...
    log_str("quickjs: pixi-runtime: JS_Eval end\n");

    if mod_ret.is_exception() {
        log_str("quickjs: pixi-runtime: eval exception\n");
        dump_exception(ctx);
    } else {
        qjs::js_free_value(ctx, mod_ret);
        log_str("quickjs: pixi-runtime: eval ok\n");
    }
    ...
    loop {
        ...
        progress |= qjs::async_ops::pump(ctx);
        progress |= qjs::workers::pump(ctx);
        let _ = drain_pending_jobs(rt, ctx);
