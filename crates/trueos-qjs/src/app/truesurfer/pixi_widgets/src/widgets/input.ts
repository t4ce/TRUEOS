...
import { clampWrappedLines, getCaretIndexFromPoint, wrapFieldTextWithIndices } from './textField';
import { clearContainerEvents, clearGraphics, getOrCreateGraphics, getOrCreateText } from '../pixiReuse';
...
export type InputState = {
  ...
  // Per-pointer selections so multiple cursors can select simultaneously.
  // Each pointerId should have at most one active selection (cleared when that
  // pointer selects in another field).
  selections?: Map<number, { start: number; end: number }>;
};
...
export function applyYogaDefaultsInput(yogaNode: any, node: { attrs?: Record<string, string> }, Yoga: any): void {
...
export function renderInput(opts: {
  ...
  uiState: {
    focusedKeyByPointer: Map<number, string | null>;
    keyboardOwnerPointerId: number;
    inputs: Map<string, InputState>;
    fieldBounds: Map<string, FieldBounds>;
  };

  getOrInitInputState: (key: string, attrs?: Record<string, string>) => InputState;
  clamp: (n: number, lo: number, hi: number) => number;

  // For radio groups.
  radioGroups: Map<string, string[]>;

  // Drag-selection coordination.
  textDrags: Map<number, { key: string; anchor: number }>;

  requestPaint: (() => void) | null;

  // If true, render caret even when not focused (e.g. during multi-pointer drags).
  showCaret?: boolean;

  // Which pointerId's caret should be shown (typically keyboard focus pointerId,
  // or an actively-dragging pointerId).
  caretPointerId?: number | null;

  // Selection color for focus border/caret.
  focusColor?: number;

  // Cursor color for multi-selection highlights.
  ...
  for (const ch of container.children) {
    const lbl = (ch as any).label as string | undefined;
    if (lbl && (lbl.startsWith('__sel:') || lbl === '__caret')) (ch as any).visible = false;
  }
  ...
  container.on('pointerdown', (ev: any) => {
    ...
    if (type === 'checkbox') {
      ...
      if (typeof s.value === 'string') {
        ...
        if (!s.selections) s.selections = new Map();
        ...
        for (const [otherPid, d] of textDrags.entries()) {
          if (d.key === key && otherPid !== pid) textDrags.delete(otherPid);
        }
  ...
  // Cursor hint
  if (type === 'checkbox' || type === 'radio') container.cursor = 'pointer';
