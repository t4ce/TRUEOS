...
function drawMagnifier(g: Graphics, cx: number, cy: number, r: number, color: number): void {
  // Lens circle.
  g.circle(cx, cy, r);
...
export function applyYogaDefaultsSearchRow(yogaNode: any, Yoga: any): void {
...
export function applyYogaDefaultsSearchButton(yogaNode: any, Yoga: any): void {
  yogaNode.setPadding(Yoga.EDGE_LEFT, 0);
  yogaNode.setPadding(Yoga.EDGE_RIGHT, 0);
  yogaNode.setPadding(Yoga.EDGE_TOP, 0);
  yogaNode.setPadding(Yoga.EDGE_BOTTOM, 0);
...
export function renderSearchButton(opts: {
  ...
  theme: {
    text: number;
    control: { button: { fill: number; hoverFill: number; activeFill: number; border: number; radius: number } };
  };

  // Focus plumbing: clicking the icon button should focus the associated input.
  uiState: {
