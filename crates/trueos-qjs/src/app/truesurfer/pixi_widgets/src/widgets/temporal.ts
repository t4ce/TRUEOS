...
function parseYear2FromYYYY(yyyy: string): number | null {
  ...
  for (let i = 0; i < yyyy.length; i += 1) {
    const c = yyyy.charCodeAt(i);
...
function parseTimeValue(v: string): { hour: number; minute: number; second: number } | null {
  ...
  const h = parseIntOrNull(parts[0], 0, 23);
  const mm = parseIntOrNull(parts[1], 0, 59);
  const s = parts.length === 3 ? parseIntOrNull(parts[2], 0, 59) : 0;
...
function parseMonthValue(v: string): { year2: number; month: number } | null {
  ...
  const y2 = parseYear2FromYYYY(parts[0]);
  const mo = parseIntOrNull(parts[1], 1, 12);
...
function parseDateValue(v: string): { year2: number; month: number; weekIndex: number } | null {
  ...
  const y2 = parseYear2FromYYYY(parts[0]);
  const mo = parseIntOrNull(parts[1], 1, 12);
  const dd = parseIntOrNull(parts[2], 1, 31);
...
function parseWeekValue(v: string): { year2: number; month: number; weekIndex: number } | null {
  ...
  const sep = s.indexOf('-W');
  if (sep < 0) return null;
  const y2 = parseYear2FromYYYY(s.slice(0, sep));
  const w = parseIntOrNull(s.slice(sep + 2), 1, 48);
...
function parseDateTimeLocalValue(v: string): {
  ...
  if (sep < 0) return null;
  const dateParts = s.slice(0, sep).split('-');
  const timeParts = s.slice(sep + 1).split(':');
  if (dateParts.length !== 3 || (timeParts.length !== 2 && timeParts.length !== 3)) return null;
  const y2 = parseYear2FromYYYY(dateParts[0]);
  const mo = parseIntOrNull(dateParts[1], 1, 12);
  const dd = parseIntOrNull(dateParts[2], 1, 31);
  const hh = parseIntOrNull(timeParts[0], 0, 23);
  const mm = parseIntOrNull(timeParts[1], 0, 59);
  const ss = timeParts.length === 3 ? parseIntOrNull(timeParts[2], 0, 59) : 0;
