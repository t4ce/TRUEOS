...
export function makeImgPlaceholderSvg(width: number, height: number): string {
...
export function makeImgNoSrcPlaceholderSvg(width: number, height: number): string {
  const w = Math.max(1, Math.floor(width));
  const h = Math.max(1, Math.floor(height));
  return `<?xml version="1.0" encoding="UTF-8"?>
<svg viewBox="0 0 ${w} ${h}" xmlns="http://www.w3.org/2000/svg">
...
export function makeNeonOrbSvg({
  ring = 34,
  core = 14,
  hueA = '#00e5ff',
  hueB = '#ff2bd6',
