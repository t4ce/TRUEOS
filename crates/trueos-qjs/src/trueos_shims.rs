...
// Yoga native shim layer.
//
// Kept inside trueos_shims so layout primitives live in one C-ABI-facing place
// instead of a separate Rust-side yoga module.
pub mod yoga {
    use alloc::vec::Vec;
    use core::ffi::c_void;
    use core::ptr;
    ...
    pub fn config_free(handle: u32) {
        ...
        let idx = (handle - 1) as usize;
        let mut v = CONFIGS.lock();
        let Some(slot) = v.get_mut(idx) else { return };
        let Some(cfg) = *slot else { return };
    ...
    fn ensure_default_config() -> u32 {
        ...
        if h != 0 {
            ...
            let _ =
                DEFAULT_CONFIG_HANDLE.compare_exchange(0, h, Ordering::AcqRel, Ordering::Acquire);
        }
