...
- `lab256_multiphase.cl`: hash-locked 256x256 three-entry artifact wired as TrueOS-Spirit's deterministic ten-frame boot producer through its one-worker Embassy pool and the vGPU/GuC GPGPU lane; it contains persistent Gray-Scott state, compact GPU telemetry, CPU-fed scope history, plasma color, field-warped Mandelbrot, and native premultiplied-alpha background coverage
