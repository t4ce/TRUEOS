...
- `lab256_multiphase.cl`: hash-locked 256x256 three-entry artifact wired as TrueOS-Spirit's continuous 60 Hz cursor-plane producer and as the live `gpgpu test lab256` Shell2/UI4 preview through the vGPU/GuC GPGPU lane; it contains persistent Gray-Scott state, compact GPU telemetry, a warm radial flare with analytical rays and reaction-field turbulence, native premultiplied-alpha background coverage, a preferred-physical-cursor emitter lean plus persistent reaction injection, and one half-second CUR_SURFLIVE-rate status dot
