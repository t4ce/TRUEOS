...
Set TRUEOS_LIMINE_REF/TRUEOS_LIMINE_REPO and build with network access once.",
    ...
    // Some configurations place BIOS HDD bin in build/bin; keep the old Makefile behavior
    // of copying it into share for convenience.
    let build_hdd = paths.build_dir.join("bin").join("limine-bios-hdd.bin");
    if build_hdd.is_file() {
        fs::create_dir_all(paths.share_dir()).expect("create share dir");
        let dst = paths.share_dir().join("limine-bios-hdd.bin");
        let _ = fs::copy(&build_hdd, &dst);
    }
