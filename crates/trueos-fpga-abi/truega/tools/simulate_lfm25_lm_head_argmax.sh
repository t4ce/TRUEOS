...
"$IVERILOG" "${iverilog_base_args[@]}" -g2012 \
  -s truega_lfm25_tied_lm_head_argmax_slot_tb \
  -o "$STAGE_DIR/lm_head_argmax.vvp" "${sources[@]}"
"$VVP" "${vvp_args[@]}" "$STAGE_DIR/lm_head_argmax.vvp"

"$IVERILOG" "${iverilog_base_args[@]}" -g2012 \
  -s truega_lfm25_tied_lm_head_argmax_contract_tb \
  -o "$STAGE_DIR/lm_head_argmax_contract.vvp" "${sources[@]}"
"$VVP" "${vvp_args[@]}" "$STAGE_DIR/lm_head_argmax_contract.vvp"
