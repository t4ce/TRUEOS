...
iverilog_args=(-g2012 -s truega_lfm25_feed_decode_slice_tb
  -o "$STAGE_DIR/feed_decode_slice.vvp")
...
"$IVERILOG" "${iverilog_args[@]}" \
  ...
  "$RTL_DIR/truega_lfm25_decode_engine.v" \
  "$RTL_DIR/truega_lfm25_feed_frontend.v" \
  "$RTL_DIR/truega_lfm25_feed_completion_slot.v" \
  "$RTL_DIR/truega_lfm25_feed_decode_slice.v" \
  "$RTL_DIR/truega_lfm25_feed_decode_slice_tb.sv"

vvp_args=()
if [[ -n "$VVP_MODULE_DIR" ]]; then
  vvp_args=(-M "$VVP_MODULE_DIR")
fi
"$VVP" "${vvp_args[@]}" "$STAGE_DIR/feed_decode_slice.vvp"
