...
iverilog_args=(-g2012 -s truega_lfm25_q8_projection_row_engine_tb
  -o "$STAGE_DIR/q8_projection.vvp")
...
"$IVERILOG" "${iverilog_args[@]}" \
  ...
  "$RTL_DIR/truega_lfm25_q8_projection_row_engine_tb.sv"

vvp_args=()
if [[ -n "$VVP_MODULE_DIR" ]]; then
  vvp_args=(-M "$VVP_MODULE_DIR")
fi
"$VVP" "${vvp_args[@]}" "$STAGE_DIR/q8_projection.vvp"
