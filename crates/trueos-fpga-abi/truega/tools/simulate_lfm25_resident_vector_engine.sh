...
iverilog_args=(-g2012 -s truega_q8_0_dequant_block_slot_tb -o "$STAGE_DIR/dequant.vvp")
...
"$IVERILOG" "${iverilog_args[@]}" \
  ...
  "$RTL_DIR/truega_q8_0_dequant_block_slot_tb.sv"

vvp_args=()
if [[ -n "$VVP_MODULE_DIR" ]]; then
  vvp_args=(-M "$VVP_MODULE_DIR")
fi
"$VVP" "${vvp_args[@]}" "$STAGE_DIR/dequant.vvp"

iverilog_args=(-g2012 -s truega_lfm25_resident_vector_engine_tb -o "$STAGE_DIR/engine.vvp")
if [[ -n "$IVERILOG_BASE" ]]; then
  iverilog_args=(-B "$IVERILOG_BASE" "${iverilog_args[@]}")
fi
"$IVERILOG" "${iverilog_args[@]}" \
  ...
  "$RTL_DIR/truega_lfm25_resident_vector_engine_tb.sv"

"$VVP" "${vvp_args[@]}" "$STAGE_DIR/engine.vvp"
