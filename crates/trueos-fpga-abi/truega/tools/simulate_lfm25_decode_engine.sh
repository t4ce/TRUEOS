...
iverilog_args=(-g2012 -s truega_lfm25_decode_engine_tb
  -o "$STAGE_DIR/decode_engine.vvp")
...
"$IVERILOG" "${iverilog_args[@]}" \
  ...
  "$RTL_DIR/truega_lfm25_decode_engine_tb.sv"

vvp_args=()
if [[ -n "$VVP_MODULE_DIR" ]]; then
  vvp_args=(-M "$VVP_MODULE_DIR")
fi
"$VVP" "${vvp_args[@]}" "$STAGE_DIR/decode_engine.vvp"
