...
iverilog_args=(-g2012 -s truega_lfm25_resident_norm_residual_join_tb
  -o "$STAGE_DIR/resident_norm_residual_join.vvp")
...
"$IVERILOG" "${iverilog_args[@]}" \
  ...
  "$RTL_DIR/truega_lfm25_resident_norm_residual_join.v" \
  "$RTL_DIR/truega_lfm25_resident_norm_residual_join_tb.sv"

vvp_args=()
if [[ -n "$VVP_MODULE_DIR" ]]; then
  vvp_args=(-M "$VVP_MODULE_DIR")
fi
"$VVP" "${vvp_args[@]}" "$STAGE_DIR/resident_norm_residual_join.vvp"
