    ...
    let executor = match (args.executor.as_deref(), arch.executor_required) {
        (None, true) => {
            error(
                ...
                &f.sig,
                "\
No architecture selected for trueos-executor. Make sure you've enabled one of the `arch-*` features in your Cargo.toml.

Alternatively, if you would like to use a custom executor implementation, specify it with the `executor` argument.
For example: `#[trueos_executor::main(entry = ..., executor = \"some_crate::Executor\")]",
            );
            ""
        }
        (Some(x), _) => x,
        (None, _) => "::trueos_executor::Executor",
    };
