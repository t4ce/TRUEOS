#!/usr/bin/env python3
"""Verify that the Kokoro ADL-S artifact retained its SIMD16 DP4A loop."""
...
def resolve_iga(requested: Path | None) -> Path:
    if requested is not None:
        ...
        if candidate.is_file():
            return candidate
        raise SystemExit(f"kokoro-qgemm-isa: missing IGA executable: {candidate}")
    ...
    raise SystemExit("kokoro-qgemm-isa: iga64 was not found")
...
def main() -> int:
    ...
    if len(kernels) != 1 or kernels[0]["kernel_name"] != KERNEL:
        raise SystemExit("kokoro-qgemm-isa: artifact kernel set rejected")
    kernel = kernels[0]
    if (
        ...
        raise SystemExit("kokoro-qgemm-isa: execution environment rejected")

    entry_offset = int(kernel["text"]["entry_offset"])
    entry_size = int(kernel["text"]["entry_size"])
    binary = binary_path.read_bytes()
    if entry_size <= 0 or entry_size > 8192 or entry_offset + entry_size > len(binary):
        raise SystemExit("kokoro-qgemm-isa: kernel entry bounds rejected")

    iga = resolve_iga(args.iga)
    with tempfile.TemporaryDirectory(prefix="trueos-kokoro-qgemm-isa.") as directory:
        ...
        if completed.returncode != 0:
            raise SystemExit("kokoro-qgemm-isa: IGA disassembly failed:\n" + completed.stdout)
    ...
    if dp4a < 1:
        raise SystemExit("kokoro-qgemm-isa: no SIMD16 DP4A instruction found")
    if byte_gathers != 0:
        raise SystemExit(
            f"kokoro-qgemm-isa: packed loop regressed to {byte_gathers} byte gathers"
        )
    if instruction_lines > 600:
        raise SystemExit("kokoro-qgemm-isa: instruction count exceeded guard")

    print(
        "kokoro-qgemm-isa: PASS "
