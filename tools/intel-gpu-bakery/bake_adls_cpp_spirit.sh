...
# Canonical pinned ABI-twin bake for Spirit's two-layer C++ visual suite.
...
"${python_bin}" -B "${tool_dir}/bake.py" \
  ...
  --abi-reference-bin "${kernel_dir}/artifacts/adls/spirit_vfx_background_rgba8.bin" \
...
"${python_bin}" -B "${tool_dir}/bake.py" \
  ...
  --abi-reference-bin "${kernel_dir}/artifacts/adls/spirit_vfx_sprite_rgba8.bin" \
