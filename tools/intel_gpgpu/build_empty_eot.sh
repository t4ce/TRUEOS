...
kernel_src="$repo_root/src/intel/kernels/empty_eot.cl"
out_dir="${1:-$repo_root/bld/intel-gpgpu/adls/empty_eot}"
...
"$ocloc_bin" compile \
    -file "$kernel_src" \
    -device "$device" \
    -output "$out_dir/empty_eot"

printf 'empty_eot: source=%s device=%s out=%s\n' "$kernel_src" "$device" "$out_dir"
