...
kernel_src="$repo_root/src/intel/kernels/copy_rect_rgba8_wide.cl"
out_dir="${1:-$repo_root/bld/intel-gpgpu/adls/copy_rect_rgba8_wide}"
...
"$ocloc_bin" compile \
    -file "$kernel_src" \
    -device "$device" \
    -output "$out_dir/copy_rect_rgba8_wide"

printf 'copy_rect_rgba8_wide: source=%s device=%s out=%s\n' "$kernel_src" "$device" "$out_dir"
