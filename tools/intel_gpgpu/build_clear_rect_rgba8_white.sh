...
kernel_src="$repo_root/src/intel/kernels/clear_rect_rgba8_white.cl"
out_dir="${1:-$repo_root/bld/intel-gpgpu/adls/clear_rect_rgba8_white}"
...
"$ocloc_bin" compile \
    -file "$kernel_src" \
    -device "$device" \
    -output "$out_dir/clear_rect_rgba8_white"

printf 'clear_rect_rgba8_white: source=%s device=%s out=%s\n' "$kernel_src" "$device" "$out_dir"
